 ...
 */
/**
 * ui.js — arkusz i szkielet HTML sceny (joystick, podpowiedź, panel dev).
 */
export const STYLE = `
...
export function szablon({ tekstLadowania = "Ładowanie bohatera…", tekstPodpowiedzi = "Przesuń palcem, by iść w dowolną stronę" } = {}) {
  return `
<canvas class="scena3d-canvas"></canvas>
<div class="scena3d-loading">${tekstLadowania}</div>
<div class="scena3d-hint"><span>${tekstPodpowiedzi}</span></div>
<button class="scena3d-runmode" type="button">bieg: tempo</button>
<div class="scena3d-stick"><div class="scena3d-knob"></div></div>
<div class="scena3d-stickhint">przeciągnij = kierunek &nbsp;·&nbsp; trzymaj dłużej = bieg</div>
<div class="scena3d-controls">
  <button type="button" data-akcja="stop">Stój</button>
  <button type="button" data-akcja="go">Idź</button>
  <button type="button" data-akcja="replay">Powtórz chód</button>
</div>`;
}

let wstrzyknieto = false;
export function wstrzyknijStyl(doc) {
  if (wstrzyknieto || doc.querySelector("style[data-scena3d]")) return;
