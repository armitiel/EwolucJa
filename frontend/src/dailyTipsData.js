/**
 * dailyTipsData.js — baza 540 porad dnia dla 6 archetypów EwolucJA.
 * 
 * Struktura: 6 profili (DT/EM/ST/KR/LD/MD) × 30 dni × 3 sloty (poranek/poludnie/wieczor)
 * 
 * Każda porada zawiera:
 *  - id: unikalny identyfikator "PROFILE-Dxx-Sx"
 *  - profile: kod profilu (DT/EM/ST/KR/LD/MD)
 *  - profileName: pełna nazwa archetypu
 *  - cecha: cecha główna (Ciekawość/Życzliwość/...)
 *  - kraina: kraina domowa archetypu
 *  - day: 1-30
 *  - slot: "poranek" | "poludnie" | "wieczor"
 *  - category: "medrzec" | "misje" | "kraina" | "mentor" (zgodne z PoradyPage.jsx)
 *  - subcategory: subkategoria tematyczna (do dwupoziomowego filtra UI)
 *  - audience: "dziecko" | "rodzic"
 *  - title: krótki tytuł (5-9 słów)
 *  - body: treść porady (1-3 zdania)
 *  - icon: medrzec|zwoj|krysztal|pulse|rodzic|map|mentor|artifact
 *  - tone: magic|leaf|amber|paper|rose
 *  - time: czas czytania "1 min" | "2 min"
 *  - tags: tablica tagów (ASCII, podkreślniki)
 * 
 * Wygenerowano: 2026-05-16
 * Konsultanci: psycholog-dzieciecy-hd, wellbeing-dziecko-hd, pedagog, komunikacja-z-rodzicami, redaktor-jezykowy
 * Wiek docelowy: 6-12 lat. Porady dla dziecka (Mędrczyni mówi) lub dla rodzica (wskazówki).
 */

// ─── Meta archetypów ────────────────────────────────────────────────
export const PROFILES_META = {
  "DT": {
    "name": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań",
    "color": "#5FA76F",
    "short": "Detektyw"
  },
  "EM": {
    "name": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów",
    "color": "#E4779C",
    "short": "Empata"
  },
  "ST": {
    "name": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb",
    "color": "#7A4DC2",
    "short": "Strateg"
  },
  "KR": {
    "name": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów",
    "color": "#EF9F27",
    "short": "Kreator"
  },
  "LD": {
    "name": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń",
    "color": "#E89A3D",
    "short": "Lider"
  },
  "MD": {
    "name": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu",
    "color": "#378ADD",
    "short": "Mediator"
  }
};

// ─── Wszystkie porady (540) ─────────────────────────────────────────
export const DAILY_TIPS = [
  {
    "id": "DT-D01-S1",
    "profile": "DT",
    "day": 1,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "obserwacja",
    "audience": "dziecko",
    "title": "Pierwsze spojrzenie dnia",
    "body": "Zanim wstaniesz, popatrz uważnie na sufit. Co widzisz, czego wczoraj nie zauważyłeś? Każdy nowy dzień to nowa zagadka.",
    "icon": "krysztal",
    "tone": "leaf",
    "time": "1 min",
    "tags": [
      "obserwacja",
      "rytual_poranny",
      "uwaznosc"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D01-S2",
    "profile": "DT",
    "day": 1,
    "slot": "poludnie",
    "category": "mentor",
    "subcategory": "pytania",
    "audience": "rodzic",
    "title": "Pytanie zamiast odpowiedzi",
    "body": "Gdy dziecko o coś pyta, zanim odpowiesz, spytaj: a jak ty myślisz? To rozpędza jego ciekawość zamiast jej gasić.",
    "icon": "rodzic",
    "tone": "paper",
    "time": "1 min",
    "tags": [
      "pytania",
      "rozmowa",
      "ciekawosc"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D01-S3",
    "profile": "DT",
    "day": 1,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "cisza_skupienie",
    "audience": "rodzic",
    "title": "Ciszą można się przywitać",
    "body": "Wieczorem usiądź obok dziecka w ciszy na minutę. Bez pytań o szkołę. Bliskość bez słów też się liczy.",
    "icon": "rodzic",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "cisza",
      "wyciszenie",
      "wieczor"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D02-S1",
    "profile": "DT",
    "day": 2,
    "slot": "poranek",
    "category": "mentor",
    "subcategory": "rytmy_dnia",
    "audience": "rodzic",
    "title": "Spokojne śniadanie bez ekranu",
    "body": "Tropiciel potrzebuje świeżej głowy. Spróbuj dziś włączyć śniadanie bez bajek — usłyszysz pytania, które inaczej giną w hałasie.",
    "icon": "rodzic",
    "tone": "leaf",
    "time": "2 min",
    "tags": [
      "sniadanie",
      "rytmy",
      "bez_ekranu"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D02-S2",
    "profile": "DT",
    "day": 2,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "obserwacja",
    "audience": "dziecko",
    "title": "Misja: pięć kolorów",
    "body": "Rozejrzyj się dookoła. Znajdź pięć rzeczy w tym samym kolorze. Lupa Tropiciela widzi to, co inni przegapiają.",
    "icon": "map",
    "tone": "leaf",
    "time": "2 min",
    "tags": [
      "misja",
      "kolory",
      "obserwacja"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D02-S3",
    "profile": "DT",
    "day": 2,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "pamiec_szczegoly",
    "audience": "rodzic",
    "title": "Trzy szczegóły dnia",
    "body": "Zapytaj dziecko wieczorem o trzy rzeczy, które dziś zauważyło. Krótko, bez oceny. Bliskość rośnie z drobiazgów.",
    "icon": "rodzic",
    "tone": "paper",
    "time": "1 min",
    "tags": [
      "refleksja",
      "pamiec",
      "wieczor"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D03-S1",
    "profile": "DT",
    "day": 3,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "natura",
    "audience": "dziecko",
    "title": "Niebo jak mapa",
    "body": "Wyjrzyj przez okno. Jakiego koloru jest niebo? Czy chmury się ruszają? Niebo to mapa, która zmienia się co minutę.",
    "icon": "krysztal",
    "tone": "leaf",
    "time": "1 min",
    "tags": [
      "natura",
      "niebo",
      "rytual_poranny"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D03-S2",
    "profile": "DT",
    "day": 3,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "pytania",
    "audience": "dziecko",
    "title": "Trzy pytania w głowie",
    "body": "Wymyśl trzy pytania, na które nikt jeszcze nie zna odpowiedzi. Nie musisz ich nikomu mówić. Niech mieszkają w twojej głowie.",
    "icon": "map",
    "tone": "magic",
    "time": "2 min",
    "tags": [
      "pytania",
      "misja",
      "wyobraznia"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D03-S3",
    "profile": "DT",
    "day": 3,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "granice_dt",
    "audience": "rodzic",
    "title": "Gdy pytań jest za dużo",
    "body": "Lawina pytań wieczorem to często znak, że dziecko jest zmęczone. Spróbuj powiedzieć: zapiszmy to na jutro. To nie odrzucenie, to opieka.",
    "icon": "rodzic",
    "tone": "paper",
    "time": "1 min",
    "tags": [
      "granice",
      "wieczor",
      "zmeczenie"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D04-S1",
    "profile": "DT",
    "day": 4,
    "slot": "poranek",
    "category": "mentor",
    "subcategory": "rytmy_dnia",
    "audience": "rodzic",
    "title": "Sen Tropiciela",
    "body": "Dziecko 6-12 lat potrzebuje 9-11 godzin snu. Jeśli rano jest rozdrażnione, sprawdź najpierw godzinę zaśnięcia — nie typ HD.",
    "icon": "rodzic",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "sen",
      "rytmy",
      "wellbeing"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D04-S2",
    "profile": "DT",
    "day": 4,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "eksperyment",
    "audience": "dziecko",
    "title": "Eksperyment: zimne i ciepłe",
    "body": "Połóż jedną rękę na metalowym przedmiocie, drugą na drewnianym. Co czujesz? Dlaczego metal wydaje się zimniejszy, choć ma tę samą temperaturę?",
    "icon": "map",
    "tone": "leaf",
    "time": "2 min",
    "tags": [
      "eksperyment",
      "dotyk",
      "nauka"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D04-S3",
    "profile": "DT",
    "day": 4,
    "slot": "wieczor",
    "category": "medrzec",
    "subcategory": "cisza_skupienie",
    "audience": "dziecko",
    "title": "Oddech jak mgła",
    "body": "Wdech nosem — wyobraź sobie, że wciągasz mgłę z lasu. Wydech ustami — wypuszczasz ją z powrotem. Trzy razy. Cisza w środku.",
    "icon": "medrzec",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "oddech",
      "wyciszenie",
      "wieczor"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D05-S1",
    "profile": "DT",
    "day": 5,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "obserwacja",
    "audience": "dziecko",
    "title": "Co się zmieniło?",
    "body": "Rozejrzyj się po pokoju. Czy coś jest dziś inaczej niż wczoraj? Może coś leży gdzie indziej. Uważne oko widzi drobne zmiany.",
    "icon": "krysztal",
    "tone": "leaf",
    "time": "1 min",
    "tags": [
      "obserwacja",
      "zmiana",
      "rytual_poranny"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D05-S2",
    "profile": "DT",
    "day": 5,
    "slot": "poludnie",
    "category": "mentor",
    "subcategory": "nudy_jako_paliwo",
    "audience": "rodzic",
    "title": "Nuda to nie wróg",
    "body": "Gdy dziecko mówi nudzi mi się, nie sięgaj od razu po ekran. Daj kwadrans pustki. Z nudy często rodzą się najciekawsze pomysły.",
    "icon": "rodzic",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "nuda",
      "kreatywnosc",
      "bez_ekranu"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D05-S3",
    "profile": "DT",
    "day": 5,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "pytania",
    "audience": "rodzic",
    "title": "Pytanie na dobranoc",
    "body": "Zamiast bajki spróbuj jednego pytania: co cię dziś zaciekawiło? Czasem dziecko odpowie dopiero rano. Daj mu czas.",
    "icon": "rodzic",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "pytania",
      "wieczor",
      "refleksja"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D06-S1",
    "profile": "DT",
    "day": 6,
    "slot": "poranek",
    "category": "mentor",
    "subcategory": "relacje",
    "audience": "rodzic",
    "title": "Słuchaj pytań bez oceny",
    "body": "Gdy dziecko zadaje dziwne pytanie, nie mów to głupie. Powiedz: ciekawe, skąd ci to przyszło do głowy? Pytania potrzebują bezpieczeństwa.",
    "icon": "rodzic",
    "tone": "paper",
    "time": "1 min",
    "tags": [
      "relacje",
      "rozmowa",
      "akceptacja"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D06-S2",
    "profile": "DT",
    "day": 6,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "obserwacja",
    "audience": "dziecko",
    "title": "Misja: cień",
    "body": "Znajdź swój cień na podłodze albo na ścianie. Jakiego ma kształtu? Czy jest większy czy mniejszy od ciebie? Cień zdradza wiele tajemnic.",
    "icon": "map",
    "tone": "leaf",
    "time": "2 min",
    "tags": [
      "cien",
      "misja",
      "obserwacja"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D06-S3",
    "profile": "DT",
    "day": 6,
    "slot": "wieczor",
    "category": "kraina",
    "subcategory": "natura",
    "audience": "dziecko",
    "title": "Las Pytań szepcze",
    "body": "W Lesie Pytań każde drzewo zna jedną tajemnicę. Wyobraź sobie, że dotykasz kory. Co ci szepcze drzewo dziś wieczorem?",
    "icon": "artifact",
    "tone": "magic",
    "time": "2 min",
    "tags": [
      "kraina",
      "wyobraznia",
      "wieczor"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D07-S1",
    "profile": "DT",
    "day": 7,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "rytmy_dnia",
    "audience": "dziecko",
    "title": "Co dziś zje twój brzuszek?",
    "body": "Zanim usiądziesz do śniadania, posłuchaj brzucha. Jest głodny czy tylko śpiący? Tropiciel zna sygnały swojego ciała.",
    "icon": "krysztal",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "jedzenie",
      "cialo",
      "rytual_poranny"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D07-S2",
    "profile": "DT",
    "day": 7,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "eksperyment",
    "audience": "dziecko",
    "title": "Eksperyment: kropla wody",
    "body": "Kapnij jedną kroplę wody na talerz. Obserwuj przez minutę. Co się dzieje na brzegach? Świat jest pełen mikro-zagadek.",
    "icon": "map",
    "tone": "leaf",
    "time": "2 min",
    "tags": [
      "eksperyment",
      "woda",
      "nauka"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D07-S3",
    "profile": "DT",
    "day": 7,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "pamiec_szczegoly",
    "audience": "rodzic",
    "title": "Wieczorny dziennik tropów",
    "body": "Zaproponuj dziecku: powiedz mi trzy szczegóły z dziś, których nikt inny nie zauważył. To trening pamięci i bliskość w jednym.",
    "icon": "rodzic",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "rozmowa",
      "pamiec",
      "wieczor"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D08-S1",
    "profile": "DT",
    "day": 8,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "obserwacja",
    "audience": "dziecko",
    "title": "Twarz w lustrze",
    "body": "Spójrz w lustro przez chwilę. Czy twoje oczy są takie same jak wczoraj? A włosy? Nawet ty zmieniasz się każdego dnia.",
    "icon": "krysztal",
    "tone": "leaf",
    "time": "1 min",
    "tags": [
      "lustro",
      "obserwacja",
      "rytual_poranny"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D08-S2",
    "profile": "DT",
    "day": 8,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "natura",
    "audience": "dziecko",
    "title": "Misja: liść albo kamień",
    "body": "Jeśli wyjdziesz dziś na dwór, podnieś jeden liść lub jeden kamień. Obejrzyj z bliska. Co go odróżnia od innych?",
    "icon": "map",
    "tone": "leaf",
    "time": "2 min",
    "tags": [
      "natura",
      "misja",
      "obserwacja"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D08-S3",
    "profile": "DT",
    "day": 8,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "cisza_skupienie",
    "audience": "rodzic",
    "title": "Trzy oddechy razem",
    "body": "Połóż dłoń na brzuchu dziecka i oddychajcie razem trzy razy. Spowolnienie rodzica spowalnia dziecko. To proste i działa.",
    "icon": "rodzic",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "oddech",
      "wyciszenie",
      "wieczor"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D09-S1",
    "profile": "DT",
    "day": 9,
    "slot": "poranek",
    "category": "mentor",
    "subcategory": "granice_dt",
    "audience": "rodzic",
    "title": "Ciekawość też się męczy",
    "body": "Po nocy pełnej pytań mózg potrzebuje spokoju. Rano nie zasypuj dziecka planem dnia. Daj 10 minut ciszy przy śniadaniu.",
    "icon": "rodzic",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "granice",
      "cisza",
      "poranek"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D09-S2",
    "profile": "DT",
    "day": 9,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "pytania",
    "audience": "dziecko",
    "title": "Pytanie do dorosłego",
    "body": "Wybierz jednego dorosłego i zadaj mu pytanie: co cię dziś zaskoczyło? Posłuchaj uważnie. Dorośli też mają swoje tropy.",
    "icon": "map",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "pytania",
      "relacje",
      "misja"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D09-S3",
    "profile": "DT",
    "day": 9,
    "slot": "wieczor",
    "category": "medrzec",
    "subcategory": "obserwacja",
    "audience": "dziecko",
    "title": "Światło w oknie",
    "body": "Zerknij za okno. Jakiego koloru jest niebo o tej porze? Każdy wieczór ma inny odcień. Zapamiętaj dzisiejszy.",
    "icon": "krysztal",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "niebo",
      "obserwacja",
      "wieczor"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D10-S1",
    "profile": "DT",
    "day": 10,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "pytania",
    "audience": "dziecko",
    "title": "Pytanie do siebie",
    "body": "Zapytaj siebie po cichu: co dziś chcę odkryć? Nie musisz mówić tego głośno. Pytanie zna swoją drogę.",
    "icon": "medrzec",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "pytania",
      "rytual_poranny",
      "intencja"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D10-S2",
    "profile": "DT",
    "day": 10,
    "slot": "poludnie",
    "category": "mentor",
    "subcategory": "rytmy_dnia",
    "audience": "rodzic",
    "title": "Ruch w środku dnia",
    "body": "WHO zaleca 60 minut ruchu dziennie. Nie musi to być sport — wystarczy spacer, taniec w kuchni, podchody po klatce. Ciało Tropiciela też tropi.",
    "icon": "rodzic",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "ruch",
      "rytmy",
      "wellbeing"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D10-S3",
    "profile": "DT",
    "day": 10,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "pamiec_szczegoly",
    "audience": "rodzic",
    "title": "Mapa dnia we dwoje",
    "body": "Zaproponuj wieczorem: opowiedz mi swój dzień jak mapę. Posłuchaj bez przerywania. Dziecko samo wybierze, co ważne.",
    "icon": "rodzic",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "pamiec",
      "refleksja",
      "wieczor"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D11-S1",
    "profile": "DT",
    "day": 11,
    "slot": "poranek",
    "category": "mentor",
    "subcategory": "relacje",
    "audience": "rodzic",
    "title": "Pierwsze pięć minut",
    "body": "To, jak zaczynasz dzień z dzieckiem, ustawia ton. Spróbuj dziś przywitać je bez pośpiechu — spojrzeniem, dotykiem, ciepłym dzień dobry.",
    "icon": "rodzic",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "relacje",
      "poranek",
      "bliskosc"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D11-S2",
    "profile": "DT",
    "day": 11,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "eksperyment",
    "audience": "dziecko",
    "title": "Eksperyment: dwa naczynia",
    "body": "Nalej tyle samo wody do dwóch różnych szklanek — wąskiej i szerokiej. Wygląda inaczej, prawda? A jest tyle samo. Oko czasem nas oszukuje.",
    "icon": "map",
    "tone": "leaf",
    "time": "2 min",
    "tags": [
      "eksperyment",
      "woda",
      "nauka"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D11-S3",
    "profile": "DT",
    "day": 11,
    "slot": "wieczor",
    "category": "kraina",
    "subcategory": "natura",
    "audience": "dziecko",
    "title": "Polanka pod gwiazdami",
    "body": "Wyobraź sobie polankę w środku Lasu Pytań. Leżysz na trawie. Nad tobą gwiazdy. Każda zna jedną odpowiedź. Którą wybierasz?",
    "icon": "artifact",
    "tone": "magic",
    "time": "2 min",
    "tags": [
      "kraina",
      "wyobraznia",
      "wieczor"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D12-S1",
    "profile": "DT",
    "day": 12,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "obserwacja",
    "audience": "dziecko",
    "title": "Dźwięki domu",
    "body": "Zatrzymaj się na chwilę. Co słychać w domu? Lodówka? Drzewo za oknem? Twój dom też ma swoją muzykę.",
    "icon": "krysztal",
    "tone": "leaf",
    "time": "1 min",
    "tags": [
      "dzwieki",
      "obserwacja",
      "rytual_poranny"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D12-S2",
    "profile": "DT",
    "day": 12,
    "slot": "poludnie",
    "category": "mentor",
    "subcategory": "nudy_jako_paliwo",
    "audience": "rodzic",
    "title": "Pudełko ze skarbami",
    "body": "Zbierz w jedno pudełko kilka różnych przedmiotów: szyszkę, sznurek, lupę, gumkę. Postaw przy stole. Niech dziecko samo coś z tym wymyśli.",
    "icon": "rodzic",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "nuda",
      "kreatywnosc",
      "zabawa"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D12-S3",
    "profile": "DT",
    "day": 12,
    "slot": "wieczor",
    "category": "medrzec",
    "subcategory": "cisza_skupienie",
    "audience": "dziecko",
    "title": "Cisza w pokoju",
    "body": "Wyłącz wszystko, co da się wyłączyć. Posłuchaj ciszy. Ile sekund wytrzymasz, zanim usłyszysz najcichszy dźwięk?",
    "icon": "medrzec",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "cisza",
      "wyciszenie",
      "wieczor"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D13-S1",
    "profile": "DT",
    "day": 13,
    "slot": "poranek",
    "category": "mentor",
    "subcategory": "rytmy_dnia",
    "audience": "rodzic",
    "title": "Spokojne jedzenie",
    "body": "Dziecko, które je w pośpiechu, nie słyszy swojego ciała. Spróbuj posadzić rodzinę przy stole na 10 minut bez telefonów. Tyle wystarczy.",
    "icon": "rodzic",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "jedzenie",
      "rytmy",
      "wellbeing"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D13-S2",
    "profile": "DT",
    "day": 13,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "obserwacja",
    "audience": "dziecko",
    "title": "Misja: twarze ludzi",
    "body": "Jeśli wyjdziesz dziś z domu, zobacz trzy twarze przechodniów. Jakie miały miny? Twarze to mapy uczuć, które zmieniają się co chwilę.",
    "icon": "map",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "obserwacja",
      "ludzie",
      "misja"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D13-S3",
    "profile": "DT",
    "day": 13,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "pytania",
    "audience": "rodzic",
    "title": "Nie każde pytanie ma odpowiedź",
    "body": "Gdy dziecko pyta o coś trudnego, nie musisz wymyślać. Nie wiem, ciekawe, prawda? buduje zaufanie bardziej niż improwizacja.",
    "icon": "rodzic",
    "tone": "paper",
    "time": "1 min",
    "tags": [
      "pytania",
      "refleksja",
      "wieczor"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D14-S1",
    "profile": "DT",
    "day": 14,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "natura",
    "audience": "dziecko",
    "title": "Roślina w domu",
    "body": "Jeśli masz w domu kwiatka, podejdź do niego. Czy wypuścił nowy listek? Rośliny rosną cicho, ale rosną. Tak jak ty.",
    "icon": "krysztal",
    "tone": "leaf",
    "time": "1 min",
    "tags": [
      "natura",
      "roslina",
      "rytual_poranny"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D14-S2",
    "profile": "DT",
    "day": 14,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "pamiec_szczegoly",
    "audience": "dziecko",
    "title": "Trening pamięci: pięć rzeczy",
    "body": "Wybierz dowolne pięć przedmiotów ze swojego pokoju. Patrz na nie 20 sekund. Odwróć się. Czy pamiętasz wszystkie? Lupa Tropiciela ma dobry trening.",
    "icon": "map",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "pamiec",
      "misja",
      "trening"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D14-S3",
    "profile": "DT",
    "day": 14,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "relacje",
    "audience": "rodzic",
    "title": "Rozmowa bez ekranu",
    "body": "Spróbuj dziś wieczorem 15 minut rozmowy bez tła z telewizora czy telefonu. Tropiciel najlepiej tropi, gdy nie ma szumu.",
    "icon": "rodzic",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "relacje",
      "rozmowa",
      "wieczor"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D15-S1",
    "profile": "DT",
    "day": 15,
    "slot": "poranek",
    "category": "mentor",
    "subcategory": "cisza_skupienie",
    "audience": "rodzic",
    "title": "Minuta ciszy przed startem",
    "body": "Spróbuj dziś nie włączać radia ani telefonu przez pierwszą minutę dnia. Dziecko wybudza się łagodniej, gdy dom jest cichy.",
    "icon": "rodzic",
    "tone": "leaf",
    "time": "1 min",
    "tags": [
      "cisza",
      "rytual_poranny",
      "spokoj"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D15-S2",
    "profile": "DT",
    "day": 15,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "eksperyment",
    "audience": "dziecko",
    "title": "Eksperyment: zapach",
    "body": "Zamknij oczy. Niech ktoś dorosły przyłoży ci do nosa coś z kuchni. Po zapachu zgadnij, co to jest. Nos też ma swoją lupę.",
    "icon": "map",
    "tone": "leaf",
    "time": "2 min",
    "tags": [
      "eksperyment",
      "zapach",
      "zmysly"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D15-S3",
    "profile": "DT",
    "day": 15,
    "slot": "wieczor",
    "category": "kraina",
    "subcategory": "natura",
    "audience": "dziecko",
    "title": "Echo w Lesie Pytań",
    "body": "W głębi lasu jest skała, która powtarza pytania. Wyobraź sobie, że krzyczysz: kim jestem? Co ci odpowiada echo?",
    "icon": "artifact",
    "tone": "magic",
    "time": "2 min",
    "tags": [
      "kraina",
      "wyobraznia",
      "wieczor"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D16-S1",
    "profile": "DT",
    "day": 16,
    "slot": "poranek",
    "category": "mentor",
    "subcategory": "granice_dt",
    "audience": "rodzic",
    "title": "Jedno pytanie naraz",
    "body": "Gdy dziecko sypie pytaniami jak z rękawa, możesz powiedzieć: zatrzymajmy się na tym jednym. Każde pytanie zasługuje na chwilę uwagi.",
    "icon": "rodzic",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "granice",
      "pytania",
      "uwaga"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D16-S2",
    "profile": "DT",
    "day": 16,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "obserwacja",
    "audience": "dziecko",
    "title": "Misja: ślady stóp",
    "body": "Spójrz na swoje buty. Czy mają ślady? Z czego są? Po butach poznasz, gdzie dziś byłeś. Stopy zostawiają historię.",
    "icon": "map",
    "tone": "leaf",
    "time": "2 min",
    "tags": [
      "slady",
      "obserwacja",
      "misja"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D16-S3",
    "profile": "DT",
    "day": 16,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "pamiec_szczegoly",
    "audience": "rodzic",
    "title": "Drobiazg dnia",
    "body": "Wieczorem zapytaj o jeden mały szczegół z dnia — nie o ocenę, nie o sukces. Drobiazgi pokazują, co naprawdę zostaje w pamięci.",
    "icon": "rodzic",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "pamiec",
      "refleksja",
      "wieczor"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D17-S1",
    "profile": "DT",
    "day": 17,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "obserwacja",
    "audience": "dziecko",
    "title": "Pogoda na twarzy",
    "body": "Zerknij za okno. Słońce, deszcz, mgła? Pogoda dnia czasem pasuje do twojego nastroju, a czasem nie. Sprawdź dziś.",
    "icon": "krysztal",
    "tone": "leaf",
    "time": "1 min",
    "tags": [
      "pogoda",
      "obserwacja",
      "rytual_poranny"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D17-S2",
    "profile": "DT",
    "day": 17,
    "slot": "poludnie",
    "category": "mentor",
    "subcategory": "rytmy_dnia",
    "audience": "rodzic",
    "title": "Woda zamiast słodyczy",
    "body": "Tropiciel pełen pytań szybko się odwadnia. Postaw na stole szklankę wody w zasięgu ręki. Mózg lepiej pracuje nawodniony.",
    "icon": "rodzic",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "woda",
      "wellbeing",
      "rytmy"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D17-S3",
    "profile": "DT",
    "day": 17,
    "slot": "wieczor",
    "category": "medrzec",
    "subcategory": "cisza_skupienie",
    "audience": "dziecko",
    "title": "Spokój pod kocem",
    "body": "Wejdź pod koc. Poczuj jego ciężar. Cztery oddechy. Powiedz sobie w myślach: dziś koniec tropienia. Czas na sen.",
    "icon": "medrzec",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "wyciszenie",
      "sen",
      "wieczor"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D18-S1",
    "profile": "DT",
    "day": 18,
    "slot": "poranek",
    "category": "mentor",
    "subcategory": "pytania",
    "audience": "rodzic",
    "title": "Pytaj dziecko o intencję",
    "body": "Rano zapytaj: na co dziś chcesz zwrócić uwagę? Intencja na dzień to lepsza kotwica niż lista zadań.",
    "icon": "rodzic",
    "tone": "paper",
    "time": "1 min",
    "tags": [
      "pytania",
      "intencja",
      "rytual_poranny"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D18-S2",
    "profile": "DT",
    "day": 18,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "natura",
    "audience": "dziecko",
    "title": "Misja: ptak za oknem",
    "body": "Spójrz za okno przez minutę. Czy widzisz jakiegoś ptaka? Jaki ma kolor? Jeśli nie ma — zauważ, że dziś ich nie ma. To też trop.",
    "icon": "map",
    "tone": "leaf",
    "time": "2 min",
    "tags": [
      "natura",
      "ptaki",
      "misja"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D18-S3",
    "profile": "DT",
    "day": 18,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "relacje",
    "audience": "rodzic",
    "title": "Dobranoc bez pośpiechu",
    "body": "Spróbuj dziś nie kończyć dobranocki w biegu. Usiądź na łóżku dwie minuty po. Cisza we dwoje to też rozmowa.",
    "icon": "rodzic",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "relacje",
      "wieczor",
      "bliskosc"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D19-S1",
    "profile": "DT",
    "day": 19,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "obserwacja",
    "audience": "dziecko",
    "title": "Twoje dłonie",
    "body": "Spójrz na swoje dłonie. Linie, paznokcie, kostki. Każda dłoń jest inna na świecie. Twoja jest jedyna. Sprawdzaj ją czasem.",
    "icon": "krysztal",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "cialo",
      "obserwacja",
      "rytual_poranny"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D19-S2",
    "profile": "DT",
    "day": 19,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "eksperyment",
    "audience": "dziecko",
    "title": "Eksperyment: dwa palce",
    "body": "Przyłóż palec wskazujący do nosa z zamkniętymi oczami. Trafiłeś? A teraz drugą ręką. Ciało wie, gdzie jest, nawet po ciemku.",
    "icon": "map",
    "tone": "leaf",
    "time": "1 min",
    "tags": [
      "eksperyment",
      "cialo",
      "zmysly"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D19-S3",
    "profile": "DT",
    "day": 19,
    "slot": "wieczor",
    "category": "kraina",
    "subcategory": "natura",
    "audience": "dziecko",
    "title": "Mgła nad polanką",
    "body": "Wieczorem na polance w Lesie Pytań pojawia się mgła. Wyobraź sobie, że jej kosmyki niosą twoje zmartwienia daleko. Jutro polana będzie czysta.",
    "icon": "artifact",
    "tone": "magic",
    "time": "2 min",
    "tags": [
      "kraina",
      "wyobraznia",
      "wieczor"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D20-S1",
    "profile": "DT",
    "day": 20,
    "slot": "poranek",
    "category": "mentor",
    "subcategory": "nudy_jako_paliwo",
    "audience": "rodzic",
    "title": "Nudny poranek to dobry poranek",
    "body": "Jeśli dziecko ma rano 20 minut bez zadań, niech ma. Z tej pustki rodzą się pytania, które potem zaskoczą cię przy obiedzie.",
    "icon": "rodzic",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "nuda",
      "poranek",
      "kreatywnosc"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D20-S2",
    "profile": "DT",
    "day": 20,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "pytania",
    "audience": "dziecko",
    "title": "Pytanie do rodzica",
    "body": "Zadaj dziś rodzicowi pytanie, którego nigdy nie zadałeś. Może o jego dzieciństwo. Posłuchaj odpowiedzi do końca.",
    "icon": "map",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "pytania",
      "relacje",
      "misja"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D20-S3",
    "profile": "DT",
    "day": 20,
    "slot": "wieczor",
    "category": "medrzec",
    "subcategory": "pamiec_szczegoly",
    "audience": "dziecko",
    "title": "Trzy zapachy dnia",
    "body": "Przypomnij sobie trzy zapachy z dziś. Może chleb, może deszcz, może mama. Zapach to pamięć w niewidzialnej formie.",
    "icon": "zwoj",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "pamiec",
      "zapach",
      "wieczor"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D21-S1",
    "profile": "DT",
    "day": 21,
    "slot": "poranek",
    "category": "mentor",
    "subcategory": "rytmy_dnia",
    "audience": "rodzic",
    "title": "Ciało dziecka po nocy",
    "body": "Zerknij rano, jak dziecko wstaje: powoli, czy zrywa się? To pierwsza informacja o tym, jak spało. Lepsza niż pytanie wprost.",
    "icon": "rodzic",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "cialo",
      "rytual_poranny",
      "sen"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D21-S2",
    "profile": "DT",
    "day": 21,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "obserwacja",
    "audience": "dziecko",
    "title": "Misja: jeden kąt",
    "body": "Wybierz jeden kąt mieszkania. Przyjrzyj się mu uważnie przez minutę. Ile rzeczy zauważasz? Ten kąt zna twoje sekrety.",
    "icon": "map",
    "tone": "leaf",
    "time": "2 min",
    "tags": [
      "obserwacja",
      "misja",
      "dom"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D21-S3",
    "profile": "DT",
    "day": 21,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "granice_dt",
    "audience": "rodzic",
    "title": "Kiedy nie wiem to też odpowiedź",
    "body": "Nie musisz znać odpowiedzi na każde pytanie. Nie wiem, sprawdzimy razem to też dobra odpowiedź. Pokazuje, że nauka nigdy się nie kończy.",
    "icon": "rodzic",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "granice",
      "pytania",
      "szczerosc"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D22-S1",
    "profile": "DT",
    "day": 22,
    "slot": "poranek",
    "category": "mentor",
    "subcategory": "cisza_skupienie",
    "audience": "rodzic",
    "title": "Jedna rzecz po drugiej",
    "body": "Tropiciel źle znosi pośpiech. Spróbuj rano nie wydawać trzech poleceń naraz. Najpierw zęby, potem ubranie. To wystarczy.",
    "icon": "rodzic",
    "tone": "paper",
    "time": "1 min",
    "tags": [
      "skupienie",
      "rytual_poranny",
      "spokoj"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D22-S2",
    "profile": "DT",
    "day": 22,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "eksperyment",
    "audience": "dziecko",
    "title": "Eksperyment: monety",
    "body": "Weź trzy monety. Połóż na stole. Z zamkniętymi oczami dotknij każdej i zgadnij, która jest największa. Palce też widzą.",
    "icon": "map",
    "tone": "leaf",
    "time": "2 min",
    "tags": [
      "eksperyment",
      "dotyk",
      "zmysly"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D22-S3",
    "profile": "DT",
    "day": 22,
    "slot": "wieczor",
    "category": "medrzec",
    "subcategory": "pytania",
    "audience": "dziecko",
    "title": "Pytanie do snu",
    "body": "Połóż jedno pytanie pod poduszkę. To może być nawet: co znaczy moje imię? Sen lubi pracować nocą nad zagadkami.",
    "icon": "zwoj",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "pytania",
      "sen",
      "wieczor"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D23-S1",
    "profile": "DT",
    "day": 23,
    "slot": "poranek",
    "category": "mentor",
    "subcategory": "rytmy_dnia",
    "audience": "rodzic",
    "title": "Świeże powietrze rano",
    "body": "Otwórz na pięć minut okno przy śniadaniu. Świeże powietrze wybudza ciekawość lepiej niż druga kawa. Tropiciel potrzebuje tlenu.",
    "icon": "rodzic",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "powietrze",
      "rytmy",
      "wellbeing"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D23-S2",
    "profile": "DT",
    "day": 23,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "natura",
    "audience": "dziecko",
    "title": "Misja: szukaj wzoru",
    "body": "W naturze szukaj dziś wzoru: linii na korze, żyłek na liściu, spirali muszli. Natura rysuje wzory bez linijki.",
    "icon": "map",
    "tone": "leaf",
    "time": "2 min",
    "tags": [
      "natura",
      "wzory",
      "misja"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D23-S3",
    "profile": "DT",
    "day": 23,
    "slot": "wieczor",
    "category": "kraina",
    "subcategory": "pytania",
    "audience": "dziecko",
    "title": "Strażniczka pytań",
    "body": "W Lesie Pytań mieszka stara sowa, która zbiera pytania dzieci. Wyszepcz jej dziś jedno. Ona zaopiekuje się nim do rana.",
    "icon": "artifact",
    "tone": "magic",
    "time": "2 min",
    "tags": [
      "kraina",
      "wyobraznia",
      "wieczor"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D24-S1",
    "profile": "DT",
    "day": 24,
    "slot": "poranek",
    "category": "mentor",
    "subcategory": "obserwacja",
    "audience": "rodzic",
    "title": "Drobne nawyki budują rytm",
    "body": "Te same kroki rano — kubek, śniadanie, buty — dają dziecku poczucie bezpieczeństwa. Nie musi być spektakularnie, ma być przewidywalnie.",
    "icon": "rodzic",
    "tone": "paper",
    "time": "1 min",
    "tags": [
      "obserwacja",
      "rytual_poranny",
      "ruch"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D24-S2",
    "profile": "DT",
    "day": 24,
    "slot": "poludnie",
    "category": "mentor",
    "subcategory": "relacje",
    "audience": "rodzic",
    "title": "Pytanie zwrotne",
    "body": "Gdy dziecko opowiada o dniu, zadawaj pytania pogłębiające: co ci się tam najbardziej podobało? Pokażesz, że słuchasz naprawdę.",
    "icon": "rodzic",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "relacje",
      "rozmowa",
      "sluchanie"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D24-S3",
    "profile": "DT",
    "day": 24,
    "slot": "wieczor",
    "category": "medrzec",
    "subcategory": "pamiec_szczegoly",
    "audience": "dziecko",
    "title": "Dźwiękowy ślad dnia",
    "body": "Jaki dźwięk zapamiętałeś z dziś? Może śmiech kogoś, może dzwonek. Dźwięki są jak ślady — niewidoczne, ale prawdziwe.",
    "icon": "zwoj",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "pamiec",
      "dzwieki",
      "wieczor"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D25-S1",
    "profile": "DT",
    "day": 25,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "natura",
    "audience": "dziecko",
    "title": "Słońce na ścianie",
    "body": "Czy słońce wchodzi dziś do twojego pokoju? Sprawdź, gdzie pada plama światła. Jutro może być w innym miejscu. Słońce wędruje.",
    "icon": "krysztal",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "natura",
      "swiatlo",
      "rytual_poranny"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D25-S2",
    "profile": "DT",
    "day": 25,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "obserwacja",
    "audience": "dziecko",
    "title": "Misja: obrazek na ścianie",
    "body": "Wybierz jeden obrazek lub zdjęcie w domu. Patrz minutę. Co widzisz, czego dotąd nie zauważyłeś? Obrazek się nie zmienił. Ty tak.",
    "icon": "map",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "obserwacja",
      "misja",
      "dom"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D25-S3",
    "profile": "DT",
    "day": 25,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "granice_dt",
    "audience": "rodzic",
    "title": "Cicha godzina przed snem",
    "body": "Godzina przed snem bez ekranu i bez dyskusji o trudnych sprawach. Tropiciel potrzebuje wyciszenia, żeby zasnąć spokojnie.",
    "icon": "rodzic",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "granice",
      "sen",
      "wieczor"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D26-S1",
    "profile": "DT",
    "day": 26,
    "slot": "poranek",
    "category": "mentor",
    "subcategory": "pytania",
    "audience": "rodzic",
    "title": "Dlaczego trzy razy",
    "body": "Gdy dziecko pyta dlaczego raz za razem, nie znaczy że cię testuje. Mózg składa świat. Spróbuj odpowiedzieć przynajmniej trzy razy spokojnie.",
    "icon": "rodzic",
    "tone": "paper",
    "time": "1 min",
    "tags": [
      "pytania",
      "rytual_poranny",
      "ciekawosc"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D26-S2",
    "profile": "DT",
    "day": 26,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "eksperyment",
    "audience": "dziecko",
    "title": "Eksperyment: lustro w łyżce",
    "body": "Weź czystą łyżkę. Spójrz w jej wypukłą stronę. Teraz w wklęsłą. Dlaczego twoja twarz wygląda inaczej? Lustro też potrafi żartować.",
    "icon": "map",
    "tone": "leaf",
    "time": "2 min",
    "tags": [
      "eksperyment",
      "lustro",
      "nauka"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D26-S3",
    "profile": "DT",
    "day": 26,
    "slot": "wieczor",
    "category": "medrzec",
    "subcategory": "cisza_skupienie",
    "audience": "dziecko",
    "title": "Wewnętrzne światło",
    "body": "Zamknij oczy. Wyobraź sobie małe światełko w środku klatki piersiowej. Z każdym oddechem rośnie spokojnie. To twoje światło na noc.",
    "icon": "medrzec",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "wyciszenie",
      "oddech",
      "wieczor"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D27-S1",
    "profile": "DT",
    "day": 27,
    "slot": "poranek",
    "category": "mentor",
    "subcategory": "rytmy_dnia",
    "audience": "rodzic",
    "title": "Bez biegania rano",
    "body": "Pośpiech rano podkręca układ nerwowy na cały dzień. Spróbuj wstać 10 minut wcześniej. Spokojny start procentuje wieczorem.",
    "icon": "rodzic",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "poranek",
      "rytmy",
      "spokoj"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D27-S2",
    "profile": "DT",
    "day": 27,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "natura",
    "audience": "dziecko",
    "title": "Misja: pogoda nóg",
    "body": "Jeśli możesz, zdejmij na chwilę buty na dworze. Jak czują się stopy: zimno, ciepło, miękko, twardo? Stopy to też zwiadowcy.",
    "icon": "map",
    "tone": "leaf",
    "time": "2 min",
    "tags": [
      "natura",
      "cialo",
      "misja"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D27-S3",
    "profile": "DT",
    "day": 27,
    "slot": "wieczor",
    "category": "kraina",
    "subcategory": "obserwacja",
    "audience": "dziecko",
    "title": "Kompas Cieni świeci",
    "body": "Twój artefakt — Kompas Cieni — świeci wieczorem najmocniej. Pokazuje, że dziś też coś odkryłeś. Nawet jeśli wydaje ci się, że nie.",
    "icon": "artifact",
    "tone": "magic",
    "time": "2 min",
    "tags": [
      "kraina",
      "artefakt",
      "wieczor"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D28-S1",
    "profile": "DT",
    "day": 28,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "obserwacja",
    "audience": "dziecko",
    "title": "Twój nastrój dziś",
    "body": "Zanim zaczniesz dzień, sprawdź: jaki masz nastrój? Spokojny, ciekawy, śpiący, rozdrażniony? Nazwanie nastroju to też trop.",
    "icon": "krysztal",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "nastroj",
      "obserwacja",
      "rytual_poranny"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D28-S2",
    "profile": "DT",
    "day": 28,
    "slot": "poludnie",
    "category": "mentor",
    "subcategory": "nudy_jako_paliwo",
    "audience": "rodzic",
    "title": "Nuda po szkole",
    "body": "Po szkole nie zapełniaj od razu czasu zajęciami. 30 minut nudy daje mózgowi reset. Tropiciel po szkole potrzebuje pustki.",
    "icon": "rodzic",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "nuda",
      "szkola",
      "regeneracja"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D28-S3",
    "profile": "DT",
    "day": 28,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "pytania",
    "audience": "rodzic",
    "title": "Co cię dziś zaskoczyło",
    "body": "Wieczorne pytanie: co cię dziś zaskoczyło? otwiera dziecko bardziej niż jak było w szkole. Daje wybór, co opowiedzieć.",
    "icon": "rodzic",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "pytania",
      "refleksja",
      "wieczor"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D29-S1",
    "profile": "DT",
    "day": 29,
    "slot": "poranek",
    "category": "mentor",
    "subcategory": "relacje",
    "audience": "rodzic",
    "title": "Twoje zmęczenie to też dane",
    "body": "Jeśli jesteś dziś wykończona, to normalne. Tropiciel pyta dużo i potrafi zmęczyć. Możesz powiedzieć: dziś mama jest cicha. To uczciwe.",
    "icon": "rodzic",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "relacje",
      "szczerosc",
      "wellbeing"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D29-S2",
    "profile": "DT",
    "day": 29,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "pamiec_szczegoly",
    "audience": "dziecko",
    "title": "Misja: zapamiętaj trasę",
    "body": "Idąc dziś jakąkolwiek znaną trasą, zapamiętaj trzy nowe szczegóły: znak, drzewo, plama na chodniku. Jutro sprawdź, czy są.",
    "icon": "map",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "pamiec",
      "misja",
      "spacer"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D29-S3",
    "profile": "DT",
    "day": 29,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "cisza_skupienie",
    "audience": "rodzic",
    "title": "Cisza przed snem we dwoje",
    "body": "Posiedź przy dziecku w ciszy przed snem. Bez tabletu, bez bajki. Sama obecność to potężny sygnał: jesteś bezpieczny.",
    "icon": "rodzic",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "kraina",
      "cisza",
      "wieczor"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D30-S1",
    "profile": "DT",
    "day": 30,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "pamiec_szczegoly",
    "audience": "dziecko",
    "title": "30 dni tropienia",
    "body": "Dziś mija miesiąc twoich odkryć. Jaki ślad zapamiętałeś najmocniej? Tropiciel po miesiącu ma już całą torbę skarbów w głowie.",
    "icon": "zwoj",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "pamiec",
      "podsumowanie",
      "rytual_poranny"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D30-S2",
    "profile": "DT",
    "day": 30,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "eksperyment",
    "audience": "dziecko",
    "title": "Misja: pokaż komuś trop",
    "body": "Naucz dziś kogoś bliskiego jednej rzeczy, której nauczyłeś się w tym miesiącu. Może obserwacji, może pytania. Wiedza rośnie, gdy ją dzielisz.",
    "icon": "map",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "misja",
      "dzielenie",
      "relacje"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "DT-D30-S3",
    "profile": "DT",
    "day": 30,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "granice_dt",
    "audience": "rodzic",
    "title": "Miesiąc obok Tropiciela",
    "body": "Miesiąc razem z dociekliwym dzieckiem to wyczyn. Zauważ, co zmieniło się w was — w nim, ale też w tobie. Ciekawość zaraża obie strony.",
    "icon": "rodzic",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "podsumowanie",
      "relacje",
      "wieczor"
    ],
    "profileName": "Tropiciel Tajemnic",
    "cecha": "Ciekawość",
    "kraina": "Las Pytań"
  },
  {
    "id": "EM-D01-S1",
    "profile": "EM",
    "day": 1,
    "slot": "poranek",
    "category": "medrzec",
    "title": "W jakim kolorze jest dzis Twoje serce?",
    "body": "Zamknij oczy i wez jeden powolny oddech. Sprawdz: czy serce jest dzis rozowe, blekitne, a moze zolte? To Twoj pierwszy raport pogody dnia.",
    "icon": "medrzec",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "emocje",
      "oddech"
    ],
    "audience": "dziecko",
    "subcategory": "emocje_rozpoznanie",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D01-S2",
    "profile": "EM",
    "day": 1,
    "slot": "poludnie",
    "category": "misje",
    "title": "Jedno mile slowo do kogos bliskiego",
    "body": "Wybierz jedna osobe i powiedz jej cos cieplego - co lubisz w niej, albo ze cieszysz sie, ze ja widzisz. Krotko, prosto, od serca.",
    "icon": "pulse",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "zyczliwosc",
      "rozmowa"
    ],
    "audience": "dziecko",
    "subcategory": "rozmowa_sluchanie",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D01-S3",
    "profile": "EM",
    "day": 1,
    "slot": "wieczor",
    "category": "mentor",
    "title": "Wieczorne pytanie zamiast pytania o oceny",
    "body": "Zapytaj dzis dziecko: 'jaki dzis miales/mialas moment cieply?'. Empata otwiera sie przez emocje, nie przez ranking dnia. Sluchaj bez poprawek.",
    "icon": "rodzic",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "rozmowa",
      "wieczor"
    ],
    "audience": "rodzic",
    "subcategory": "rozmowa_sluchanie",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D02-S1",
    "profile": "EM",
    "day": 2,
    "slot": "poranek",
    "category": "kraina",
    "title": "Morze Slow budzi sie razem z Toba",
    "body": "W Morzu Slow kazde slowo ma fale. Powiedz dzis rano w glowie jedno dobre slowo dla siebie - niech ta fala plynie z Toba.",
    "icon": "map",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "rytual_poranny",
      "kraina"
    ],
    "audience": "dziecko",
    "subcategory": "rytual_kojacy",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D02-S2",
    "profile": "EM",
    "day": 2,
    "slot": "poludnie",
    "category": "misje",
    "title": "Zauwaz, kto dzis potrzebuje usmiechu",
    "body": "Rozejrzyj sie: ktos w klasie, w domu, na podworku wyglada dzis inaczej? Podejdz i usmiechnij sie. Czasem to wystarczy.",
    "icon": "pulse",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "zyczliwosc",
      "uwaznosc"
    ],
    "audience": "dziecko",
    "subcategory": "emocje_rozpoznanie",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D02-S3",
    "profile": "EM",
    "day": 2,
    "slot": "wieczor",
    "category": "medrzec",
    "title": "Czyje to byly dzis uczucia?",
    "body": "Empata chlonie cudze emocje. Pomysl: czy smutek, ktory dzis czulas, czules, byl naprawde Twoj? A moze ktos obok byl smutny? Oddychaj - to nie wszystko Twoje.",
    "icon": "medrzec",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "granice",
      "emocje"
    ],
    "audience": "rodzic",
    "subcategory": "granica_empaty",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D03-S1",
    "profile": "EM",
    "day": 3,
    "slot": "poranek",
    "category": "mentor",
    "title": "Poranny przedsionek emocji",
    "body": "Zanim wyjdziecie z domu, dajcie sobie 30 sekund ciszy. Empata potrzebuje wewnetrznego progu, by nie wchodzic w swiat z nadmiarem. Oddech razem - w ciszy.",
    "icon": "rodzic",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "oddech",
      "rytual_poranny"
    ],
    "audience": "rodzic",
    "subcategory": "rytual_kojacy",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D03-S2",
    "profile": "EM",
    "day": 3,
    "slot": "poludnie",
    "category": "misje",
    "title": "Wysluchaj kogos przez minute",
    "body": "Znajdz kogos, kto chce cos opowiedziec. Sluchaj bez przerywania, bez rad. Tylko badz. Twoje serce slyszy wiecej niz uszy.",
    "icon": "pulse",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "sluchanie",
      "rozmowa"
    ],
    "audience": "dziecko",
    "subcategory": "rozmowa_sluchanie",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D03-S3",
    "profile": "EM",
    "day": 3,
    "slot": "wieczor",
    "category": "kraina",
    "title": "Muszla Echa szepcze przed snem",
    "body": "Wyobraz sobie, ze trzymasz Muszle Echa przy uchu. Co dzis uslyszala? Jedno wspomnienie z dnia - to Twoj skarb. Schowaj go pod poduszke.",
    "icon": "artifact",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "wieczor",
      "kraina"
    ],
    "audience": "dziecko",
    "subcategory": "emocje_rozpoznanie",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D04-S1",
    "profile": "EM",
    "day": 4,
    "slot": "poranek",
    "category": "medrzec",
    "title": "Twoje serce wie, czego dzis potrzebuje",
    "body": "Poloz dlon na sercu. Zapytaj cicho: czego mi dzis trzeba? Spokoju, ruchu, przytulenia? Posluchaj pierwszej odpowiedzi.",
    "icon": "medrzec",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "uwaznosc",
      "emocje"
    ],
    "audience": "dziecko",
    "subcategory": "emocje_rozpoznanie",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D04-S2",
    "profile": "EM",
    "day": 4,
    "slot": "poludnie",
    "category": "misje",
    "title": "Maly liscik dla kogos z rodziny",
    "body": "Napisz na karteczce jedno zdanie dla kogos z domu: 'jestem szczesliwa, ze jestes'. Zostaw tam, gdzie znajdzie. Male - a porusza fale.",
    "icon": "pulse",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "rodzina",
      "list"
    ],
    "audience": "dziecko",
    "subcategory": "list_komplement",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D04-S3",
    "profile": "EM",
    "day": 4,
    "slot": "wieczor",
    "category": "mentor",
    "title": "Pomoz jej zostawic cudze emocje za drzwiami",
    "body": "Przed snem zrobcie razem gest 'strzepywania' - jak woda splywajaca z dloni. To rytual oddania swiatu tego, co nie nalezy do Waszego dziecka. Nazwij go: 'odkladamy obce'.",
    "icon": "rodzic",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "granice",
      "rytual_wieczorny"
    ],
    "audience": "rodzic",
    "subcategory": "granica_empaty",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D05-S1",
    "profile": "EM",
    "day": 5,
    "slot": "poranek",
    "category": "kraina",
    "title": "Swietlna cma czeka na Twoj oddech",
    "body": "Twoja mala swietlna cma siedzi na ramieniu. Wez trzy oddechy - przy kazdym jej swiatlo robi sie jasniejsze. Gotowa, gotowy na dzien.",
    "icon": "artifact",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "oddech",
      "kraina"
    ],
    "audience": "dziecko",
    "subcategory": "rytual_kojacy",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D05-S2",
    "profile": "EM",
    "day": 5,
    "slot": "poludnie",
    "category": "misje",
    "title": "Zapytaj: co Cie dzis ucieszylo?",
    "body": "Wybierz jedna osobe - siostre, brata, kolezanke, dziadka. Zapytaj: 'co Cie dzis ucieszylo?'. Sluchaj odpowiedzi z prawdziwa ciekawoscia.",
    "icon": "pulse",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "rozmowa",
      "radosc"
    ],
    "audience": "dziecko",
    "subcategory": "rozmowa_sluchanie",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D05-S3",
    "profile": "EM",
    "day": 5,
    "slot": "wieczor",
    "category": "medrzec",
    "title": "Smutek to tez kolor - mozna go pomalowac",
    "body": "Jesli dzis bylo smutno, wez kredke i narysuj ten smutek. Jakim jest ksztaltem? Jakim kolorem? Kiedy go zobaczysz, robi sie lzejszy.",
    "icon": "medrzec",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "emocje",
      "smutek"
    ],
    "audience": "rodzic",
    "subcategory": "emocje_rozpoznanie",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D06-S1",
    "profile": "EM",
    "day": 6,
    "slot": "poranek",
    "category": "mentor",
    "title": "Zapytaj o kolor jej serca dzis rano",
    "body": "Zamiast 'jak sie czujesz?' - sprobuj: 'jakiego koloru jest Twoje serce dzis rano?'. Empata odpowie szczerze, bo ma swoj jezyk. Nie oceniaj koloru - tylko go przyjmij.",
    "icon": "rodzic",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "rozmowa",
      "rytual_poranny"
    ],
    "audience": "rodzic",
    "subcategory": "rozmowa_sluchanie",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D06-S2",
    "profile": "EM",
    "day": 6,
    "slot": "poludnie",
    "category": "misje",
    "title": "Podziel sie jedna rzecza z kims",
    "body": "Kanapka, kredka, miejsce na lawce - cokolwiek. Dziel sie dzis bez powodu. Zyczliwosc nie potrzebuje okazji.",
    "icon": "pulse",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "zyczliwosc",
      "dzielenie"
    ],
    "audience": "dziecko",
    "subcategory": "emocje_rozpoznanie",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D06-S3",
    "profile": "EM",
    "day": 6,
    "slot": "wieczor",
    "category": "kraina",
    "title": "Fala dnia wraca do morza",
    "body": "Wyobraz sobie, ze dzien byl fala. Teraz fala wraca do Morza Slow. Wszystko, co bylo trudne - odplywa. To, co dobre - zostaje na brzegu.",
    "icon": "map",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "wieczor",
      "kraina"
    ],
    "audience": "dziecko",
    "subcategory": "emocje_rozpoznanie",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D07-S1",
    "profile": "EM",
    "day": 7,
    "slot": "poranek",
    "category": "medrzec",
    "title": "Mozesz miec dzis inny kolor niz wczoraj",
    "body": "Wczoraj bylo rozowe - dzis moze byc blekitne. Tak ma byc. Twoje serce nie musi codziennie byc takie samo. To znaczy, ze zyje.",
    "icon": "medrzec",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "emocje",
      "akceptacja"
    ],
    "audience": "dziecko",
    "subcategory": "emocje_rozpoznanie",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D07-S2",
    "profile": "EM",
    "day": 7,
    "slot": "poludnie",
    "category": "misje",
    "title": "Rytual kubka herbaty dla kogos",
    "body": "Zaproponuj mamie, tacie, babci albo bratu - 'zrobie ci herbate' albo 'usiadzmy razem'. Maly gest, duza fala ciepla.",
    "icon": "pulse",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "rodzina",
      "rytual"
    ],
    "audience": "dziecko",
    "subcategory": "emocje_rozpoznanie",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D07-S3",
    "profile": "EM",
    "day": 7,
    "slot": "wieczor",
    "category": "mentor",
    "title": "Niedziela bez 'co dzis robilismy zle'",
    "body": "Empata juz sama analizuje swoje bledy. Dzis wieczorem opowiedzcie tylko o trzech cieplych momentach z tygodnia. Reszta poczeka.",
    "icon": "rodzic",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "wieczor",
      "rozmowa"
    ],
    "audience": "rodzic",
    "subcategory": "rozmowa_sluchanie",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D08-S1",
    "profile": "EM",
    "day": 8,
    "slot": "poranek",
    "category": "kraina",
    "title": "Muszla Echa przypomni Ci, kim jestes",
    "body": "Przyloz dlon do ucha jak do muszli. Co slyszysz? Swoj oddech. Swoje serce. To Ty - pelna, gotowa na dzien.",
    "icon": "artifact",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "rytual_poranny",
      "kraina"
    ],
    "audience": "dziecko",
    "subcategory": "rytual_kojacy",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D08-S2",
    "profile": "EM",
    "day": 8,
    "slot": "poludnie",
    "category": "misje",
    "title": "Narysuj, jak wyglada Twoja dzisiejsza emocja",
    "body": "Wez kartke i kredki. Narysuj, co teraz czujesz - bez nazywania. Niech kolor i ksztalt mowia same. Nikomu nie musisz pokazywac.",
    "icon": "pulse",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "emocje",
      "rysunek"
    ],
    "audience": "dziecko",
    "subcategory": "emocje_rozpoznanie",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D08-S3",
    "profile": "EM",
    "day": 8,
    "slot": "wieczor",
    "category": "medrzec",
    "title": "Mozesz powiedziec 'nie' - lagodnie",
    "body": "Jesli ktos chcial dzis za duzo Twojej uwagi, jutro wolno Ci powiedziec: 'teraz potrzebuje chwili dla siebie'. To nie zle - to madre.",
    "icon": "medrzec",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "granice",
      "asertywnosc"
    ],
    "audience": "rodzic",
    "subcategory": "granica_empaty",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D09-S1",
    "profile": "EM",
    "day": 9,
    "slot": "poranek",
    "category": "mentor",
    "title": "Sprawdz, czy spala gleboko",
    "body": "Empata czesto sni duzo i intensywnie. Jesli dziecko jest dzis rano przytulone do Was bardziej niz zwykle - zwolnijcie tempo. Daj 5 dodatkowych minut na rozbudzenie.",
    "icon": "rodzic",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "sen",
      "rytual_poranny"
    ],
    "audience": "rodzic",
    "subcategory": "rytmy_dnia",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D09-S2",
    "profile": "EM",
    "day": 9,
    "slot": "poludnie",
    "category": "misje",
    "title": "Powiedz komus 'dziekuje, ze jestes'",
    "body": "Wybierz jedna osobe - moze mame, moze kolezanke. Powiedz tylko te trzy slowa: 'dziekuje, ze jestes'. Zobacz, co sie dzieje z jej oczami.",
    "icon": "pulse",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "wdziecznosc",
      "rozmowa"
    ],
    "audience": "dziecko",
    "subcategory": "rozmowa_sluchanie",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D09-S3",
    "profile": "EM",
    "day": 9,
    "slot": "wieczor",
    "category": "kraina",
    "title": "Swietlik nad lozkiem",
    "body": "Wyobraz sobie malego swietlika krazacego nad lozkiem. Trzyma w skrzydlach wszystkie dzisiejsze cieple momenty. Czuwa, gdy zasypiasz.",
    "icon": "artifact",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "wieczor",
      "kraina"
    ],
    "audience": "dziecko",
    "subcategory": "emocje_rozpoznanie",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D10-S1",
    "profile": "EM",
    "day": 10,
    "slot": "poranek",
    "category": "medrzec",
    "title": "Twoje serce widzi to, czego oczy nie widza",
    "body": "Dzis, gdy spotkasz kogos, popatrz nie tylko na usmiech. Popatrz glebiej. Twoje serce wie, jak naprawde ktos sie czuje. To Twoj dar.",
    "icon": "medrzec",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "empatia",
      "uwaznosc"
    ],
    "audience": "dziecko",
    "subcategory": "empatia_komus",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D10-S2",
    "profile": "EM",
    "day": 10,
    "slot": "poludnie",
    "category": "misje",
    "title": "Zapytaj kogos, jak sie naprawde czuje",
    "body": "Wiele osob mowi 'dobrze', kiedy wcale tak nie jest. Zapytaj jeszcze raz, ciszej: 'a tak naprawde?'. Sluchaj sercem.",
    "icon": "pulse",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "rozmowa",
      "empatia"
    ],
    "audience": "dziecko",
    "subcategory": "rozmowa_sluchanie",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D10-S3",
    "profile": "EM",
    "day": 10,
    "slot": "wieczor",
    "category": "mentor",
    "title": "Wieczorne 'odkladanie' cudzego",
    "body": "Powiedzcie razem przed snem: 'co bylo moje, zostaje. co bylo cudze, oddaje swiatu.' Empata potrzebuje rytualu rozdzielania. To go uspokaja.",
    "icon": "rodzic",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "granice",
      "rytual_wieczorny"
    ],
    "audience": "rodzic",
    "subcategory": "granica_empaty",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D11-S1",
    "profile": "EM",
    "day": 11,
    "slot": "poranek",
    "category": "kraina",
    "title": "Morze dzis szepcze: 'idz lagodnie'",
    "body": "Wsluchaj sie rano w siebie jak w morze. Czy fale sa spokojne czy wzburzone? Idz w dzien w takim tempie, jakim szepcze Twoje serce.",
    "icon": "map",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "kraina",
      "tempo"
    ],
    "audience": "dziecko",
    "subcategory": "emocje_rozpoznanie",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D11-S2",
    "profile": "EM",
    "day": 11,
    "slot": "poludnie",
    "category": "misje",
    "title": "Przytul kogos dluzej niz zwykle",
    "body": "Zwyczajne przytulenie trwa 2 sekundy. Sprobuj przytulic mame, tate albo dziadka przez 5 sekund. Policz w glowie. Poczujesz roznice.",
    "icon": "pulse",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "przytulenie",
      "rodzina"
    ],
    "audience": "dziecko",
    "subcategory": "przytulenie_dotyk",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D11-S3",
    "profile": "EM",
    "day": 11,
    "slot": "wieczor",
    "category": "medrzec",
    "title": "Zlosc tez ma kolor",
    "body": "Jesli dzis bylas, byles zla, zly - to w porzadku. Zlosc nie znaczy, ze jestes niedobra. Zlosc to sygnal, ze cos bylo za duzo. Posluchaj jej.",
    "icon": "medrzec",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "emocje",
      "zlosc"
    ],
    "audience": "rodzic",
    "subcategory": "emocje_rozpoznanie",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D12-S1",
    "profile": "EM",
    "day": 12,
    "slot": "poranek",
    "category": "mentor",
    "title": "Empata potrzebuje cichego sniadania",
    "body": "Bajka w tle, telewizor, glosne radio - to dla Empaty juz za duzo bodzcow z rana. Sprobujcie dzis zjesc w ciszy lub przy cichej muzyce. Zobaczcie roznice.",
    "icon": "rodzic",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "rytual_poranny",
      "bodzce"
    ],
    "audience": "rodzic",
    "subcategory": "rytual_kojacy",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D12-S2",
    "profile": "EM",
    "day": 12,
    "slot": "poludnie",
    "category": "misje",
    "title": "Powiedz jedno dobre o sobie",
    "body": "Empata mowi dobrze o innych. Dzis powiedz jedna dobra rzecz o sobie - w glowie albo na glos. 'Jestem dobra/dobry, bo...'. Tyle wystarczy.",
    "icon": "pulse",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "samoakceptacja",
      "uwaznosc"
    ],
    "audience": "dziecko",
    "subcategory": "emocje_rozpoznanie",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D12-S3",
    "profile": "EM",
    "day": 12,
    "slot": "wieczor",
    "category": "kraina",
    "title": "Muszla Echa pamieta smiech",
    "body": "Przed snem przyloz dlon do ucha jak do muszli. Przypomnij jeden smiech z dnia - swoj albo cudzy. Muszla zachowa go do jutra.",
    "icon": "artifact",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "wieczor",
      "radosc"
    ],
    "audience": "dziecko",
    "subcategory": "emocje_rozpoznanie",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D13-S1",
    "profile": "EM",
    "day": 13,
    "slot": "poranek",
    "category": "medrzec",
    "title": "Nie wszystko, co czujesz, jest Twoje",
    "body": "Jesli rano jest Ci dziwnie smutno bez powodu - moze to nie Twoje. Moze ktos obok sie martwi. Powiedz sobie: 'sprawdzam - co jest moje?'.",
    "icon": "medrzec",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "granice",
      "emocje"
    ],
    "audience": "dziecko",
    "subcategory": "granica_empaty",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D13-S2",
    "profile": "EM",
    "day": 13,
    "slot": "poludnie",
    "category": "misje",
    "title": "Mala pomoc bez proszenia",
    "body": "Zrob dzis jedna drobna rzecz, o ktora nikt nie prosil - odloz buty kogos, podaj wode, pozbieraj zabawki rodzenstwa. Cicha zyczliwosc smakuje najlepiej.",
    "icon": "pulse",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "zyczliwosc",
      "rodzina"
    ],
    "audience": "dziecko",
    "subcategory": "emocje_rozpoznanie",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D13-S3",
    "profile": "EM",
    "day": 13,
    "slot": "wieczor",
    "category": "mentor",
    "title": "Pytanie, ktore otwiera Empate",
    "body": "Zamiast 'jak bylo w szkole?' - sprobuj: 'kto dzis byl dla Ciebie mily?' albo 'kogo dzis bylo Ci zal?'. Te pytania trafiaja do jej jezyka.",
    "icon": "rodzic",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "rozmowa",
      "wieczor"
    ],
    "audience": "rodzic",
    "subcategory": "rozmowa_sluchanie",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D14-S1",
    "profile": "EM",
    "day": 14,
    "slot": "poranek",
    "category": "kraina",
    "title": "Swietlik mruga: 'gotowa, gotowy?'",
    "body": "Twoja swietlna cma siadla na palcu. Mrugnij do niej. Powiedz: 'dzis idziemy razem'. Macie sie nawzajem przez caly dzien.",
    "icon": "artifact",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "rytual_poranny",
      "kraina"
    ],
    "audience": "dziecko",
    "subcategory": "rytual_kojacy",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D14-S2",
    "profile": "EM",
    "day": 14,
    "slot": "poludnie",
    "category": "misje",
    "title": "Uwazne 'dzien dobry' do kogos",
    "body": "Powiedz dzis 'dzien dobry' patrzac komus w oczy - pani w sklepie, sasiadowi, nauczycielce. Trzy sekundy uwagi. Dla niego to duzo.",
    "icon": "pulse",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "zyczliwosc",
      "uwaznosc"
    ],
    "audience": "dziecko",
    "subcategory": "emocje_rozpoznanie",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D14-S3",
    "profile": "EM",
    "day": 14,
    "slot": "wieczor",
    "category": "medrzec",
    "title": "Samotnosc tez jest fala",
    "body": "Jesli dzis czulas, czules sie sama, sam - to nie znaczy, ze jestes. To uczucie przychodzi i odchodzi. Poloz dlon na sercu: 'jestem ze soba'.",
    "icon": "medrzec",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "samotnosc",
      "wieczor"
    ],
    "audience": "rodzic",
    "subcategory": "samotnosc",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D15-S1",
    "profile": "EM",
    "day": 15,
    "slot": "poranek",
    "category": "mentor",
    "title": "Polowa drogi - zobaczcie, co sie zmienilo",
    "body": "Dwa tygodnie razem. Zapytajcie sie nawzajem rano: 'co juz mi sie udalo zauwazyc w sobie?'. Empata kwitnie, gdy ktos widzi jej drobne kroki.",
    "icon": "rodzic",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "rozmowa",
      "podsumowanie"
    ],
    "audience": "rodzic",
    "subcategory": "rozmowa_sluchanie",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D15-S2",
    "profile": "EM",
    "day": 15,
    "slot": "poludnie",
    "category": "misje",
    "title": "Spacer i jedna rzecz, ktorej sluchasz",
    "body": "Idz dzis chwile pieszo. Sluchaj jednej rzeczy - wiatru, krokow, ptakow. Swiat ma duzo do powiedzenia, gdy mu pozwolisz.",
    "icon": "pulse",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "spacer",
      "uwaznosc"
    ],
    "audience": "dziecko",
    "subcategory": "emocje_rozpoznanie",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D15-S3",
    "profile": "EM",
    "day": 15,
    "slot": "wieczor",
    "category": "kraina",
    "title": "Morze Slow swieci dzis delikatnie",
    "body": "Wieczorem Morze Slow jest jak srebrne lustro. Powiedz jedno slowo, ktore chcesz tam zostawic. Niech poplynie. Wraca jutro lzejsze.",
    "icon": "map",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "wieczor",
      "kraina"
    ],
    "audience": "dziecko",
    "subcategory": "emocje_rozpoznanie",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D16-S1",
    "profile": "EM",
    "day": 16,
    "slot": "poranek",
    "category": "medrzec",
    "title": "Twoje 'czuje' jest madroscia",
    "body": "Kiedy mowisz 'czuje, ze...' - to nie wymysl. To Twoja inteligencja. Dorosli czasem zapominaja, ze tak tez sie mysli. Ty pamietaj.",
    "icon": "medrzec",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "empatia",
      "madrosc"
    ],
    "audience": "dziecko",
    "subcategory": "empatia_komus",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D16-S2",
    "profile": "EM",
    "day": 16,
    "slot": "poludnie",
    "category": "misje",
    "title": "Zauwaz rodzenstwo / kolege dzis inaczej",
    "body": "Jesli masz rodzenstwo albo dobrego kolege, powiedz dzis jedna rzecz, ktora w nim lubisz. Bez okazji. Niespodziewanie. To zostaje w pamieci.",
    "icon": "pulse",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "rodzenstwo",
      "zyczliwosc"
    ],
    "audience": "dziecko",
    "subcategory": "rodzenstwo_konflikt",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D16-S3",
    "profile": "EM",
    "day": 16,
    "slot": "wieczor",
    "category": "mentor",
    "title": "Gdy dziecko placze 'za innych'",
    "body": "Jesli Empata placze nad smutna bajka lub historia kolegi - nie mowcie 'to tylko film'. Powiedzcie: 'masz dobre, czujace serce'. Walidacja, nie odcinanie.",
    "icon": "rodzic",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "empatia",
      "walidacja"
    ],
    "audience": "rodzic",
    "subcategory": "empatia_komus",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D17-S1",
    "profile": "EM",
    "day": 17,
    "slot": "poranek",
    "category": "kraina",
    "title": "Trzy fale oddechu",
    "body": "Wyobraz sobie morze. Wciagnij powietrze - fala nadchodzi. Wypusc - fala odchodzi. Trzy razy. Juz jestes na brzegu dnia.",
    "icon": "pulse",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "oddech",
      "rytual_poranny"
    ],
    "audience": "dziecko",
    "subcategory": "rytual_kojacy",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D17-S2",
    "profile": "EM",
    "day": 17,
    "slot": "poludnie",
    "category": "misje",
    "title": "Badz obok kogos smutnego - bez slow",
    "body": "Jesli ktos w klasie albo w domu jest dzis smutny, nie musisz nic mowic. Po prostu usiadz obok. Czasem obecnosc leczy wiecej niz slowa.",
    "icon": "pulse",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "empatia",
      "obecnosc"
    ],
    "audience": "dziecko",
    "subcategory": "empatia_komus",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D17-S3",
    "profile": "EM",
    "day": 17,
    "slot": "wieczor",
    "category": "medrzec",
    "title": "Nie musisz wszystkich pocieszac",
    "body": "Czasem chcesz pomoc kazdemu - ale Ty tez jestes dzieckiem. To dorosli pocieszaja dzieci, a nie odwrotnie. Mozesz odpoczac.",
    "icon": "medrzec",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "granice",
      "wieczor"
    ],
    "audience": "rodzic",
    "subcategory": "granica_empaty",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D18-S1",
    "profile": "EM",
    "day": 18,
    "slot": "poranek",
    "category": "mentor",
    "title": "Dzis sprawdzcie zapas wody",
    "body": "Empata zapomina pic - bo jest w glowie u innych. Wreczcie rano butelke i krotko: 'twoja na dzis'. Drobny gest, duza zmiana w samopoczuciu.",
    "icon": "rodzic",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "woda",
      "cialo"
    ],
    "audience": "rodzic",
    "subcategory": "rytmy_dnia",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D18-S2",
    "profile": "EM",
    "day": 18,
    "slot": "poludnie",
    "category": "misje",
    "title": "Komplement od serca, nie z grzecznosci",
    "body": "Powiedz komus cos milego, co naprawde myslisz - nie z grzecznosci. Nie 'ladna bluzka', a 'lubie, jak sie smiejesz'. Prawda grzeje mocniej.",
    "icon": "pulse",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "rozmowa",
      "zyczliwosc"
    ],
    "audience": "dziecko",
    "subcategory": "rozmowa_sluchanie",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D18-S3",
    "profile": "EM",
    "day": 18,
    "slot": "wieczor",
    "category": "kraina",
    "title": "Swietlik zbiera dzis dobre chwile",
    "body": "Twoja cma swietlna lata przez caly dzien i zbiera cieple momenty. Wieczorem przynosi je do Ciebie. Wymien w glowie trzy. To wystarczy.",
    "icon": "artifact",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "wieczor",
      "wdziecznosc"
    ],
    "audience": "dziecko",
    "subcategory": "emocje_rozpoznanie",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D19-S1",
    "profile": "EM",
    "day": 19,
    "slot": "poranek",
    "category": "medrzec",
    "title": "Twoja wrazliwosc to nie slabosc",
    "body": "Czasem ktos powie 'jestes za czula, czuly'. To nieprawda. Twoja czulosc to sila. Bez ludzi takich jak Ty swiat bylby zimniejszy.",
    "icon": "medrzec",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "empatia",
      "sila"
    ],
    "audience": "dziecko",
    "subcategory": "empatia_komus",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D19-S2",
    "profile": "EM",
    "day": 19,
    "slot": "poludnie",
    "category": "misje",
    "title": "Zaproponuj wspolny spacer",
    "body": "Zaproponuj komus bliskiemu: 'pojdziemy razem na krotki spacer?'. Ruch + obecnosc + powietrze. Empata kwitnie w takich momentach.",
    "icon": "pulse",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "spacer",
      "ruch"
    ],
    "audience": "dziecko",
    "subcategory": "rytmy_dnia",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D19-S3",
    "profile": "EM",
    "day": 19,
    "slot": "wieczor",
    "category": "mentor",
    "title": "Wieczor ze swieca lub lampka nocna",
    "body": "Empata zasypia lepiej w cieplym, miekkim swietle niz w jasnym. Wieczorem zgascie gorne swiatlo. Zostawcie tylko jedna lampke. Swiat sam sie ucisza.",
    "icon": "rodzic",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "sen",
      "rytual_wieczorny"
    ],
    "audience": "rodzic",
    "subcategory": "rytmy_dnia",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D20-S1",
    "profile": "EM",
    "day": 20,
    "slot": "poranek",
    "category": "kraina",
    "title": "Morze Slow uczy slow dobrych",
    "body": "W Morzu Slow zyja tylko dobre slowa. Wybierz dzis jedno: 'spokoj', 'czulosc', 'odwaga'. Niech bedzie Twoim slowem dnia.",
    "icon": "map",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "slowo_dnia",
      "kraina"
    ],
    "audience": "dziecko",
    "subcategory": "emocje_rozpoznanie",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D20-S2",
    "profile": "EM",
    "day": 20,
    "slot": "poludnie",
    "category": "misje",
    "title": "Narysuj komus jego dobre serce",
    "body": "Narysuj na karteczce serce i daj komus z rodziny ze slowem 'twoje serce jest dobre'. Tyle. Prosty rysunek, ogromna fala.",
    "icon": "pulse",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "rysunek",
      "rodzina"
    ],
    "audience": "dziecko",
    "subcategory": "emocje_rozpoznanie",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D20-S3",
    "profile": "EM",
    "day": 20,
    "slot": "wieczor",
    "category": "medrzec",
    "title": "Lzy tez przychodza jak fala",
    "body": "Jesli dzis chcialo Ci sie plakac, pamietaj - lzy to fala. Przyszla, odeszla. To nie znaczy, ze jest zle. To znaczy, ze zyjesz gleboko.",
    "icon": "medrzec",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "emocje",
      "lzy"
    ],
    "audience": "rodzic",
    "subcategory": "emocje_rozpoznanie",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D21-S1",
    "profile": "EM",
    "day": 21,
    "slot": "poranek",
    "category": "mentor",
    "title": "Gdy zaczyna sie przeladowanie",
    "body": "Jesli Empata rano placze 'bez powodu' - czesto to za duzo bodzcow z poprzedniego dnia. Zwolnijcie tempo. Mniej rozmow. Wiecej ciszy. To nie kaprys.",
    "icon": "rodzic",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "przeciazenie",
      "wsparcie"
    ],
    "audience": "rodzic",
    "subcategory": "emocje_rozpoznanie",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D21-S2",
    "profile": "EM",
    "day": 21,
    "slot": "poludnie",
    "category": "misje",
    "title": "Twoje cicho miejsce na chwile",
    "body": "Znajdz dzis swoje cicho miejsce - pod kocem, na parapecie, w lazience. Posiedz tam 2 minuty. Twoje serce sie resetuje.",
    "icon": "pulse",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "cisza",
      "regeneracja"
    ],
    "audience": "dziecko",
    "subcategory": "emocje_rozpoznanie",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D21-S3",
    "profile": "EM",
    "day": 21,
    "slot": "wieczor",
    "category": "kraina",
    "title": "Muszla Echa przyjmuje sekret",
    "body": "Szepnij dzis do swojej dloni - jak do muszli - jeden sekret z dnia. Nikomu nie musisz mowic. Muszla Echa go pilnuje.",
    "icon": "artifact",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "wieczor",
      "kraina"
    ],
    "audience": "dziecko",
    "subcategory": "emocje_rozpoznanie",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D22-S1",
    "profile": "EM",
    "day": 22,
    "slot": "poranek",
    "category": "medrzec",
    "title": "Mozesz dzis byc w swoim tempie",
    "body": "Nie musisz dzis biec za innymi. Twoje tempo jest dobre. Powiedz sobie: 'ide swoim tempem, w swoim swietle'.",
    "icon": "medrzec",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "tempo",
      "uwaznosc"
    ],
    "audience": "dziecko",
    "subcategory": "emocje_rozpoznanie",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D22-S2",
    "profile": "EM",
    "day": 22,
    "slot": "poludnie",
    "category": "misje",
    "title": "Pomoz komus dzisiaj sie usmiechnac",
    "body": "Opowiedz dowcip, smieszna historie, zrob mine. Dzisiaj jestes osoba, ktora przynosi usmiech. Tylko jeden - to duzo.",
    "icon": "pulse",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "radosc",
      "zyczliwosc"
    ],
    "audience": "dziecko",
    "subcategory": "emocje_rozpoznanie",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D22-S3",
    "profile": "EM",
    "day": 22,
    "slot": "wieczor",
    "category": "mentor",
    "title": "Wspolne 'co dzis czulam, co dzis czules'",
    "body": "Zaproponujcie wieczorny rytual: jedno zdanie kazdy - 'dzis czulam/czulem...'. Mama, tata tez. Empata uczy sie, ze uczucia maja wszyscy. To uziemia.",
    "icon": "rodzic",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "rodzina",
      "rytual_wieczorny"
    ],
    "audience": "rodzic",
    "subcategory": "rytual_kojacy",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D23-S1",
    "profile": "EM",
    "day": 23,
    "slot": "poranek",
    "category": "kraina",
    "title": "Swietlik szepcze: 'jestes bezpieczna, bezpieczny'",
    "body": "Twoja swietlna cma siedzi na poduszce. Mruga: 'jestes bezpieczna. jestes bezpieczny.' Powtorz to w glowie trzy razy. Dzien sie otwiera.",
    "icon": "artifact",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "bezpieczenstwo",
      "rytual_poranny"
    ],
    "audience": "dziecko",
    "subcategory": "rytual_kojacy",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D23-S2",
    "profile": "EM",
    "day": 23,
    "slot": "poludnie",
    "category": "misje",
    "title": "List bez okazji",
    "body": "Napisz dzis krotki liscik do kogos z rodziny - tylko trzy zdania. Bez urodzin, bez swieta. Po prostu - bo myslisz o tej osobie.",
    "icon": "pulse",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "list",
      "rodzina"
    ],
    "audience": "dziecko",
    "subcategory": "list_komplement",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D23-S3",
    "profile": "EM",
    "day": 23,
    "slot": "wieczor",
    "category": "medrzec",
    "title": "Gdy ktos krzyknal - to nie o Tobie",
    "body": "Jesli ktos dzis krzyknal przy Tobie - to byla jego fala, nie Twoja. Nie zabieraj jej. Powiedz w sercu: 'to jego, nie moje'. Oddychaj.",
    "icon": "medrzec",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "granice",
      "emocje"
    ],
    "audience": "rodzic",
    "subcategory": "granica_empaty",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D24-S1",
    "profile": "EM",
    "day": 24,
    "slot": "poranek",
    "category": "mentor",
    "title": "Empata + ruch = rownowaga",
    "body": "Empata ma duzo emocji w ciele. 15 minut ruchu rano - taniec, skakanie, rower - to nie 'sport', to regulacja emocji. Sprobujcie dzis.",
    "icon": "rodzic",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "ruch",
      "cialo"
    ],
    "audience": "rodzic",
    "subcategory": "rytmy_dnia",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D24-S2",
    "profile": "EM",
    "day": 24,
    "slot": "poludnie",
    "category": "misje",
    "title": "Pochwala dla samej, samego siebie",
    "body": "Powiedz sobie dzis jedna pochwale za cos malego - 'fajnie, ze pomoglam', 'dobrze, ze sprobowalem'. Mowisz innym cieplo. Sobie tez wolno.",
    "icon": "pulse",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "samoakceptacja",
      "uwaznosc"
    ],
    "audience": "dziecko",
    "subcategory": "emocje_rozpoznanie",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D24-S3",
    "profile": "EM",
    "day": 24,
    "slot": "wieczor",
    "category": "kraina",
    "title": "Morze szumi do snu",
    "body": "Wyobraz sobie szum morza. Fala - wdech. Fala - wydech. Piec fal i juz jestes w glebokim, cieplym snie. Morze Cie trzyma.",
    "icon": "map",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "sen",
      "kraina"
    ],
    "audience": "dziecko",
    "subcategory": "rytmy_dnia",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D25-S1",
    "profile": "EM",
    "day": 25,
    "slot": "poranek",
    "category": "medrzec",
    "title": "Mozesz prosic o pomoc",
    "body": "Czesto pomagasz innym. Pamietaj - Ty tez mozesz prosic. Dorosly obok Ciebie chce pomoc. Powiedz dzis, gdy cos jest za trudne.",
    "icon": "medrzec",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "pomoc",
      "wsparcie"
    ],
    "audience": "dziecko",
    "subcategory": "empatia_komus",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D25-S2",
    "profile": "EM",
    "day": 25,
    "slot": "poludnie",
    "category": "misje",
    "title": "Zauwaz jeden szczegol w drugiej osobie",
    "body": "Popatrz dzis uwaznie na kogos bliskiego. Co w niej, w nim widzisz nowego? Moze zmarszczki smiechu, nowa bluzke, zmeczenie. Powiedz to.",
    "icon": "pulse",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "uwaznosc",
      "rozmowa"
    ],
    "audience": "dziecko",
    "subcategory": "rozmowa_sluchanie",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D25-S3",
    "profile": "EM",
    "day": 25,
    "slot": "wieczor",
    "category": "mentor",
    "title": "Gdy cos jest naprawde za trudne",
    "body": "Jesli Wasze dziecko mowi, ze jest mu bardzo smutno przez wiele dni, albo nie chce nic robic - porozmawiajcie z psychologiem dzieciecym. To nie porazka. To madrosc.",
    "icon": "rodzic",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "specjalista",
      "wsparcie"
    ],
    "audience": "rodzic",
    "subcategory": "emocje_rozpoznanie",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D26-S1",
    "profile": "EM",
    "day": 26,
    "slot": "poranek",
    "category": "kraina",
    "title": "Muszla Echa zna Twoje imie",
    "body": "Wymow rano swoje imie cicho do dloni - jak do muszli. Uslysz, jak ladnie brzmi. To Twoje imie. Nalezy do Ciebie i do nikogo innego.",
    "icon": "artifact",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "tozsamosc",
      "rytual_poranny"
    ],
    "audience": "dziecko",
    "subcategory": "rytual_kojacy",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D26-S2",
    "profile": "EM",
    "day": 26,
    "slot": "poludnie",
    "category": "misje",
    "title": "Zrob kogos czescia swojej zabawy",
    "body": "Jesli widzisz kogos, kto stoi sam - zapros go do swojej zabawy. Wystarczy: 'chcesz z nami?'. Trzy slowa, ktore zmieniaja dzien.",
    "icon": "pulse",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "rowiesnicy",
      "wlaczenie"
    ],
    "audience": "dziecko",
    "subcategory": "emocje_rozpoznanie",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D26-S3",
    "profile": "EM",
    "day": 26,
    "slot": "wieczor",
    "category": "medrzec",
    "title": "Twoje serce jest madrzejsze, gdy odpoczywa",
    "body": "Twoje serce dzis duzo czulo. Teraz potrzebuje cichej chwili. Wloz dlon pod policzek i powiedz: 'odpoczywam. dziekuje ci, serce.'",
    "icon": "medrzec",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "wieczor",
      "wdziecznosc"
    ],
    "audience": "rodzic",
    "subcategory": "emocje_rozpoznanie",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D27-S1",
    "profile": "EM",
    "day": 27,
    "slot": "poranek",
    "category": "mentor",
    "title": "Daj jej, daj mu wybor koloru ubrania",
    "body": "Empata czuje przez kolor. Daj dziecku rano wybor: dwie bluzki, w dwoch kolorach. Decyzja kolorem to maly trening 'co mi dzis sluzy'.",
    "icon": "rodzic",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "rytual_poranny",
      "autonomia"
    ],
    "audience": "rodzic",
    "subcategory": "rytual_kojacy",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D27-S2",
    "profile": "EM",
    "day": 27,
    "slot": "poludnie",
    "category": "misje",
    "title": "Tancz z radoscia przez minute",
    "body": "Wlacz ulubiona piosenke. Tancz bez patrzenia, czy ktos widzi. Radosc to tez emocja - Empata tez ma do niej prawo, mocno i glosno.",
    "icon": "pulse",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "radosc",
      "ruch"
    ],
    "audience": "dziecko",
    "subcategory": "rytmy_dnia",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D27-S3",
    "profile": "EM",
    "day": 27,
    "slot": "wieczor",
    "category": "kraina",
    "title": "Morze odplywa razem z dniem",
    "body": "Wyobraz sobie, ze stoisz na brzegu. Morze odplywa i zabiera dzien. Zostaje tylko gladki piasek. Jutro narysuje nowy slad.",
    "icon": "map",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "wieczor",
      "kraina"
    ],
    "audience": "dziecko",
    "subcategory": "emocje_rozpoznanie",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D28-S1",
    "profile": "EM",
    "day": 28,
    "slot": "poranek",
    "category": "medrzec",
    "title": "Trzy kolory dnia",
    "body": "Wybierz dzis trzy kolory, ktore Ci pasuja: jeden dla rana, jeden dla poludnia, jeden dla wieczora. Sprawdz wieczorem, czy sie zgadzaly.",
    "icon": "medrzec",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "emocje",
      "uwaznosc"
    ],
    "audience": "dziecko",
    "subcategory": "emocje_rozpoznanie",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D28-S2",
    "profile": "EM",
    "day": 28,
    "slot": "poludnie",
    "category": "misje",
    "title": "Powiedz doroslemu 'dziekuje'",
    "body": "Wybierz doroslego - mame, tate, pania, dziadka - i powiedz: 'dziekuje, ze jestes dla mnie'. Patrz w oczy. Krotko. Prawdziwie.",
    "icon": "pulse",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "wdziecznosc",
      "rodzina"
    ],
    "audience": "dziecko",
    "subcategory": "emocje_rozpoznanie",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D28-S3",
    "profile": "EM",
    "day": 28,
    "slot": "wieczor",
    "category": "mentor",
    "title": "Zauwazcie postep, nie idealnosc",
    "body": "Cztery tygodnie razem. Powiedzcie dziecku konkretnie: 'widze, ze ostatnio lepiej mowisz, co czujesz' albo 'widze, ze umiesz powiedziec nie'. Empata kwitnie od dostrzegania.",
    "icon": "rodzic",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "walidacja",
      "podsumowanie"
    ],
    "audience": "rodzic",
    "subcategory": "emocje_rozpoznanie",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D29-S1",
    "profile": "EM",
    "day": 29,
    "slot": "poranek",
    "category": "kraina",
    "title": "Swietlik leci przodem",
    "body": "Twoja swietlna cma juz wstala. Leci przed Toba i pokazuje, gdzie jest jasno. Idz za jej swiatlem. Dzien bedzie spokojny.",
    "icon": "artifact",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "rytual_poranny",
      "kraina"
    ],
    "audience": "dziecko",
    "subcategory": "rytual_kojacy",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D29-S2",
    "profile": "EM",
    "day": 29,
    "slot": "poludnie",
    "category": "misje",
    "title": "Mala rzecz, ktora Cie cieszy",
    "body": "Zauwaz dzis jedna drobna rzecz, ktora Cie ucieszyla - listek, zapach, czyjes slowo. Powiedz cicho 'dziekuje'. To cwiczenie szczescia.",
    "icon": "pulse",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "wdziecznosc",
      "radosc"
    ],
    "audience": "dziecko",
    "subcategory": "list_komplement",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D29-S3",
    "profile": "EM",
    "day": 29,
    "slot": "wieczor",
    "category": "medrzec",
    "title": "Twoje uczucia nie sa problemem",
    "body": "Nikt nigdy nie powinien Ci mowic: 'nie czuj tego'. Twoje uczucia sa czescia Ciebie. Medrczyni mowi: 'czuj. zauwazaj. oddychaj.'",
    "icon": "medrzec",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "emocje",
      "akceptacja"
    ],
    "audience": "rodzic",
    "subcategory": "emocje_rozpoznanie",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D30-S1",
    "profile": "EM",
    "day": 30,
    "slot": "poranek",
    "category": "mentor",
    "title": "Trzydziesty dzien - swieto serca",
    "body": "Zacznijcie dzis od wspolnego oddechu i jednego zdania: 'cieszem sie, ze jestes moim/moja dzieckiem'. To wystarczy. Empata to zapamieta na lata.",
    "icon": "rodzic",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "rodzina",
      "swieto"
    ],
    "audience": "rodzic",
    "subcategory": "rytual_kojacy",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D30-S2",
    "profile": "EM",
    "day": 30,
    "slot": "poludnie",
    "category": "misje",
    "title": "Zrob cos milego dla trzech osob",
    "body": "Wybierz dzis trzy osoby - z domu, z klasy, z podworka. Dla kazdej zrob cos malutkiego: usmiech, slowo, gest. Mala fala, trzy razy.",
    "icon": "pulse",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "zyczliwosc",
      "swieto"
    ],
    "audience": "dziecko",
    "subcategory": "emocje_rozpoznanie",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "EM-D30-S3",
    "profile": "EM",
    "day": 30,
    "slot": "wieczor",
    "category": "kraina",
    "title": "Muszla Echa pamieta wszystko",
    "body": "Trzydziesci dni razem. Twoja Muszla Echa zna teraz Twoj oddech, Twoj kolor, Twoje slowo. Przyloz dlon do ucha. Posluchaj - to Ty. Cala, piekna.",
    "icon": "artifact",
    "tone": "magic",
    "time": "2 min",
    "tags": [
      "wieczor",
      "podsumowanie"
    ],
    "audience": "dziecko",
    "subcategory": "rytual_kojacy",
    "profileName": "Zaklinacz Uczuć",
    "cecha": "Życzliwość",
    "kraina": "Morze Słów"
  },
  {
    "id": "ST-D01-S1",
    "profile": "ST",
    "day": 1,
    "slot": "poranek",
    "category": "misje",
    "title": "Mapa dnia na trzech krokach",
    "body": "Mędrczyni mówi: poproś dziecko, by narysowało dziś trzy małe kółka i wpisało w nie trzy rzeczy do zrobienia. Mapa dnia gotowa.",
    "icon": "map",
    "tone": "magic",
    "time": "2 min",
    "tags": [
      "planowanie",
      "poranek",
      "mapy"
    ],
    "audience": "dziecko",
    "subcategory": "planowanie",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D01-S2",
    "profile": "ST",
    "day": 1,
    "slot": "poludnie",
    "category": "misje",
    "title": "Krok pierwszy, krok drugi",
    "body": "Misja: weź jedną zwykłą czynność, na przykład mycie rąk, i powiedz na głos trzy kroki po kolei. Strateg wie, że nawet proste rzeczy mają mapę.",
    "icon": "zwoj",
    "tone": "paper",
    "time": "1 min",
    "tags": [
      "sekwencja",
      "krok_po_kroku"
    ],
    "audience": "dziecko",
    "subcategory": "liczby_logika",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D01-S3",
    "profile": "ST",
    "day": 1,
    "slot": "wieczor",
    "category": "medrzec",
    "title": "Co z mapy się udało",
    "body": "Wieczorem spójrzcie razem na rysunek z rana. Mędrczyni szepcze: zaznacz, co się udało, a co przesuwamy na jutro. Nic się nie psuje, tylko zmienia trasę.",
    "icon": "medrzec",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "przeglad",
      "wieczor",
      "plan_b"
    ],
    "audience": "rodzic",
    "subcategory": "elastycznosc_plan_b",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D02-S1",
    "profile": "ST",
    "day": 2,
    "slot": "poranek",
    "category": "mentor",
    "title": "Dziecko-Strateg lubi wiedzieć co dalej",
    "body": "Rodzicu, rano powiedz krótko, co będzie po kolei: śniadanie, ubieranie, droga. Twoje dziecko-Strateg wycisza się, gdy zna trasę.",
    "icon": "rodzic",
    "tone": "paper",
    "time": "1 min",
    "tags": [
      "rytm",
      "przewidywalnosc"
    ],
    "audience": "rodzic",
    "subcategory": "rytm_rytual_dnia",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D02-S2",
    "profile": "ST",
    "day": 2,
    "slot": "poludnie",
    "category": "misje",
    "title": "Policz drobne dobre rzeczy",
    "body": "Mędrczyni prosi: policz w głowie pięć drobnych dobrych rzeczy z dzisiaj. Mały promień słońca też się liczy.",
    "icon": "krysztal",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "uwaznosc",
      "liczby"
    ],
    "audience": "dziecko",
    "subcategory": "liczby_logika",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D02-S3",
    "profile": "ST",
    "day": 2,
    "slot": "wieczor",
    "category": "misje",
    "title": "Trzy zdania o dzisiaj",
    "body": "Powiedz wieczorem trzy zdania: co było łatwe, co trudne, co ciekawe. Strateg porządkuje dzień jak zwoje w kieszeniach płaszcza.",
    "icon": "zwoj",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "przeglad",
      "wieczor"
    ],
    "audience": "dziecko",
    "subcategory": "planowanie",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D03-S1",
    "profile": "ST",
    "day": 3,
    "slot": "poranek",
    "category": "kraina",
    "title": "Góry Liczb budzą się powoli",
    "body": "Mędrczyni opowiada: w Górach Liczb poranki zaczynają się od cichego liczenia oddechów. Spróbuj policzyć pięć wdechów, zanim wstaniesz z łóżka.",
    "icon": "krysztal",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "oddech",
      "poranek",
      "liczby"
    ],
    "audience": "dziecko",
    "subcategory": "liczby_logika",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D03-S2",
    "profile": "ST",
    "day": 3,
    "slot": "poludnie",
    "category": "misje",
    "title": "Cukierek teraz czy potem",
    "body": "Misja odraczania: jeśli masz dziś coś smacznego, spróbuj zjeść połowę teraz, a połowę po obiedzie. Sprawdź, czy druga połowa smakuje inaczej.",
    "icon": "pulse",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "odraczanie",
      "cierpliwosc"
    ],
    "audience": "dziecko",
    "subcategory": "odraczanie_gratyfikacji",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D03-S3",
    "profile": "ST",
    "day": 3,
    "slot": "wieczor",
    "category": "mentor",
    "title": "Gdy plan się sypie, nie znika",
    "body": "Rodzicu, jeśli dzisiejszy plan dziecka się rozsypał, powiedz: plan B to też plan. Strateg uczy się, że mapy się rysuje wielokrotnie.",
    "icon": "rodzic",
    "tone": "paper",
    "time": "1 min",
    "tags": [
      "plan_b",
      "elastycznosc"
    ],
    "audience": "rodzic",
    "subcategory": "elastycznosc_plan_b",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D04-S1",
    "profile": "ST",
    "day": 4,
    "slot": "poranek",
    "category": "misje",
    "title": "Mapa pokoju z czterech stref",
    "body": "Narysuj swój pokój i podziel go na cztery strefy: spanie, nauka, zabawa, spokój. Mędrczyni mówi: każda kraina ma swoje miejsce.",
    "icon": "map",
    "tone": "magic",
    "time": "2 min",
    "tags": [
      "mapy",
      "porzadek"
    ],
    "audience": "dziecko",
    "subcategory": "mapy_kolejnosc",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D04-S2",
    "profile": "ST",
    "day": 4,
    "slot": "poludnie",
    "category": "misje",
    "title": "Sekwencja trzech małych zadań",
    "body": "Wybierz trzy małe zadania i zrób je dokładnie w takiej kolejności, w jakiej je wymyślisz. Strateg czuje, jak kroki układają się w drogę.",
    "icon": "zwoj",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "sekwencja",
      "planowanie"
    ],
    "audience": "dziecko",
    "subcategory": "liczby_logika",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D04-S3",
    "profile": "ST",
    "day": 4,
    "slot": "wieczor",
    "category": "misje",
    "title": "Jutro zaczynamy od jednego",
    "body": "Wieczorem wybierz jedną rzecz, od której chcesz zacząć jutro. Tylko jedną. Mędrczyni mówi: pierwszy kamień milowy waży najmniej.",
    "icon": "krysztal",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "jutro",
      "wieczor"
    ],
    "audience": "dziecko",
    "subcategory": "planowanie",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D05-S1",
    "profile": "ST",
    "day": 5,
    "slot": "poranek",
    "category": "mentor",
    "title": "Dawaj wybór z dwóch ścieżek",
    "body": "Rodzicu, rano zaproponuj wybór: czerwona koszulka czy zielona, owsianka czy kanapka. Strateg potrzebuje czuć, że decyduje o własnej mapie.",
    "icon": "rodzic",
    "tone": "paper",
    "time": "1 min",
    "tags": [
      "wybor",
      "autonomia"
    ],
    "audience": "rodzic",
    "subcategory": "planowanie",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D05-S2",
    "profile": "ST",
    "day": 5,
    "slot": "poludnie",
    "category": "misje",
    "title": "Zegnij papier trzy razy",
    "body": "Misja: weź kartkę i zegnij ją na pół trzy razy z rzędu. Policz, ile masz teraz warstw. Strateg lubi, gdy liczby się układają.",
    "icon": "pulse",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "liczby",
      "eksperyment"
    ],
    "audience": "dziecko",
    "subcategory": "liczby_logika",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D05-S3",
    "profile": "ST",
    "day": 5,
    "slot": "wieczor",
    "category": "medrzec",
    "title": "Mosiężne okulary patrzą wstecz",
    "body": "Mędrczyni zakłada okulary do patrzenia w dzień. Spójrz wstecz na dziś i powiedz jednym słowem, jaki był ten dzień.",
    "icon": "medrzec",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "refleksja",
      "wieczor"
    ],
    "audience": "rodzic",
    "subcategory": "planowanie",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D06-S1",
    "profile": "ST",
    "day": 6,
    "slot": "poranek",
    "category": "misje",
    "title": "Plan poranka na trzy kółka",
    "body": "Narysuj trzy kółka i wpisz: umyć się, ubrać, zjeść. Odhacz, gdy zrobione. Mędrczyni mówi: zwykłe rzeczy też zasługują na mapę.",
    "icon": "map",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "planowanie",
      "poranek"
    ],
    "audience": "dziecko",
    "subcategory": "planowanie",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D06-S2",
    "profile": "ST",
    "day": 6,
    "slot": "poludnie",
    "category": "misje",
    "title": "Wytrzymaj minutę ciszy",
    "body": "Misja cierpliwości: spróbuj wytrzymać jedną minutę zupełnej ciszy. Strateg wie, że cisza to też strategia.",
    "icon": "krysztal",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "cierpliwosc",
      "cisza"
    ],
    "audience": "dziecko",
    "subcategory": "odraczanie_gratyfikacji",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D06-S3",
    "profile": "ST",
    "day": 6,
    "slot": "wieczor",
    "category": "misje",
    "title": "Co zrobiłbym inaczej",
    "body": "Powiedz wieczorem jedną rzecz, którą jutro zrobisz inaczej. Bez wyrzutów. Mędrczyni nazywa to przepisaniem mapy.",
    "icon": "zwoj",
    "tone": "paper",
    "time": "1 min",
    "tags": [
      "refleksja",
      "plan_b"
    ],
    "audience": "dziecko",
    "subcategory": "elastycznosc_plan_b",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D07-S1",
    "profile": "ST",
    "day": 7,
    "slot": "poranek",
    "category": "mentor",
    "title": "Pochwal proces, nie tylko wynik",
    "body": "Rodzicu, gdy dziecko coś planuje, powiedz: widzę, że rozkładasz to na kroki. Strateg rośnie, gdy widzi, że droga się liczy bardziej niż meta.",
    "icon": "rodzic",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "proces",
      "pochwala"
    ],
    "audience": "rodzic",
    "subcategory": "nagroda_proces",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D07-S2",
    "profile": "ST",
    "day": 7,
    "slot": "poludnie",
    "category": "misje",
    "title": "Odrocz przyjemność na pół godziny",
    "body": "Misja: jeśli chcesz coś zrobić teraz dla zabawy, spróbuj odłożyć to o trzydzieści minut. Sprawdź, czy nadal masz na to ochotę.",
    "icon": "pulse",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "odraczanie",
      "cierpliwosc"
    ],
    "audience": "dziecko",
    "subcategory": "odraczanie_gratyfikacji",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D07-S3",
    "profile": "ST",
    "day": 7,
    "slot": "wieczor",
    "category": "kraina",
    "title": "Wieczór w Górach Liczb",
    "body": "W Górach Liczb wieczorem światła zapalają się po kolei, jedno po drugim. Wymień trzy światełka z twojego dnia, które warto zapamiętać.",
    "icon": "krysztal",
    "tone": "magic",
    "time": "2 min",
    "tags": [
      "wieczor",
      "wdziecznosc"
    ],
    "audience": "dziecko",
    "subcategory": "planowanie",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D08-S1",
    "profile": "ST",
    "day": 8,
    "slot": "poranek",
    "category": "misje",
    "title": "Plan B na wszelki wypadek",
    "body": "Zaplanuj dziś coś prostego, a obok napisz plan B. Mędrczyni mówi: dobry strateg ma zawsze drugą ścieżkę w kieszeni.",
    "icon": "map",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "plan_b",
      "elastycznosc"
    ],
    "audience": "dziecko",
    "subcategory": "elastycznosc_plan_b",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D08-S2",
    "profile": "ST",
    "day": 8,
    "slot": "poludnie",
    "category": "misje",
    "title": "Ułóż klocki w trzech rzędach",
    "body": "Misja: weź dowolne klocki lub patyczki i ułóż je w trzech równych rzędach. Sprawdź, czy zostanie ci coś z boku.",
    "icon": "zwoj",
    "tone": "paper",
    "time": "1 min",
    "tags": [
      "liczby",
      "sekwencja"
    ],
    "audience": "dziecko",
    "subcategory": "liczby_logika",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D08-S3",
    "profile": "ST",
    "day": 8,
    "slot": "wieczor",
    "category": "medrzec",
    "title": "Dobry-wystarczający, nie idealny",
    "body": "Mędrczyni szepcze: dziś coś było dobre-wystarczające? To też skarb. Idealny to nie jest cel każdej mapy.",
    "icon": "medrzec",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "perfekcjonizm",
      "wieczor"
    ],
    "audience": "rodzic",
    "subcategory": "perfekcjonizm_antidote",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D09-S1",
    "profile": "ST",
    "day": 9,
    "slot": "poranek",
    "category": "mentor",
    "title": "Pozwól dziecku planować weekend",
    "body": "Rodzicu, daj dziecku-Strategowi szansę zaplanować jedną rzecz na weekend od początku do końca. Nawet drobiazg. Buduje to poczucie kompetencji.",
    "icon": "rodzic",
    "tone": "paper",
    "time": "1 min",
    "tags": [
      "autonomia",
      "planowanie"
    ],
    "audience": "rodzic",
    "subcategory": "planowanie",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D09-S2",
    "profile": "ST",
    "day": 9,
    "slot": "poludnie",
    "category": "misje",
    "title": "Sekwencja pięciu kroków do wody",
    "body": "Idź po szklankę wody i policz w głowie kroki: jeden, dwa, trzy. Wypij powoli. Mędrczyni mówi: woda też podróżuje w sekwencji.",
    "icon": "pulse",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "woda",
      "sekwencja"
    ],
    "audience": "dziecko",
    "subcategory": "zdrowie_strateg",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D09-S3",
    "profile": "ST",
    "day": 9,
    "slot": "wieczor",
    "category": "misje",
    "title": "Jedna rzecz, z której jestem dumny",
    "body": "Wieczorem powiedz jedną rzecz, z której jesteś dziś dumny. Może być malutka. Strateg liczy każdy krok, nie tylko ostatni.",
    "icon": "krysztal",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "duma",
      "wieczor"
    ],
    "audience": "dziecko",
    "subcategory": "planowanie",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D10-S1",
    "profile": "ST",
    "day": 10,
    "slot": "poranek",
    "category": "kraina",
    "title": "Mosty nad doliną Liczb",
    "body": "Mędrczyni opowiada: między szczytami Gór Liczb wiszą mosty z mocnych sznurów. Wyobraź sobie jeden taki most i przejdź po nim w myślach.",
    "icon": "map",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "wyobraznia",
      "poranek"
    ],
    "audience": "dziecko",
    "subcategory": "planowanie",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D10-S2",
    "profile": "ST",
    "day": 10,
    "slot": "poludnie",
    "category": "misje",
    "title": "Trzy etapy obiadu",
    "body": "Misja: jedząc obiad, podziel go na trzy etapy. Najpierw zupa lub pierwsze, potem główne, na końcu coś słodkiego lub owoc. Czuj sekwencję.",
    "icon": "zwoj",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "sekwencja",
      "jedzenie"
    ],
    "audience": "dziecko",
    "subcategory": "liczby_logika",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D10-S3",
    "profile": "ST",
    "day": 10,
    "slot": "wieczor",
    "category": "mentor",
    "title": "Sen jest drogą do jutra",
    "body": "Rodzicu, przypomnij dziecku, że sen 9–13 godzin to nie strata czasu, tylko cicha podróż mózgu. Strateg odpoczywa, by jutro lepiej planować.",
    "icon": "rodzic",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "sen",
      "wieczor"
    ],
    "audience": "rodzic",
    "subcategory": "zdrowie_strateg",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D11-S1",
    "profile": "ST",
    "day": 11,
    "slot": "poranek",
    "category": "misje",
    "title": "Mapa drogi do szkoły",
    "body": "Narysuj prostą mapę drogi z domu do szkoły lub przedszkola. Zaznacz trzy ciekawe punkty po drodze. Mędrczyni mówi: każda droga jest opowieścią.",
    "icon": "map",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "mapy",
      "obserwacja"
    ],
    "audience": "dziecko",
    "subcategory": "mapy_kolejnosc",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D11-S2",
    "profile": "ST",
    "day": 11,
    "slot": "poludnie",
    "category": "misje",
    "title": "Czekaj dziesięć oddechów",
    "body": "Misja cierpliwości: gdy chcesz coś natychmiast powiedzieć lub zrobić, policz dziesięć powolnych oddechów. Sprawdź, jak zmienia się chęć.",
    "icon": "krysztal",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "cierpliwosc",
      "oddech"
    ],
    "audience": "dziecko",
    "subcategory": "odraczanie_gratyfikacji",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D11-S3",
    "profile": "ST",
    "day": 11,
    "slot": "wieczor",
    "category": "misje",
    "title": "Trzy kamienie milowe dnia",
    "body": "Wieczorem nazwij trzy kamienie milowe z dziś. Cokolwiek, co czuło się jak krok do przodu. Strateg buduje drogę z takich kamieni.",
    "icon": "zwoj",
    "tone": "paper",
    "time": "1 min",
    "tags": [
      "refleksja",
      "wieczor"
    ],
    "audience": "dziecko",
    "subcategory": "planowanie",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D12-S1",
    "profile": "ST",
    "day": 12,
    "slot": "poranek",
    "category": "mentor",
    "title": "Gdy dziecko utknie na planie",
    "body": "Rodzicu, gdy widzisz, że dziecko utyka w nieskończonym planowaniu, powiedz łagodnie: zacznij od pierwszego kroku, reszta się ułoży.",
    "icon": "rodzic",
    "tone": "paper",
    "time": "1 min",
    "tags": [
      "perfekcjonizm",
      "wsparcie"
    ],
    "audience": "rodzic",
    "subcategory": "perfekcjonizm_antidote",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D12-S2",
    "profile": "ST",
    "day": 12,
    "slot": "poludnie",
    "category": "misje",
    "title": "Trzy razy podskocz, raz przykucnij",
    "body": "Misja sekwencji ruchu: trzy podskoki, jeden przykuck, powtórz cztery razy. Strateg porządkuje też ciało, nie tylko myśli.",
    "icon": "pulse",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "ruch",
      "sekwencja"
    ],
    "audience": "dziecko",
    "subcategory": "zdrowie_strateg",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D12-S3",
    "profile": "ST",
    "day": 12,
    "slot": "wieczor",
    "category": "medrzec",
    "title": "Co się nie udało, czego nauczyło",
    "body": "Mędrczyni mówi: weź jedną rzecz, która się nie udała, i pomyśl, czego cię nauczyła. Błąd to też informacja na mapie.",
    "icon": "medrzec",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "blad",
      "nauka"
    ],
    "audience": "rodzic",
    "subcategory": "elastycznosc_plan_b",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D13-S1",
    "profile": "ST",
    "day": 13,
    "slot": "poranek",
    "category": "misje",
    "title": "Zaplanuj poranek w trzech zdaniach",
    "body": "Wymyśl trzy zdania o tym, co dziś rano się wydarzy. Powiedz je na głos. Mędrczyni mówi: słowo wypowiedziane jest pierwszym krokiem mapy.",
    "icon": "zwoj",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "poranek",
      "planowanie"
    ],
    "audience": "dziecko",
    "subcategory": "planowanie",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D13-S2",
    "profile": "ST",
    "day": 13,
    "slot": "poludnie",
    "category": "misje",
    "title": "Znajdź wzór wokół siebie",
    "body": "Rozejrzyj się i znajdź jeden powtarzający się wzór: w płytkach, na firanie, na talerzu. Strateg widzi powtórzenia tam, gdzie inni widzą chaos.",
    "icon": "krysztal",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "wzory",
      "obserwacja"
    ],
    "audience": "dziecko",
    "subcategory": "planowanie",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D13-S3",
    "profile": "ST",
    "day": 13,
    "slot": "wieczor",
    "category": "misje",
    "title": "Schody dnia",
    "body": "Wyobraź sobie dzisiejszy dzień jako schody. Ile masz stopni? Który był najwyższy? Mędrczyni mówi: schody widać dopiero z dołu.",
    "icon": "zwoj",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "refleksja",
      "metafora"
    ],
    "audience": "dziecko",
    "subcategory": "planowanie",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D14-S1",
    "profile": "ST",
    "day": 14,
    "slot": "poranek",
    "category": "mentor",
    "title": "Nie poprawiaj każdego planu",
    "body": "Rodzicu, gdy dziecko pokaże swój plan, nie poprawiaj od razu. Zapytaj: a co potem? Strateg uczy się myśleć dalej, gdy ma przestrzeń.",
    "icon": "rodzic",
    "tone": "paper",
    "time": "1 min",
    "tags": [
      "wsparcie",
      "autonomia"
    ],
    "audience": "rodzic",
    "subcategory": "planowanie",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D14-S2",
    "profile": "ST",
    "day": 14,
    "slot": "poludnie",
    "category": "misje",
    "title": "Posegreguj coś na trzy kupki",
    "body": "Misja porządku: weź kredki, klocki albo skarpetki i posegreguj na trzy kupki według własnego pomysłu. Strateg lubi swoje systemy.",
    "icon": "zwoj",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "porzadek",
      "klasyfikacja"
    ],
    "audience": "dziecko",
    "subcategory": "planowanie",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D14-S3",
    "profile": "ST",
    "day": 14,
    "slot": "wieczor",
    "category": "kraina",
    "title": "Skarb na końcu mapy",
    "body": "W Górach Liczb na końcu każdej mapy jest mały skarb. Twoim dzisiejszym skarbem może być spokój, śmiech albo nowa myśl. Wybierz jeden.",
    "icon": "krysztal",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "wieczor",
      "wdziecznosc"
    ],
    "audience": "dziecko",
    "subcategory": "mapy_kolejnosc",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D15-S1",
    "profile": "ST",
    "day": 15,
    "slot": "poranek",
    "category": "misje",
    "title": "Połowa miesiąca w drodze",
    "body": "Mędrczyni mówi: jesteś w połowie miesiącznej wędrówki. Narysuj kreskę pośrodku kartki i napisz po lewej, co już umiesz, po prawej, czego chcesz spróbować.",
    "icon": "map",
    "tone": "magic",
    "time": "2 min",
    "tags": [
      "polmetek",
      "refleksja"
    ],
    "audience": "dziecko",
    "subcategory": "planowanie",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D15-S2",
    "profile": "ST",
    "day": 15,
    "slot": "poludnie",
    "category": "misje",
    "title": "Wypij wodę trzema łykami",
    "body": "Wypij szklankę wody dokładnie trzema łykami. Policz na głos. Strateg lubi, gdy zwykłe czynności mają strukturę.",
    "icon": "pulse",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "woda",
      "sekwencja"
    ],
    "audience": "dziecko",
    "subcategory": "zdrowie_strateg",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D15-S3",
    "profile": "ST",
    "day": 15,
    "slot": "wieczor",
    "category": "misje",
    "title": "Lista TODO i trzy odhaczenia",
    "body": "Spójrz, co dziś zrobiłeś. Napisz krótką listę i odhacz trzy rzeczy. Mędrczyni mówi: odhaczanie to dźwięk kończonego kroku.",
    "icon": "zwoj",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "lista",
      "wieczor"
    ],
    "audience": "dziecko",
    "subcategory": "planowanie",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D16-S1",
    "profile": "ST",
    "day": 16,
    "slot": "poranek",
    "category": "mentor",
    "title": "Frustracja, gdy plan się sypie",
    "body": "Rodzicu, gdy dziecko-Strateg wpada w złość, że coś poszło nie po jego planie, usiądź obok i powiedz: rozumiem, mapa się zmieniła. Pomóż przerysować.",
    "icon": "rodzic",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "emocje",
      "plan_b"
    ],
    "audience": "rodzic",
    "subcategory": "elastycznosc_plan_b",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D16-S2",
    "profile": "ST",
    "day": 16,
    "slot": "poludnie",
    "category": "misje",
    "title": "Sekwencja zwierząt",
    "body": "Misja: wymień pięć zwierząt od najmniejszego do największego. Mędrczyni mówi: porządkowanie też jest formą myślenia.",
    "icon": "krysztal",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "sekwencja",
      "klasyfikacja"
    ],
    "audience": "dziecko",
    "subcategory": "liczby_logika",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D16-S3",
    "profile": "ST",
    "day": 16,
    "slot": "wieczor",
    "category": "medrzec",
    "title": "Dziś byłem cierpliwy gdy",
    "body": "Mędrczyni pyta: dziś byłem cierpliwy, gdy... Dokończ zdanie. Nawet drobna cierpliwość zostawia ślad na mapie.",
    "icon": "medrzec",
    "tone": "paper",
    "time": "1 min",
    "tags": [
      "cierpliwosc",
      "wieczor"
    ],
    "audience": "rodzic",
    "subcategory": "odraczanie_gratyfikacji",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D17-S1",
    "profile": "ST",
    "day": 17,
    "slot": "poranek",
    "category": "misje",
    "title": "Trzy najważniejsze dziś",
    "body": "Wybierz trzy rzeczy najważniejsze na dziś. Tylko trzy. Mędrczyni mówi: kto wybiera mniej, ten dochodzi dalej.",
    "icon": "map",
    "tone": "paper",
    "time": "1 min",
    "tags": [
      "priorytety",
      "poranek"
    ],
    "audience": "dziecko",
    "subcategory": "planowanie",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D17-S2",
    "profile": "ST",
    "day": 17,
    "slot": "poludnie",
    "category": "misje",
    "title": "Eksperyment z czasem",
    "body": "Misja: zgadnij, ile to jest minuta. Zamknij oczy i otwórz, gdy myślisz, że minęła. Sprawdź na zegarze. Strateg ćwiczy poczucie czasu.",
    "icon": "pulse",
    "tone": "magic",
    "time": "2 min",
    "tags": [
      "czas",
      "eksperyment"
    ],
    "audience": "dziecko",
    "subcategory": "planowanie",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D17-S3",
    "profile": "ST",
    "day": 17,
    "slot": "wieczor",
    "category": "kraina",
    "title": "Lampka w domku w Górach",
    "body": "W Górach Liczb wieczorem w każdym oknie zapala się jedna lampka. Twoja dzisiejsza lampka to jedno dobre słowo o sobie. Powiedz je.",
    "icon": "krysztal",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "wieczor",
      "samoocena"
    ],
    "audience": "dziecko",
    "subcategory": "planowanie",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D18-S1",
    "profile": "ST",
    "day": 18,
    "slot": "poranek",
    "category": "mentor",
    "title": "Pytaj o jak, nie o czemu",
    "body": "Rodzicu, gdy dziecko coś planuje, pytaj raczej jak chcesz to zrobić niż czemu tak. Strateg rozkwita w pytaniach o sposób.",
    "icon": "rodzic",
    "tone": "paper",
    "time": "1 min",
    "tags": [
      "rozmowa",
      "wsparcie"
    ],
    "audience": "rodzic",
    "subcategory": "planowanie",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D18-S2",
    "profile": "ST",
    "day": 18,
    "slot": "poludnie",
    "category": "misje",
    "title": "Zbuduj wieżę z pięciu rzeczy",
    "body": "Misja: zbuduj wieżę z pięciu różnych przedmiotów. Najpierw zaplanuj kolejność. Mędrczyni mówi: dobra wieża zaczyna się od dobrego dołu.",
    "icon": "zwoj",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "planowanie",
      "konstrukcja"
    ],
    "audience": "dziecko",
    "subcategory": "planowanie",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D18-S3",
    "profile": "ST",
    "day": 18,
    "slot": "wieczor",
    "category": "misje",
    "title": "Dwie rzeczy łatwe, jedna trudna",
    "body": "Wieczorem nazwij dwie rzeczy, które dziś były łatwe, i jedną, która była trudna. Mędrczyni mówi: trudne też ma swoje miejsce na mapie.",
    "icon": "zwoj",
    "tone": "paper",
    "time": "1 min",
    "tags": [
      "refleksja",
      "wieczor"
    ],
    "audience": "dziecko",
    "subcategory": "planowanie",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D19-S1",
    "profile": "ST",
    "day": 19,
    "slot": "poranek",
    "category": "misje",
    "title": "Plan na sześćdziesiąt minut ruchu",
    "body": "Mędrczyni mówi: zaplanuj, jak dziś uzbierasz godzinę ruchu. Może spacer, podskoki, taniec. Mapa ciała też istnieje.",
    "icon": "map",
    "tone": "magic",
    "time": "2 min",
    "tags": [
      "ruch",
      "planowanie"
    ],
    "audience": "dziecko",
    "subcategory": "zdrowie_strateg",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D19-S2",
    "profile": "ST",
    "day": 19,
    "slot": "poludnie",
    "category": "misje",
    "title": "Liczby wokół ciebie",
    "body": "Misja obserwacji: rozejrzyj się i znajdź pięć różnych liczb wokół siebie. Na zegarze, książce, koszulce. Strateg czyta świat przez liczby.",
    "icon": "krysztal",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "liczby",
      "obserwacja"
    ],
    "audience": "dziecko",
    "subcategory": "liczby_logika",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D19-S3",
    "profile": "ST",
    "day": 19,
    "slot": "wieczor",
    "category": "medrzec",
    "title": "Mędrczyni o cichym sukcesie",
    "body": "Mędrczyni szepcze: są sukcesy, których nikt nie widzi, oprócz ciebie. Pomyśl o jednym takim z dziś.",
    "icon": "medrzec",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "sukces",
      "wieczor"
    ],
    "audience": "rodzic",
    "subcategory": "planowanie",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D20-S1",
    "profile": "ST",
    "day": 20,
    "slot": "poranek",
    "category": "mentor",
    "title": "Krótkie ostrzeżenia o zmianie",
    "body": "Rodzicu, gdy plan dnia się zmieni, uprzedź dziecko z wyprzedzeniem. Strateg trzyma się map, więc nagła zmiana boli mocniej.",
    "icon": "rodzic",
    "tone": "paper",
    "time": "1 min",
    "tags": [
      "zmiana",
      "przewidywalnosc"
    ],
    "audience": "rodzic",
    "subcategory": "planowanie",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D20-S2",
    "profile": "ST",
    "day": 20,
    "slot": "poludnie",
    "category": "misje",
    "title": "Trzy razy głęboki oddech",
    "body": "Misja: zrób trzy razy z rzędu wdech na cztery, wydech na sześć. Strateg uczy się, że oddech to też sekwencja.",
    "icon": "pulse",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "oddech",
      "spokoj"
    ],
    "audience": "dziecko",
    "subcategory": "liczby_logika",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D20-S3",
    "profile": "ST",
    "day": 20,
    "slot": "wieczor",
    "category": "misje",
    "title": "Mapa jutra w trzech słowach",
    "body": "Zanim zaśniesz, wymyśl trzy słowa opisujące jutro: na przykład spokojnie, ciekawie, wspólnie. Mędrczyni mówi: słowa wyciągają mapę z ciemności.",
    "icon": "zwoj",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "jutro",
      "wieczor"
    ],
    "audience": "dziecko",
    "subcategory": "planowanie",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D21-S1",
    "profile": "ST",
    "day": 21,
    "slot": "poranek",
    "category": "kraina",
    "title": "Trzy szczyty Gór Liczb",
    "body": "W Górach Liczb są trzy szczyty: Wiem, Próbuję, Pytam. Wybierz dziś rano, na którym chcesz być najwięcej.",
    "icon": "krysztal",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "intencja",
      "poranek"
    ],
    "audience": "dziecko",
    "subcategory": "planowanie",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D21-S2",
    "profile": "ST",
    "day": 21,
    "slot": "poludnie",
    "category": "misje",
    "title": "Pokrój owoc w równe kawałki",
    "body": "Misja: poproś dorosłego o owoc i pokrój go z pomocą na cztery równe kawałki. Mędrczyni mówi: równy podział to mała sztuka.",
    "icon": "zwoj",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "liczby",
      "praktyka"
    ],
    "audience": "dziecko",
    "subcategory": "liczby_logika",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D21-S3",
    "profile": "ST",
    "day": 21,
    "slot": "wieczor",
    "category": "mentor",
    "title": "Nie ratuj zbyt szybko",
    "body": "Rodzicu, gdy dziecko utyka na zadaniu, daj chwilę więcej niż zwykle. Strateg uczy się przez próbowanie kolejnych kroków.",
    "icon": "rodzic",
    "tone": "paper",
    "time": "1 min",
    "tags": [
      "cierpliwosc",
      "wsparcie"
    ],
    "audience": "rodzic",
    "subcategory": "odraczanie_gratyfikacji",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D22-S1",
    "profile": "ST",
    "day": 22,
    "slot": "poranek",
    "category": "misje",
    "title": "Sześć minut ciszy o poranku",
    "body": "Mędrczyni mówi: zanim zaczniesz dzień, posiedź sześć minut bez rozmowy i ekranu. Może być z herbatą, może z oknem. Sprawdź, co usłyszysz.",
    "icon": "krysztal",
    "tone": "magic",
    "time": "2 min",
    "tags": [
      "cisza",
      "poranek"
    ],
    "audience": "dziecko",
    "subcategory": "planowanie",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D22-S2",
    "profile": "ST",
    "day": 22,
    "slot": "poludnie",
    "category": "misje",
    "title": "Odrocz na 24 godziny",
    "body": "Misja odraczania: jeśli czegoś bardzo chcesz dziś, zapisz to i wróć do tego jutro o tej samej porze. Sprawdź, czy nadal chcesz tak samo.",
    "icon": "pulse",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "odraczanie",
      "decyzje"
    ],
    "audience": "dziecko",
    "subcategory": "odraczanie_gratyfikacji",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D22-S3",
    "profile": "ST",
    "day": 22,
    "slot": "wieczor",
    "category": "medrzec",
    "title": "Trzy rzeczy, które wiem o sobie",
    "body": "Mędrczyni pyta: wymień trzy rzeczy, które wiesz o sobie dzisiaj. Nawet drobne. Strateg zbiera wiedzę o sobie jak zwoje.",
    "icon": "medrzec",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "samoocena",
      "wieczor"
    ],
    "audience": "rodzic",
    "subcategory": "planowanie",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D23-S1",
    "profile": "ST",
    "day": 23,
    "slot": "poranek",
    "category": "mentor",
    "title": "Pochwal próbę, nie tylko sukces",
    "body": "Rodzicu, dziś pochwal próbę: spróbowałeś, mimo że nie wyszło. Strateg uczy się wtedy, że próbowanie ma wartość samo w sobie.",
    "icon": "rodzic",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "proces",
      "pochwala"
    ],
    "audience": "rodzic",
    "subcategory": "nagroda_proces",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D23-S2",
    "profile": "ST",
    "day": 23,
    "slot": "poludnie",
    "category": "misje",
    "title": "Kompas Strategiczny w głowie",
    "body": "Misja: wyobraź sobie mały kompas w klatce piersiowej. Wskazuje cztery kierunki: spokój, ruch, śmiech, sen. Który kierunek dziś najpotrzebniejszy?",
    "icon": "krysztal",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "artifact",
      "samoswiadomosc"
    ],
    "audience": "dziecko",
    "subcategory": "zdrowie_strateg",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D23-S3",
    "profile": "ST",
    "day": 23,
    "slot": "wieczor",
    "category": "misje",
    "title": "Jutrzejszy poranek na kartce",
    "body": "Narysuj jutrzejszy poranek w trzech obrazkach: obudzenie, śniadanie, wyjście. Mędrczyni mówi: poranek narysowany dziś łatwiej idzie jutro.",
    "icon": "map",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "planowanie",
      "wieczor"
    ],
    "audience": "dziecko",
    "subcategory": "planowanie",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D24-S1",
    "profile": "ST",
    "day": 24,
    "slot": "poranek",
    "category": "misje",
    "title": "Wybierz tempo poranka",
    "body": "Wybierz dziś tempo poranka: żółw, koń czy ptak. Spróbuj się tego trzymać. Strateg lubi wiedzieć, w jakim rytmie idzie.",
    "icon": "zwoj",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "rytm",
      "poranek"
    ],
    "audience": "dziecko",
    "subcategory": "rytm_rytual_dnia",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D24-S2",
    "profile": "ST",
    "day": 24,
    "slot": "poludnie",
    "category": "misje",
    "title": "Powiedz najpierw co, potem dlaczego",
    "body": "Misja: gdy dziś o coś prosisz, najpierw powiedz co chcesz, potem dlaczego. Strateg uczy się układać myśli w prostą sekwencję.",
    "icon": "pulse",
    "tone": "paper",
    "time": "1 min",
    "tags": [
      "rozmowa",
      "sekwencja"
    ],
    "audience": "dziecko",
    "subcategory": "liczby_logika",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D24-S3",
    "profile": "ST",
    "day": 24,
    "slot": "wieczor",
    "category": "kraina",
    "title": "Mosiężne soczewki na zmianę",
    "body": "Mędrczyni zmienia w okularach soczewki: jedna pokazuje co było trudne, druga co było piękne. Dziś wybierz drugą.",
    "icon": "medrzec",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "refleksja",
      "wieczor"
    ],
    "audience": "dziecko",
    "subcategory": "planowanie",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D25-S1",
    "profile": "ST",
    "day": 25,
    "slot": "poranek",
    "category": "mentor",
    "title": "Krótkie listy, nie wielkie cele",
    "body": "Rodzicu, dla dziecka-Stratega lepsza jest krótka, zrobiona lista niż długi, niezrealizowany cel. Tnij rzeczy na małe kawałki.",
    "icon": "rodzic",
    "tone": "paper",
    "time": "1 min",
    "tags": [
      "lista",
      "planowanie"
    ],
    "audience": "rodzic",
    "subcategory": "planowanie",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D25-S2",
    "profile": "ST",
    "day": 25,
    "slot": "poludnie",
    "category": "misje",
    "title": "Pięć kroków powolnego marszu",
    "body": "Misja: zrób pięć bardzo powolnych kroków, licząc je na głos. Czuj, co się dzieje w stopach. Strateg czasem zwalnia, by zobaczyć więcej.",
    "icon": "pulse",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "uwaznosc",
      "ruch"
    ],
    "audience": "dziecko",
    "subcategory": "zdrowie_strateg",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D25-S3",
    "profile": "ST",
    "day": 25,
    "slot": "wieczor",
    "category": "misje",
    "title": "Co jutro przeniosę z dzisiaj",
    "body": "Mędrczyni mówi: wybierz jedną rzecz z dziś, którą warto wziąć ze sobą do jutra. Pomysł, słowo albo uśmiech.",
    "icon": "zwoj",
    "tone": "paper",
    "time": "1 min",
    "tags": [
      "jutro",
      "wieczor"
    ],
    "audience": "dziecko",
    "subcategory": "planowanie",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D26-S1",
    "profile": "ST",
    "day": 26,
    "slot": "poranek",
    "category": "misje",
    "title": "Mapa uczuć na trzy pola",
    "body": "Narysuj kartkę z trzema polami: spokój, ciekawość, niepokój. Postaw kropkę tam, gdzie jesteś dziś rano. Mędrczyni mówi: mapy uczuć też się rysuje.",
    "icon": "map",
    "tone": "magic",
    "time": "2 min",
    "tags": [
      "emocje",
      "poranek"
    ],
    "audience": "dziecko",
    "subcategory": "mapy_kolejnosc",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D26-S2",
    "profile": "ST",
    "day": 26,
    "slot": "poludnie",
    "category": "misje",
    "title": "Powtórz to, czego się uczysz",
    "body": "Misja: weź coś, czego się dziś uczysz, i powtórz to trzy razy w innych słowach. Strateg utrwala wiedzę przez powtórzenie z różnych stron.",
    "icon": "zwoj",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "nauka",
      "powtorzenie"
    ],
    "audience": "dziecko",
    "subcategory": "planowanie",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D26-S3",
    "profile": "ST",
    "day": 26,
    "slot": "wieczor",
    "category": "medrzec",
    "title": "Jedna rzecz, która mnie zaskoczyła",
    "body": "Mędrczyni pyta: co cię dziś zaskoczyło? Może mała rzecz. Zaskoczenie jest znakiem, że mapa się rozszerza.",
    "icon": "medrzec",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "ciekawosc",
      "wieczor"
    ],
    "audience": "rodzic",
    "subcategory": "mapy_kolejnosc",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D27-S1",
    "profile": "ST",
    "day": 27,
    "slot": "poranek",
    "category": "mentor",
    "title": "Strateg potrzebuje przerw",
    "body": "Rodzicu, między zadaniami daj dziecku-Strategowi krótką pauzę. Pięć minut bez planu, bez ekranu. Mózg porządkuje wtedy mapy w tle.",
    "icon": "rodzic",
    "tone": "paper",
    "time": "1 min",
    "tags": [
      "przerwa",
      "regeneracja"
    ],
    "audience": "rodzic",
    "subcategory": "planowanie",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D27-S2",
    "profile": "ST",
    "day": 27,
    "slot": "poludnie",
    "category": "misje",
    "title": "Zagadka licz po dwa",
    "body": "Misja: licz od dwa do dwudziestu, dodając po dwa: dwa, cztery, sześć. Sprawdź, jak szybko ci idzie. Strateg lubi rytm liczb.",
    "icon": "krysztal",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "liczby",
      "zagadka"
    ],
    "audience": "dziecko",
    "subcategory": "liczby_logika",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D27-S3",
    "profile": "ST",
    "day": 27,
    "slot": "wieczor",
    "category": "misje",
    "title": "Co poszło inaczej niż w planie",
    "body": "Wieczorem nazwij jedną rzecz, która poszła inaczej niż w planie, i jedną dobrą rzecz, która z tego wyszła. Mędrczyni mówi: zakręty bywają skrótami.",
    "icon": "zwoj",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "plan_b",
      "wieczor"
    ],
    "audience": "dziecko",
    "subcategory": "elastycznosc_plan_b",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D28-S1",
    "profile": "ST",
    "day": 28,
    "slot": "poranek",
    "category": "kraina",
    "title": "Zwój przed wyruszeniem",
    "body": "W Górach Liczb przed wyjściem z domu rozwija się jeden zwój i czyta jedno zdanie. Twoje dzisiejsze zdanie: idę krok po kroku.",
    "icon": "zwoj",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "intencja",
      "poranek"
    ],
    "audience": "dziecko",
    "subcategory": "planowanie",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D28-S2",
    "profile": "ST",
    "day": 28,
    "slot": "poludnie",
    "category": "misje",
    "title": "Posprzątaj jedno miejsce",
    "body": "Misja: wybierz jedno małe miejsce, na przykład biurko, i posprzątaj je w czterech krokach. Mędrczyni mówi: porządek na zewnątrz pomaga porządkowi w środku.",
    "icon": "pulse",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "porzadek",
      "krok_po_kroku"
    ],
    "audience": "dziecko",
    "subcategory": "planowanie",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D28-S3",
    "profile": "ST",
    "day": 28,
    "slot": "wieczor",
    "category": "mentor",
    "title": "Doceń elastyczność, nie tylko plan",
    "body": "Rodzicu, gdy dziecko zmieniło plan i poradziło sobie, powiedz to wprost: widzę, że umiesz przerysować mapę. To rzadka mądrość.",
    "icon": "rodzic",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "elastycznosc",
      "pochwala"
    ],
    "audience": "rodzic",
    "subcategory": "elastycznosc_plan_b",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D29-S1",
    "profile": "ST",
    "day": 29,
    "slot": "poranek",
    "category": "misje",
    "title": "Co chcę zapamiętać z tego miesiąca",
    "body": "Mędrczyni mówi: zostały dwa dni wędrówki. Pomyśl rano o jednej rzeczy, którą chcesz zapamiętać z tego miesiąca.",
    "icon": "krysztal",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "refleksja",
      "podsumowanie"
    ],
    "audience": "dziecko",
    "subcategory": "planowanie",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D29-S2",
    "profile": "ST",
    "day": 29,
    "slot": "poludnie",
    "category": "misje",
    "title": "Trzy razy się uśmiechnij",
    "body": "Misja: uśmiechnij się dziś świadomie trzy razy do innej osoby. Policz w głowie. Strateg liczy też ciepło.",
    "icon": "pulse",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "relacje",
      "liczby"
    ],
    "audience": "dziecko",
    "subcategory": "liczby_logika",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D29-S3",
    "profile": "ST",
    "day": 29,
    "slot": "wieczor",
    "category": "medrzec",
    "title": "Czego się o sobie dowiedziałem",
    "body": "Mędrczyni pyta: czego się w tym miesiącu dowiedziałeś o sobie? Jedna rzecz wystarczy. Zwiń jak zwój i schowaj w kieszeni serca.",
    "icon": "medrzec",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "samoocena",
      "podsumowanie"
    ],
    "audience": "rodzic",
    "subcategory": "planowanie",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D30-S1",
    "profile": "ST",
    "day": 30,
    "slot": "poranek",
    "category": "kraina",
    "title": "Ostatni szczyt tej mapy",
    "body": "W Górach Liczb dzisiaj wchodzisz na ostatni szczyt tej mapy. Spójrz w dół na drogę, którą przeszedłeś. Mędrczyni mówi: widok stąd jest twój.",
    "icon": "map",
    "tone": "magic",
    "time": "2 min",
    "tags": [
      "podsumowanie",
      "poranek"
    ],
    "audience": "dziecko",
    "subcategory": "mapy_kolejnosc",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D30-S2",
    "profile": "ST",
    "day": 30,
    "slot": "poludnie",
    "category": "misje",
    "title": "Trzy nowe rzeczy, które umiem",
    "body": "Misja: wymień trzy rzeczy, które dziś umiesz lepiej niż miesiąc temu. Małe też się liczą. Strateg zbiera dowody postępu.",
    "icon": "zwoj",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "postep",
      "podsumowanie"
    ],
    "audience": "dziecko",
    "subcategory": "planowanie",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "ST-D30-S3",
    "profile": "ST",
    "day": 30,
    "slot": "wieczor",
    "category": "mentor",
    "title": "Następna mapa już się rysuje",
    "body": "Rodzicu, dziś powiedz dziecku: skończyliśmy tę mapę razem. Następna już się rysuje. Strateg odpoczywa lepiej, gdy wie, że droga ma ciąg dalszy.",
    "icon": "rodzic",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "zamkniecie",
      "wieczor"
    ],
    "audience": "rodzic",
    "subcategory": "planowanie",
    "profileName": "Mistrz Map",
    "cecha": "Mądrość",
    "kraina": "Góry Liczb"
  },
  {
    "id": "KR-D01-S1",
    "profile": "KR",
    "day": 1,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "iskra_pomyslu",
    "audience": "dziecko",
    "title": "Jakim zwierzęciem jest dziś twoje śniadanie?",
    "body": "Spójrz na talerz i wymyśl, jakim stworkiem byłoby twoje śniadanie. Królikiem? Smokiem? Pływającym kotem? Nie ma złej odpowiedzi.",
    "icon": "krysztal",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "kreatywnosc",
      "wyobraznia",
      "poranek"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D01-S2",
    "profile": "KR",
    "day": 1,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "materialy_proste",
    "audience": "dziecko",
    "title": "Zamień karton w coś kosmicznego",
    "body": "Znajdź kawałek kartonu i zrób z niego jedną rzecz: hełm, statek, maskę. Nie musi być ładne. Wystarczy, że powstanie.",
    "icon": "artifact",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "karton",
      "material_z_domu",
      "tworzenie"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D01-S3",
    "profile": "KR",
    "day": 1,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "skarbnica_pomyslow",
    "audience": "rodzic",
    "title": "Załóż dziecku Skarbnicę Pomysłów",
    "body": "Weź zwykły zeszyt i nazwijcie go razem 'Skarbnica'. To miejsce, gdzie lądować będą pomysły na potem. Bez oceniania, bez musu używania.",
    "icon": "zwoj",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "notes",
      "skarbnica",
      "rytual"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D02-S1",
    "profile": "KR",
    "day": 2,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "slowa_jezyk",
    "audience": "dziecko",
    "title": "Wymyśl słowo, którego nie ma",
    "body": "Powiedz na głos jedno słowo, którego nikt nigdy nie słyszał. Co ono może znaczyć? Może to nazwa nowego koloru?",
    "icon": "krysztal",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "slowa",
      "jezyk",
      "wymyslanie"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D02-S2",
    "profile": "KR",
    "day": 2,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "eksperyment_artystyczny",
    "audience": "dziecko",
    "title": "Narysuj coś lewą ręką",
    "body": "Weź kartkę i spróbuj narysować kota ręką, której zwykle nie używasz. Będzie krzywy. Właśnie o to chodzi.",
    "icon": "artifact",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "rysunek",
      "eksperyment",
      "proces"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D02-S3",
    "profile": "KR",
    "day": 2,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "proces_nie_wynik",
    "audience": "rodzic",
    "title": "Pochwal próbowanie, nie efekt",
    "body": "Zamiast 'ładnie narysowałeś', spróbuj 'widzę, że próbowałeś czegoś nowego'. Dla Kreatora ważna jest droga, nie ocena końcowa.",
    "icon": "mentor",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "pochwala",
      "proces",
      "komunikacja"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D03-S1",
    "profile": "KR",
    "day": 3,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "iskra_pomyslu",
    "audience": "dziecko",
    "title": "Jakim kolorem pachnie dzisiaj powietrze?",
    "body": "Wyobraź sobie, że zapachy mają kolory. Jakim kolorem pachnie twoja kuchnia rano? A jakim twoja poduszka?",
    "icon": "krysztal",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "zmysly",
      "wyobraznia",
      "kolor"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D03-S2",
    "profile": "KR",
    "day": 3,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "dokonczenie",
    "audience": "dziecko",
    "title": "Wybierz jeden stary pomysł i skończ go",
    "body": "Zajrzyj do swoich rysunków albo budowli z klocków. Wybierz jeden niedokończony i zrób mu finisz. Choć mały.",
    "icon": "pulse",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "dokonczenie",
      "finisz",
      "rytm"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D03-S3",
    "profile": "KR",
    "day": 3,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "rytmy_dnia",
    "audience": "rodzic",
    "title": "Kreator zapomina o ciele",
    "body": "Dzieci-Kreatorzy w zapale tworzenia potrafią pominąć obiad. Łagodne przypomnienia o jedzeniu i wodzie są ważniejsze niż zegar.",
    "icon": "mentor",
    "tone": "leaf",
    "time": "2 min",
    "tags": [
      "jedzenie",
      "woda",
      "rytm_dnia"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D04-S1",
    "profile": "KR",
    "day": 4,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "iskra_pomyslu",
    "audience": "dziecko",
    "title": "Co by było, gdyby chmury były z waty cukrowej?",
    "body": "Pomyśl chwilę. Jakim smakiem byłyby chmury? Czy ptaki jadałyby je na śniadanie? Wymyśl trzy rzeczy.",
    "icon": "krysztal",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "co_by_bylo",
      "wyobraznia",
      "swiat"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D04-S2",
    "profile": "KR",
    "day": 4,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "materialy_proste",
    "audience": "dziecko",
    "title": "Zrób postać z trzech rzeczy z kuchni",
    "body": "Weź łyżkę, korek i serwetkę. Połącz je w postać. Ma imię? Ma supermoc? Tylko ty wiesz.",
    "icon": "artifact",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "material_z_domu",
      "postac",
      "budowanie"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D04-S3",
    "profile": "KR",
    "day": 4,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "balagan_porzadek",
    "audience": "rodzic",
    "title": "Bałagan to ślad pracy, nie nieporządek",
    "body": "Po sesji tworzenia warto ustalić prosty rytuał: pięć minut zbierania razem. Nie kara, tylko zamknięcie procesu. Kreator potrzebuje granicy.",
    "icon": "mentor",
    "tone": "leaf",
    "time": "2 min",
    "tags": [
      "balagan",
      "rytual",
      "sprzatanie"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D05-S1",
    "profile": "KR",
    "day": 5,
    "slot": "poranek",
    "category": "kraina",
    "subcategory": "iskra_pomyslu",
    "audience": "dziecko",
    "title": "Sekret Pustyni Pomysłów",
    "body": "Na Pustyni Pomysłów piasek mówi szeptem. Gdy zamkniesz oczy na chwilę rano, może usłyszysz jedną małą podpowiedź.",
    "icon": "map",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "kraina",
      "pustynia",
      "sekret"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D05-S2",
    "profile": "KR",
    "day": 5,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "eksperyment_artystyczny",
    "audience": "dziecko",
    "title": "Zaśpiewaj piosenkę o swoim kubku",
    "body": "Weź swój kubek. Wymyśl mu piosenkę na trzy linijki. Może być o jego uchu, o tym co lubi pić, o jego nastroju.",
    "icon": "pulse",
    "tone": "magic",
    "time": "2 min",
    "tags": [
      "piosenka",
      "spiew",
      "przedmiot"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D05-S3",
    "profile": "KR",
    "day": 5,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "relacje_inspiracja",
    "audience": "rodzic",
    "title": "Gdy pokazuje ci swoje dzieło",
    "body": "Kiedy dziecko pokazuje rysunek, najpierw zapytaj: 'Opowiedz mi o tym'. Bez interpretacji. Pytanie otwiera, ocena zamyka.",
    "icon": "mentor",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "rozmowa",
      "dzielo",
      "uwaga"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D06-S1",
    "profile": "KR",
    "day": 6,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "slowa_jezyk",
    "audience": "dziecko",
    "title": "Znajdź rym do swojego imienia",
    "body": "Powtórz swoje imię kilka razy i znajdź słowo, które brzmi podobnie. Śmieszne też mogą być. Tym lepiej.",
    "icon": "krysztal",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "rym",
      "imie",
      "slowa"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D06-S2",
    "profile": "KR",
    "day": 6,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "iskra_pomyslu",
    "audience": "dziecko",
    "title": "Wymyśl wynalazek dla leniwego smoka",
    "body": "Smok jest leniwy i nie chce latać. Co mu zaprojektujesz? Rysuj albo opowiedz na głos. Trzy minuty wystarczą.",
    "icon": "krysztal",
    "tone": "magic",
    "time": "2 min",
    "tags": [
      "wynalazek",
      "smok",
      "projekt"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D06-S3",
    "profile": "KR",
    "day": 6,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "skarbnica_pomyslow",
    "audience": "rodzic",
    "title": "Zapisz dziś jeden pomysł dziecka",
    "body": "Wieczorem zapytaj, co dziś wymyśliło. Zapisz to w skarbnicy, nawet jednym słowem. Zobaczy, że pomysły mają wartość.",
    "icon": "zwoj",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "skarbnica",
      "zapisywanie",
      "wartosc"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D07-S1",
    "profile": "KR",
    "day": 7,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "iskra_pomyslu",
    "audience": "dziecko",
    "title": "Wybierz dziś kolor dla całego dnia",
    "body": "Jeden kolor, który będzie ci dziś towarzyszył. Zobacz, ile go znajdziesz wokół. Pod stołem, w drzewie, w czyjejś koszulce.",
    "icon": "krysztal",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "kolor",
      "uwaga",
      "dzien"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D07-S2",
    "profile": "KR",
    "day": 7,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "proces_nie_wynik",
    "audience": "dziecko",
    "title": "Zrób brzydki szkic z dumą",
    "body": "Narysuj coś celowo brzydko. Krzywe oczy, dziwne kolory, niech nic nie pasuje. To też jest dzieło. Pierwszy szkic zawsze taki jest.",
    "icon": "pulse",
    "tone": "magic",
    "time": "2 min",
    "tags": [
      "szkic",
      "proces",
      "brzydkie"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D07-S3",
    "profile": "KR",
    "day": 7,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "rytmy_dnia",
    "audience": "rodzic",
    "title": "Sen Kreatora porządkuje pomysły",
    "body": "Dzieci 6-12 lat potrzebują 9-13 godzin snu. Dla małego Kreatora to czas, w którym mózg porządkuje wszystko, co wymyślił w dzień.",
    "icon": "mentor",
    "tone": "leaf",
    "time": "2 min",
    "tags": [
      "sen",
      "wellbeing",
      "regeneracja"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D08-S1",
    "profile": "KR",
    "day": 8,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "slowa_jezyk",
    "audience": "dziecko",
    "title": "Wymyśl tajne hasło na dziś",
    "body": "Słowo, które znasz tylko ty. Możesz je powtarzać w głowie. Jak ono brzmi? Jakie ma znaczenie tylko dla ciebie?",
    "icon": "krysztal",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "haslo",
      "sekret",
      "slowa"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D08-S2",
    "profile": "KR",
    "day": 8,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "materialy_proste",
    "audience": "dziecko",
    "title": "Zrób rzeźbę z papierowej kuli",
    "body": "Weź kartkę, zgniec ją w kulę, a potem powoli rozprostowuj tak, by została postać. Co widzisz? Gościa? Zwierzaka? Statek?",
    "icon": "artifact",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "papier",
      "rzezba",
      "material_z_domu"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D08-S3",
    "profile": "KR",
    "day": 8,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "balagan_porzadek",
    "audience": "rodzic",
    "title": "Trzy pudełka kończą tworzenie",
    "body": "Postaw obok stołu trzy pudła: 'gotowe', 'na potem', 'do wyrzucenia'. Dziecko samo segreguje. Sprzątanie staje się częścią tworzenia.",
    "icon": "mentor",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "pudelka",
      "sprzatanie",
      "system"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D09-S1",
    "profile": "KR",
    "day": 9,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "iskra_pomyslu",
    "audience": "dziecko",
    "title": "Twoje łóżko leci dziś w kosmos",
    "body": "Wstajesz z łóżka jak astronauta. Jakie planety odwiedziłeś we śnie? Powiedz jedną nazwę, choć wymyśloną.",
    "icon": "krysztal",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "kosmos",
      "sen",
      "wyobraznia"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D09-S2",
    "profile": "KR",
    "day": 9,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "eksperyment_artystyczny",
    "audience": "dziecko",
    "title": "Zrób kolaż z trzech śmiesznych skrawków",
    "body": "Znajdź trzy małe skrawki: gazety, opakowania, naklejki. Połącz je na kartce. Zobaczysz coś zupełnie nowego.",
    "icon": "artifact",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "kolaz",
      "skrawki",
      "laczenie"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D09-S3",
    "profile": "KR",
    "day": 9,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "relacje_inspiracja",
    "audience": "rodzic",
    "title": "Pokaż dziecku coś, co ty stworzyłaś",
    "body": "Zdjęcie, rysunek, ciasto, lista zakupów z bazgrołami. Dziecko widzi, że dorosłym też wolno tworzyć niedoskonałe rzeczy.",
    "icon": "mentor",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "wspolne",
      "tworzenie",
      "wzor"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D10-S1",
    "profile": "KR",
    "day": 10,
    "slot": "poranek",
    "category": "kraina",
    "subcategory": "iskra_pomyslu",
    "audience": "dziecko",
    "title": "Oaza na Pustyni Pomysłów",
    "body": "Na Pustyni jest oaza, gdzie rosną drzewa z papierowymi liśćmi. Każdy liść to jeden pomysł. Weź sobie dziś jeden.",
    "icon": "map",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "oaza",
      "kraina",
      "pustynia"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D10-S2",
    "profile": "KR",
    "day": 10,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "dokonczenie",
    "audience": "dziecko",
    "title": "Skończ jedno zdanie z opowieści",
    "body": "Zacznij od: 'Pewnego dnia kot założył czapkę i...'. Skończ jednym zdaniem. Tylko jednym. To wystarczy, by miało finisz.",
    "icon": "pulse",
    "tone": "magic",
    "time": "2 min",
    "tags": [
      "opowiesc",
      "dokonczenie",
      "zdanie"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D10-S3",
    "profile": "KR",
    "day": 10,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "proces_nie_wynik",
    "audience": "rodzic",
    "title": "Pierwszy szkic ma prawo być brzydki",
    "body": "Gdy dziecko mówi 'to brzydkie, wyrzucę', powiedz: 'pierwszy szkic zawsze taki jest, drugi będzie inny'. Nie ratuj dzieła, ratuj proces.",
    "icon": "mentor",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "szkic",
      "proces",
      "akceptacja"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D11-S1",
    "profile": "KR",
    "day": 11,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "slowa_jezyk",
    "audience": "dziecko",
    "title": "Powiedz dzień dobry w wymyślonym języku",
    "body": "Wymyśl trzy słowa, które znaczą 'dzień dobry'. Powiedz je do lustra. Jak brzmi twój język?",
    "icon": "krysztal",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "jezyk",
      "wymyslony",
      "powitanie"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D11-S2",
    "profile": "KR",
    "day": 11,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "iskra_pomyslu",
    "audience": "dziecko",
    "title": "Wymyśl trzy użycia dla łyżki",
    "body": "Co jeszcze można zrobić łyżką? Lustro? Mikrofon? Wahadło? Wymień trzy rzeczy, choć dziwne.",
    "icon": "krysztal",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "uzycia",
      "lyzka",
      "myslenie"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D11-S3",
    "profile": "KR",
    "day": 11,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "skarbnica_pomyslow",
    "audience": "rodzic",
    "title": "Pudełko na 'na potem'",
    "body": "Dziecko chce zacząć trzy projekty naraz? Powiedz: 'wrzućmy jeden do pudełka na potem'. Pomysł nie ginie, ale dziś kończysz tylko jedno.",
    "icon": "zwoj",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "pudelko",
      "na_potem",
      "wybor"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D12-S1",
    "profile": "KR",
    "day": 12,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "iskra_pomyslu",
    "audience": "dziecko",
    "title": "Wyobraź sobie szkołę na Marsie",
    "body": "Jaka byłaby lekcja w nieważkości? Z czego były tornistry? Co byłoby na obiad? Jedno zdanie wystarczy.",
    "icon": "krysztal",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "mars",
      "szkola",
      "wyobraznia"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D12-S2",
    "profile": "KR",
    "day": 12,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "materialy_proste",
    "audience": "dziecko",
    "title": "Zrób koronę z patyka i sznurka",
    "body": "Znajdź patyk i kawałek sznurka albo nitki. Owiń go w dowolny sposób. To korona, miecz, talizman. Decydujesz ty.",
    "icon": "artifact",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "patyk",
      "sznurek",
      "material_z_domu"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D12-S3",
    "profile": "KR",
    "day": 12,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "rytmy_dnia",
    "audience": "rodzic",
    "title": "Skład dnia daje przestrzeń na tworzenie",
    "body": "Kreator kwitnie, gdy ma czas wolny bez planu, ale też stałe ramy: posiłki, sen, ruch. Wolność rośnie w przewidywalności.",
    "icon": "mentor",
    "tone": "leaf",
    "time": "2 min",
    "tags": [
      "rytm",
      "przestrzen",
      "ramy"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D13-S1",
    "profile": "KR",
    "day": 13,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "iskra_pomyslu",
    "audience": "dziecko",
    "title": "Jak brzmi twój poranek?",
    "body": "Posłuchaj. Co słychać? Czajnik, kran, ptaki, krok. Wyobraź, że to instrumenty. Co grają?",
    "icon": "krysztal",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "dzwiek",
      "sluchanie",
      "muzyka"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D13-S2",
    "profile": "KR",
    "day": 13,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "eksperyment_artystyczny",
    "audience": "dziecko",
    "title": "Plama z wody i kolorów",
    "body": "Weź akwarele albo bibułki. Zostaw na kartce jedną dużą plamę. Co widzisz w jej kształcie? Smoka? Mapę? Dom?",
    "icon": "artifact",
    "tone": "magic",
    "time": "2 min",
    "tags": [
      "plama",
      "akwarela",
      "ksztalt"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D13-S3",
    "profile": "KR",
    "day": 13,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "relacje_inspiracja",
    "audience": "rodzic",
    "title": "Krytyka rodzeństwa boli najbardziej",
    "body": "Gdy rodzeństwo śmieje się z dzieła Kreatora, nie minimalizuj. Powiedz: 'wiem, że to bolało'. Walidacja chroni odwagę tworzenia.",
    "icon": "mentor",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "krytyka",
      "rodzenstwo",
      "walidacja"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D14-S1",
    "profile": "KR",
    "day": 14,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "slowa_jezyk",
    "audience": "dziecko",
    "title": "Wymyśl trzy słowa na to, co czujesz",
    "body": "Czasem 'wesoły' albo 'smutny' to za mało. Wymyśl własne słowo na to, co czujesz dziś. Może 'kremowosłonecznie'?",
    "icon": "krysztal",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "uczucia",
      "slowa",
      "wlasne"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D14-S2",
    "profile": "KR",
    "day": 14,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "dokonczenie",
    "audience": "dziecko",
    "title": "Wybierz jeden szkic i pokoloruj go",
    "body": "Weź stary rysunek bez koloru. Wybierz trzy kredki i skończ go. Niech będzie gotowy do skarbnicy.",
    "icon": "pulse",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "dokonczenie",
      "kolor",
      "szkic"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D14-S3",
    "profile": "KR",
    "day": 14,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "balagan_porzadek",
    "audience": "rodzic",
    "title": "Razem zbierajcie po tworzeniu",
    "body": "Pięć minut sprzątania razem na koniec. Własny gest, własna piosenka. Sprzątanie staje się częścią tworzenia, nie karą po nim.",
    "icon": "mentor",
    "tone": "leaf",
    "time": "2 min",
    "tags": [
      "sprzatanie",
      "razem",
      "rytual"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D15-S1",
    "profile": "KR",
    "day": 15,
    "slot": "poranek",
    "category": "kraina",
    "subcategory": "iskra_pomyslu",
    "audience": "dziecko",
    "title": "Miraż na Pustyni Pomysłów",
    "body": "Na pustyni czasem widzi się coś, czego nie ma. To miraż. Twoja wyobraźnia też umie tak czarować. Co dziś zobaczysz?",
    "icon": "map",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "miraz",
      "pustynia",
      "kraina"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D15-S2",
    "profile": "KR",
    "day": 15,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "iskra_pomyslu",
    "audience": "dziecko",
    "title": "Wymyśl historyjkę o znalezionym kamyku",
    "body": "Wyjdź na chwilę, znajdź kamyk. Skąd on jest? Czego słuchał? Wymyśl jedno zdanie o jego życiu.",
    "icon": "krysztal",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "kamyk",
      "historia",
      "natura"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D15-S3",
    "profile": "KR",
    "day": 15,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "skarbnica_pomyslow",
    "audience": "rodzic",
    "title": "Połowa miesiąca w skarbnicy",
    "body": "Po dwóch tygodniach zajrzyjcie razem do skarbnicy. Żaden pomysł nie musi być zrealizowany. Wystarczy, że tam jest. To uczy szacunku do własnych myśli.",
    "icon": "zwoj",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "skarbnica",
      "szacunek",
      "mysli"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D16-S1",
    "profile": "KR",
    "day": 16,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "iskra_pomyslu",
    "audience": "dziecko",
    "title": "Gdyby drzewa mogły chodzić",
    "body": "Dokąd by szły? Do morza? Do sąsiada? Po lody? Wymyśl jedno miejsce, do którego idzie twoje drzewo za oknem.",
    "icon": "krysztal",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "drzewa",
      "co_by_bylo",
      "wyobraznia"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D16-S2",
    "profile": "KR",
    "day": 16,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "eksperyment_artystyczny",
    "audience": "dziecko",
    "title": "Maluj palcem zamoczonym w soku",
    "body": "Kropla soku, palec, kartka. Bez pędzla, bez planu. Co powstaje? Plamy mają swój urok. Zostaw je takimi, jakie są.",
    "icon": "artifact",
    "tone": "magic",
    "time": "2 min",
    "tags": [
      "palec",
      "sok",
      "malowanie"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D16-S3",
    "profile": "KR",
    "day": 16,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "proces_nie_wynik",
    "audience": "rodzic",
    "title": "Dziwne dzieło to znak myślenia",
    "body": "Gdy widzisz coś 'dziwnego', to znak, że dziecko połączyło dwa odległe światy. To wartość, nie błąd. Doceniaj zaskoczenie.",
    "icon": "mentor",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "dziwne",
      "polaczenia",
      "myslenie"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D17-S1",
    "profile": "KR",
    "day": 17,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "slowa_jezyk",
    "audience": "dziecko",
    "title": "Wymyśl śmieszne imię dla swojej skarpetki",
    "body": "Skarpetki na nogach mają imiona. Lewa nazywa się..., prawa nazywa się.... Powiedz na głos.",
    "icon": "krysztal",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "imie",
      "skarpetka",
      "humor"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D17-S2",
    "profile": "KR",
    "day": 17,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "materialy_proste",
    "audience": "dziecko",
    "title": "Zbuduj most dla mrówki",
    "body": "Weź patyk, kawałek folii, słomkę. Zbuduj coś, po czym może przejść mała mrówka. Nie musi być trwałe.",
    "icon": "artifact",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "most",
      "mrowka",
      "budowanie"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D17-S3",
    "profile": "KR",
    "day": 17,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "rytmy_dnia",
    "audience": "rodzic",
    "title": "Ruch lekarstwem na natłok pomysłów",
    "body": "WHO zaleca 60 minut ruchu dziennie. Dla Kreatora to nie obowiązek, lecz reset. Po bieganiu, skakaniu, tańcu wraca jasna głowa.",
    "icon": "mentor",
    "tone": "leaf",
    "time": "2 min",
    "tags": [
      "ruch",
      "wellbeing",
      "reset"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D18-S1",
    "profile": "KR",
    "day": 18,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "iskra_pomyslu",
    "audience": "dziecko",
    "title": "Twój cień dziś jest tancerzem",
    "body": "Stań w słońcu albo w lampie. Niech twój cień zatańczy. Jaki jest taniec? Śmieszny? Powolny? Dziki?",
    "icon": "krysztal",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "cien",
      "taniec",
      "ruch"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D18-S2",
    "profile": "KR",
    "day": 18,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "dokonczenie",
    "audience": "dziecko",
    "title": "Wymyśl koniec ulubionej bajki",
    "body": "Pomyśl o bajce, którą znasz. A teraz wymyśl inny koniec. Co by było, gdyby Czerwony Kapturek poleciał w kosmos?",
    "icon": "pulse",
    "tone": "magic",
    "time": "2 min",
    "tags": [
      "bajka",
      "koniec",
      "wersja"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D18-S3",
    "profile": "KR",
    "day": 18,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "relacje_inspiracja",
    "audience": "rodzic",
    "title": "Pierwsza publiczność jest delikatna",
    "body": "Gdy dziecko po raz pierwszy pokazuje dzieło, jest jak pierwszy pomidor wystawiony przez okno. Poważnie wysłuchaj. Reszta świata będzie surowsza.",
    "icon": "mentor",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "publicznosc",
      "pierwsza",
      "ochrona"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D19-S1",
    "profile": "KR",
    "day": 19,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "iskra_pomyslu",
    "audience": "dziecko",
    "title": "Co by było, gdyby buty mówiły",
    "body": "Co dziś powiedziałyby twoje buty? Są zmęczone? Głodne nowych przygód? Wymyśl jedno zdanie ich głosem.",
    "icon": "krysztal",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "buty",
      "glos",
      "co_by_bylo"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D19-S2",
    "profile": "KR",
    "day": 19,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "eksperyment_artystyczny",
    "audience": "dziecko",
    "title": "Rysuj z zamkniętymi oczami",
    "body": "Weź kartkę i kredkę. Zamknij oczy na 30 sekund i rysuj. Otwórz i zobacz, co powstało. Można się zaśmiać.",
    "icon": "artifact",
    "tone": "magic",
    "time": "2 min",
    "tags": [
      "oczy",
      "rysunek",
      "eksperyment"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D19-S3",
    "profile": "KR",
    "day": 19,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "skarbnica_pomyslow",
    "audience": "rodzic",
    "title": "Pomysł uratowany od zapomnienia",
    "body": "Dziecko mówi przed snem: 'mam pomysł!'. Zapisz w jego skarbnicy jednym słowem. Jutro rano znajdzie iskrę tam, gdzie wczoraj ją zostawiło.",
    "icon": "zwoj",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "pomysl",
      "wieczor",
      "zapis"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D20-S1",
    "profile": "KR",
    "day": 20,
    "slot": "poranek",
    "category": "kraina",
    "subcategory": "iskra_pomyslu",
    "audience": "dziecko",
    "title": "Karawana pomysłów na Pustyni",
    "body": "Na Pustyni Pomysłów idą długie karawany. Każdy wielbłąd niesie jeden pomysł. Co dziś niesie twój wielbłąd?",
    "icon": "map",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "karawana",
      "wielblad",
      "pomysl"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D20-S2",
    "profile": "KR",
    "day": 20,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "iskra_pomyslu",
    "audience": "dziecko",
    "title": "Wymyśl nowy smak lodów",
    "body": "Smak, którego nie ma w lodziarni. Lody o smaku deszczu? Lody o smaku poniedziałku? Powiedz na głos albo narysuj.",
    "icon": "krysztal",
    "tone": "magic",
    "time": "2 min",
    "tags": [
      "lody",
      "smak",
      "wymyslanie"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D20-S3",
    "profile": "KR",
    "day": 20,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "balagan_porzadek",
    "audience": "rodzic",
    "title": "Kącik tworzenia z własnymi zasadami",
    "body": "Jeśli możesz, wyznacz mały stolik albo kąt tylko do tworzenia. Tam bałagan jest dozwolony. Reszta domu odpoczywa.",
    "icon": "mentor",
    "tone": "leaf",
    "time": "2 min",
    "tags": [
      "kacik",
      "stol",
      "przestrzen"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D21-S1",
    "profile": "KR",
    "day": 21,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "slowa_jezyk",
    "audience": "dziecko",
    "title": "Trzy rymujące się słowa do owsianki",
    "body": "Owsianka, ławka, oranżanka? Wymyśl trzy słowa, które brzmią podobnie do tego, co masz na śniadanie.",
    "icon": "krysztal",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "rym",
      "sniadanie",
      "slowa"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D21-S2",
    "profile": "KR",
    "day": 21,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "materialy_proste",
    "audience": "dziecko",
    "title": "Domek dla owada z liści",
    "body": "Idź na dwór albo na balkon. Z liścia, patyka, kamyka zrób mikro-domek. Dla biedronki? Ślimaka? Mrówki?",
    "icon": "artifact",
    "tone": "leaf",
    "time": "2 min",
    "tags": [
      "domek",
      "owad",
      "natura"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D21-S3",
    "profile": "KR",
    "day": 21,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "rytmy_dnia",
    "audience": "rodzic",
    "title": "Woda obok stanowiska pracy",
    "body": "Dzieci-Kreatorzy często są rozkojarzone i zapominają pić. Postaw butelkę obok ich miejsca pracy. Bez upominania, po prostu blisko.",
    "icon": "mentor",
    "tone": "leaf",
    "time": "2 min",
    "tags": [
      "woda",
      "wellbeing",
      "drobne"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D22-S1",
    "profile": "KR",
    "day": 22,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "iskra_pomyslu",
    "audience": "dziecko",
    "title": "Wymyśl supermoc dla swojej szczoteczki",
    "body": "Co potrafi twoja szczoteczka, oprócz mycia zębów? Lata? Śpiewa? Zna język delfinów? Jedna supermoc.",
    "icon": "krysztal",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "supermoc",
      "szczoteczka",
      "humor"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D22-S2",
    "profile": "KR",
    "day": 22,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "eksperyment_artystyczny",
    "audience": "dziecko",
    "title": "Naucz się rysować jeden kształt",
    "body": "Wybierz jeden kształt, np. spiralę. Narysuj go pięć razy w różnych rozmiarach. Pierwszy będzie krzywy, piąty będzie twój.",
    "icon": "artifact",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "ksztalt",
      "spirala",
      "praktyka"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D22-S3",
    "profile": "KR",
    "day": 22,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "relacje_inspiracja",
    "audience": "rodzic",
    "title": "Wystawa w korytarzu",
    "body": "Powieś jedno dzieło dziecka na sznurku albo na lodówce. To pierwsze publiczne miejsce. Wymieniaj co tydzień. Dom staje się galerią.",
    "icon": "mentor",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "wystawa",
      "galeria",
      "dom"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D23-S1",
    "profile": "KR",
    "day": 23,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "iskra_pomyslu",
    "audience": "dziecko",
    "title": "Jak pachnie dzisiejszy poniedziałek?",
    "body": "Albo wtorek, albo środa. Każdy dzień ma swój zapach. Wymyśl jaki. Cynamon? Deszcz? Stara książka?",
    "icon": "krysztal",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "zapach",
      "dzien",
      "tygodnia"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D23-S2",
    "profile": "KR",
    "day": 23,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "dokonczenie",
    "audience": "dziecko",
    "title": "Skończ piosenkę jednym refrenem",
    "body": "Wymyśl jedną linijkę, która będzie refrenem twojej piosenki o dzisiejszym dniu. Powtórz ją trzy razy. To gotowe dzieło.",
    "icon": "pulse",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "piosenka",
      "refren",
      "dokonczenie"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D23-S3",
    "profile": "KR",
    "day": 23,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "proces_nie_wynik",
    "audience": "rodzic",
    "title": "Trzy pytania zamiast oceny",
    "body": "Zamiast 'ładnie', spróbuj: 'co tu się dzieje?', 'co było trudne?', 'co lubisz w tym najbardziej?'. Pytania pokazują, że widzisz proces.",
    "icon": "mentor",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "pytania",
      "ocena",
      "proces"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D24-S1",
    "profile": "KR",
    "day": 24,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "slowa_jezyk",
    "audience": "dziecko",
    "title": "Wymyśl nazwę dla dzisiejszej chmury",
    "body": "Spójrz przez okno. Widzisz chmurę? Daj jej imię. Jeśli nie ma chmur, nazwij niebo. Niebo też może mieć imię.",
    "icon": "krysztal",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "chmura",
      "imie",
      "niebo"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D24-S2",
    "profile": "KR",
    "day": 24,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "iskra_pomyslu",
    "audience": "dziecko",
    "title": "Połącz dwie rzeczy w jedną",
    "body": "Weź dwie rzeczy z domu i wymyśl, czym mogłyby być razem. Łyżka + grzebień = grzebkołyżka? Co by robiła?",
    "icon": "krysztal",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "polaczenia",
      "rzeczy",
      "wymyslanie"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D24-S3",
    "profile": "KR",
    "day": 24,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "skarbnica_pomyslow",
    "audience": "rodzic",
    "title": "Skarbnica jako prezent od siebie",
    "body": "Pokaż dziecku skarbnicę jako miejsce, gdzie wraca po inspirację. 'Twoje pomysły czekają na ciebie'. To uczy, że warto słuchać własnej głowy.",
    "icon": "zwoj",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "skarbnica",
      "wraca",
      "wartosc"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D25-S1",
    "profile": "KR",
    "day": 25,
    "slot": "poranek",
    "category": "kraina",
    "subcategory": "iskra_pomyslu",
    "audience": "dziecko",
    "title": "Wiatr na Pustyni szepcze pomysł",
    "body": "Zamknij oczy i posłuchaj wiatru, choć tylko swojego oddechu. Jaki pomysł ci przynosi dziś? Jedno słowo wystarczy.",
    "icon": "map",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "wiatr",
      "oddech",
      "pomysl"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D25-S2",
    "profile": "KR",
    "day": 25,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "materialy_proste",
    "audience": "dziecko",
    "title": "Lampa ze słoika i papieru",
    "body": "Weź słoik i kawałek bibułki albo papieru. Owiń słoik kolorowym papierem. Postaw obok lampy. Będzie jak latarnia.",
    "icon": "artifact",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "sloik",
      "lampa",
      "material_z_domu"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D25-S3",
    "profile": "KR",
    "day": 25,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "rytmy_dnia",
    "audience": "rodzic",
    "title": "Wieczorne wyciszenie po dniu pomysłów",
    "body": "Po dniu pełnym tworzenia mózg małego Kreatora jest pełen iskier. Ciepła woda, książka, ciemniejsze światło pomaga łagodnie zakończyć dzień.",
    "icon": "mentor",
    "tone": "leaf",
    "time": "2 min",
    "tags": [
      "wyciszenie",
      "sen",
      "wieczor"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D26-S1",
    "profile": "KR",
    "day": 26,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "iskra_pomyslu",
    "audience": "dziecko",
    "title": "Co by zjadł twój pluszak na śniadanie?",
    "body": "Weź ulubioną maskotkę. Wymyśl, co by zjadła dziś na śniadanie. Krzesło? Część kapci? Trzy okruszki marzeń?",
    "icon": "krysztal",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "pluszak",
      "sniadanie",
      "humor"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D26-S2",
    "profile": "KR",
    "day": 26,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "eksperyment_artystyczny",
    "audience": "dziecko",
    "title": "Stempelki z warzyw",
    "body": "Poproś dorosłego o pomoc. Pół marchewki, pół jabłka, kropla farby. Odbij na kartce. Zobaczysz wzory, których nie wymyślisz.",
    "icon": "artifact",
    "tone": "magic",
    "time": "2 min",
    "tags": [
      "stempel",
      "warzywa",
      "wzory"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D26-S3",
    "profile": "KR",
    "day": 26,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "balagan_porzadek",
    "audience": "rodzic",
    "title": "Zdjęcie zamiast trzymania wszystkiego",
    "body": "Nie da się zatrzymać każdego dzieła. Zrób zdjęcie i wpiszcie do cyfrowej skarbnicy. Dzieło może ruszyć dalej, pamięć zostaje.",
    "icon": "mentor",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "zdjecie",
      "pamiec",
      "skarbnica"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D27-S1",
    "profile": "KR",
    "day": 27,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "slowa_jezyk",
    "audience": "dziecko",
    "title": "Słowo, które brzmi jak smak",
    "body": "Powiedz słowo, które brzmi słodko. Albo kwaśno. Albo chrupiąco. Jak to słowo wygląda? Jak smakuje?",
    "icon": "krysztal",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "slowo",
      "smak",
      "brzmienie"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D27-S2",
    "profile": "KR",
    "day": 27,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "dokonczenie",
    "audience": "dziecko",
    "title": "Wybierz jeden pomysł ze skarbnicy",
    "body": "Otwórz skarbnicę. Pokaż palcem na pierwszy pomysł, jaki widzisz. Zrób z nim jedno małe coś. Zamknij projekt.",
    "icon": "pulse",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "skarbnica",
      "dokonczenie",
      "wybor"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D27-S3",
    "profile": "KR",
    "day": 27,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "relacje_inspiracja",
    "audience": "rodzic",
    "title": "Inspiracja z natury, nie z ekranów",
    "body": "Krótki spacer, okno, balkon. Dla Kreatora świat realny daje więcej iskier niż długie oglądanie filmów. Patrzcie razem na chmury, drzewa, deszcze.",
    "icon": "mentor",
    "tone": "leaf",
    "time": "2 min",
    "tags": [
      "natura",
      "inspiracja",
      "spacer"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D28-S1",
    "profile": "KR",
    "day": 28,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "iskra_pomyslu",
    "audience": "dziecko",
    "title": "Jakim kształtem jest twój sen?",
    "body": "Właśnie się obudziłeś. Twój sen miał kształt? Trójkąt? Spirala? Chmura? Powiedz po cichu w głowie.",
    "icon": "krysztal",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "sen",
      "ksztalt",
      "wyobraznia"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D28-S2",
    "profile": "KR",
    "day": 28,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "iskra_pomyslu",
    "audience": "dziecko",
    "title": "Wymyśl tytuł książki, której nie ma",
    "body": "Tytuł ma być śmieszny, dziwny albo tajemniczy. 'Kot, który zjadł kalendarz'? 'Spirala bez końca'? Powiedz na głos.",
    "icon": "krysztal",
    "tone": "magic",
    "time": "2 min",
    "tags": [
      "ksiazka",
      "tytul",
      "wymyslanie"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D28-S3",
    "profile": "KR",
    "day": 28,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "skarbnica_pomyslow",
    "audience": "rodzic",
    "title": "Pomysł ze skarbnicy w kalendarzu",
    "body": "Wybierzcie razem jeden pomysł ze skarbnicy, który chcecie zrobić w tym tygodniu. Zapis w kalendarzu pokazuje, że marzenia mają datę.",
    "icon": "zwoj",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "kalendarz",
      "skarbnica",
      "plan"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D29-S1",
    "profile": "KR",
    "day": 29,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "iskra_pomyslu",
    "audience": "dziecko",
    "title": "Jaki kolor ma dziś twój humor?",
    "body": "Nie 'wesoły' ani 'zły'. Kolor. Pomarańczowy? Granatowy w kropki? Powiedz, jaki kolor wybrałabyś dziś na swoją koszulkę.",
    "icon": "krysztal",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "humor",
      "kolor",
      "uczucia"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D29-S2",
    "profile": "KR",
    "day": 29,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "eksperyment_artystyczny",
    "audience": "dziecko",
    "title": "Mapa twojego pokoju z gwiazd",
    "body": "Narysuj swój pokój tak, jakby składał się z gwiazd. Łóżko to konstelacja, biurko to kometa. Wystarczy szkic.",
    "icon": "artifact",
    "tone": "magic",
    "time": "2 min",
    "tags": [
      "pokoj",
      "gwiazdy",
      "mapa"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D29-S3",
    "profile": "KR",
    "day": 29,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "proces_nie_wynik",
    "audience": "rodzic",
    "title": "Niedokończone też ma wartość",
    "body": "Czasem proces sam w sobie jest dziełem. Jeśli dziecko nie chce kończyć, zapytaj: 'czego się nauczyłeś?'. Doświadczenie liczy się tak samo.",
    "icon": "mentor",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "niedokonczone",
      "nauka",
      "wartosc"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D30-S1",
    "profile": "KR",
    "day": 30,
    "slot": "poranek",
    "category": "kraina",
    "subcategory": "iskra_pomyslu",
    "audience": "dziecko",
    "title": "Atrament Kronikarski w twoich palcach",
    "body": "Dziś jest ostatni dzień podróży. Wyobraź sobie, że masz w palcach świetlistego atramentu. Co napiszesz w powietrzu?",
    "icon": "artifact",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "atrament",
      "artefakt",
      "kronika"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D30-S2",
    "profile": "KR",
    "day": 30,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "dokonczenie",
    "audience": "dziecko",
    "title": "Pokaż jedno dzieło komuś bliskiemu",
    "body": "Wybierz jedną rzecz, którą zrobiłeś w tym miesiącu. Pokaż mamie, tacie, siostrze, koledze. Powiedz: 'to zrobiłem'. Zamykasz krąg.",
    "icon": "pulse",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "pokaz",
      "dzielo",
      "zamkniecie"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "KR-D30-S3",
    "profile": "KR",
    "day": 30,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "relacje_inspiracja",
    "audience": "rodzic",
    "title": "Po miesiącu zajrzyjcie do skarbnicy razem",
    "body": "Otwórzcie razem skarbnicę. Bez oceniania, bez planów. Po prostu zobaczcie, ile myśli się tam zebrało. To podarunek dziecka samemu sobie.",
    "icon": "mentor",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "skarbnica",
      "podsumowanie",
      "razem"
    ],
    "profileName": "Tkacz Snów",
    "cecha": "Kreatywność",
    "kraina": "Pustynia Pomysłów"
  },
  {
    "id": "LD-D01-S1",
    "profile": "LD",
    "day": 1,
    "slot": "poranek",
    "category": "medrzec",
    "title": "Twoje serce wie, co dobre",
    "body": "Dobry dzień, mały Gwardzisto. Zanim wstaniesz, połóż rękę na piersi. Posłuchaj jednej rzeczy, którą serce ci podpowie. To twój pierwszy krok.",
    "icon": "medrzec",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "odwaga",
      "intuicja"
    ],
    "audience": "dziecko",
    "subcategory": "odwaga_mala",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D01-S2",
    "profile": "LD",
    "day": 1,
    "slot": "poludnie",
    "category": "misje",
    "title": "Powiedz cześć pierwszy",
    "body": "Dzisiejsza mała misja: powiedz dziś komuś cześć jako pierwsza/pierwszy. Może to być koleżanka, sąsiad, pani w sklepie. Jeden mały krok odwagi.",
    "icon": "pulse",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "inicjatywa",
      "odwaga"
    ],
    "audience": "dziecko",
    "subcategory": "inicjatywa_grupa",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D01-S3",
    "profile": "LD",
    "day": 1,
    "slot": "wieczor",
    "category": "medrzec",
    "title": "Gdzie dziś byłem pierwszy?",
    "body": "Wieczorem pomyśl: w czym dziś zrobiłaś/zrobiłeś pierwszy ruch? Nawet maleńki. Pamiętaj, lider to nie ten, kto krzyczy najgłośniej, tylko ten, kto zaczyna z troską.",
    "icon": "pulse",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "refleksja",
      "inicjatywa"
    ],
    "audience": "rodzic",
    "subcategory": "inicjatywa_grupa",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D02-S1",
    "profile": "LD",
    "day": 2,
    "slot": "poranek",
    "category": "mentor",
    "title": "Rodzicu: nie tłum jego głosu",
    "body": "Drogi Rodzicu, dziecko-Lider potrzebuje słyszeć: „dobrze, że masz zdanie”. Spróbuj dziś nie przerywać, gdy mówi coś z przekonaniem. Pyta o pozwolenie tylko wtedy, gdy je dasz.",
    "icon": "rodzic",
    "tone": "magic",
    "time": "2 min",
    "tags": [
      "rodzic",
      "asertywność"
    ],
    "audience": "rodzic",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D02-S2",
    "profile": "LD",
    "day": 2,
    "slot": "poludnie",
    "category": "misje",
    "title": "Zaproponuj jedną zabawę",
    "body": "Misja: dzisiaj zaproponuj rodzeństwu, koledze albo klasie jedną nową zabawę albo pomysł. Powiedz spokojnie: „mam pomysł, posłuchajcie”. I poczekaj, co odpowiedzą.",
    "icon": "pulse",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "inicjatywa",
      "grupa"
    ],
    "audience": "dziecko",
    "subcategory": "inicjatywa_grupa",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D02-S3",
    "profile": "LD",
    "day": 2,
    "slot": "wieczor",
    "category": "kraina",
    "title": "Niebo Marzeń słucha cię wieczorem",
    "body": "Twoja kraina, Niebo Marzeń, otwiera się wieczorem. Spójrz przez okno na niebo. Powiedz cicho jedno marzenie — Mędrczyni je zapisze gwiazdami.",
    "icon": "artifact",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "kraina",
      "marzenia"
    ],
    "audience": "dziecko",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D03-S1",
    "profile": "LD",
    "day": 3,
    "slot": "poranek",
    "category": "medrzec",
    "title": "Dziś jestem dla kogoś",
    "body": "Mały Gwardzisto, zacznij dzień od myśli: „dziś jestem dla kogoś”. Nie musisz wiedzieć dla kogo. Życie ci podpowie. Tarcza Słońca rozgrzewa twoją pierś.",
    "icon": "artifact",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "troska",
      "postawa"
    ],
    "audience": "dziecko",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D03-S2",
    "profile": "LD",
    "day": 3,
    "slot": "poludnie",
    "category": "misje",
    "title": "Pomóż, zanim poprosi",
    "body": "Misja odwagi: znajdź dziś kogoś, kto sam by nie poprosił o pomoc. Może koleżanka, która siedzi sama. Podejdź. Powiedz: „chcesz, posiedzę z tobą?”.",
    "icon": "pulse",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "pomoc",
      "troska"
    ],
    "audience": "dziecko",
    "subcategory": "pomoc_proaktywna",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D03-S3",
    "profile": "LD",
    "day": 3,
    "slot": "wieczor",
    "category": "medrzec",
    "title": "Pycha czy troska?",
    "body": "Wieczorne pytanie Mędrczyni: dziś kiedy byłam/byłem pierwszy — z pychy, czy z troski? Obie odpowiedzi są w porządku. Ważne, żeby je zobaczyć.",
    "icon": "medrzec",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "refleksja",
      "troska"
    ],
    "audience": "rodzic",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D04-S1",
    "profile": "LD",
    "day": 4,
    "slot": "poranek",
    "category": "mentor",
    "title": "Rodzicu: daj mu wybór",
    "body": "Drogi Rodzicu, Lider rośnie, gdy decyduje. Daj dziecku dziś jeden prawdziwy wybór: co na śniadanie, jaką trasę do szkoły, jaką książkę wieczorem. Decyzja buduje serce.",
    "icon": "rodzic",
    "tone": "magic",
    "time": "2 min",
    "tags": [
      "rodzic",
      "decyzje"
    ],
    "audience": "rodzic",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D04-S2",
    "profile": "LD",
    "day": 4,
    "slot": "poludnie",
    "category": "misje",
    "title": "Powiedz „nie” spokojnie",
    "body": "Mała wielka misja: jeśli ktoś dziś poprosi cię o coś, na co nie masz ochoty, powiedz spokojnie „dziękuję, nie”. Bez krzyku, bez tłumaczenia długo. „Nie” też jest odwagą.",
    "icon": "pulse",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "granice",
      "asertywność"
    ],
    "audience": "dziecko",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D04-S3",
    "profile": "LD",
    "day": 4,
    "slot": "wieczor",
    "category": "medrzec",
    "title": "Sen to też zbroja",
    "body": "Posłuchaj, mały Gwardzisto: lider, który nie śpi, traci tarczę. Idź spać tak, żeby spać 9 do 13 godzin. To nie jest słabość. To naładowanie grzywy.",
    "icon": "medrzec",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "sen",
      "odpoczynek_lidera"
    ],
    "audience": "rodzic",
    "subcategory": "regeneracja_lidera",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D05-S1",
    "profile": "LD",
    "day": 5,
    "slot": "poranek",
    "category": "kraina",
    "title": "Tarcza Słońca w piersi",
    "body": "Twój pierwszy artefakt to Tarcza Słońca. Wyobraź sobie, że masz ją w piersi. Świeci ciepłem dla każdego, kto cię spotka. Wstajesz z tarczą.",
    "icon": "artifact",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "artefakt",
      "kraina"
    ],
    "audience": "dziecko",
    "subcategory": "odwaga_mala",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D05-S2",
    "profile": "LD",
    "day": 5,
    "slot": "poludnie",
    "category": "misje",
    "title": "Zapytaj cichego kolegę",
    "body": "Misja: znajdź dziś osobę, która mało mówi w grupie. Zapytaj: „co o tym myślisz?”. Posłuchaj. Liderzy nie tylko mówią — pomagają mówić innym.",
    "icon": "pulse",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "słuchanie",
      "grupa"
    ],
    "audience": "dziecko",
    "subcategory": "inicjatywa_grupa",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D05-S3",
    "profile": "LD",
    "day": 5,
    "slot": "wieczor",
    "category": "medrzec",
    "title": "Kogo dziś podniosłam/podniosłem?",
    "body": "Wieczorem zlicz: komu dziś dodałam/dodałem siły? Choćby jednym zdaniem, uśmiechem, „dasz radę”. Mała tarcza, którą podałeś — wraca podwójnie.",
    "icon": "medrzec",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "refleksja",
      "troska"
    ],
    "audience": "rodzic",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D06-S1",
    "profile": "LD",
    "day": 6,
    "slot": "poranek",
    "category": "medrzec",
    "title": "Pierś do przodu, oczy miękkie",
    "body": "Gwardzisto, ustaw się dziś tak: pierś do przodu, ramiona w dół, oczy miękkie. Ciało powie sercu: „jestem gotowa, jestem gotowy”. I dzień zacznie się dobrze.",
    "icon": "pulse",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "postawa",
      "ciało"
    ],
    "audience": "dziecko",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D06-S2",
    "profile": "LD",
    "day": 6,
    "slot": "poludnie",
    "category": "misje",
    "title": "60 minut ruchu, jak lew",
    "body": "Misja ciała: dziś ruszaj się 60 minut. Biegaj, skacz, tańcz, kop piłkę. Lwia grzywa lubi wiatr. Tarcza Słońca świeci jaśniej, gdy się ruszasz.",
    "icon": "pulse",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "ruch",
      "ciało"
    ],
    "audience": "dziecko",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D06-S3",
    "profile": "LD",
    "day": 6,
    "slot": "wieczor",
    "category": "mentor",
    "title": "Rodzicu: pochwal odwagę, nie wynik",
    "body": "Drogi Rodzicu, dziś wieczorem powiedz dziecku konkretnie: „widziałam, że było ci trudno i zrobiłaś to”. To zdanie buduje Lidera bardziej niż „brawo”.",
    "icon": "rodzic",
    "tone": "magic",
    "time": "2 min",
    "tags": [
      "rodzic",
      "pochwała"
    ],
    "audience": "rodzic",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D07-S1",
    "profile": "LD",
    "day": 7,
    "slot": "poranek",
    "category": "medrzec",
    "title": "Dziś jestem uczestnikiem",
    "body": "Mała tajemnica Mędrczyni: prawdziwy lider umie też nie prowadzić. Dziś spróbuj być uczestnikiem. Pozwól komuś innemu wybrać. Patrz, jak inni prowadzą.",
    "icon": "medrzec",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "nie-prowadzenie",
      "pokora"
    ],
    "audience": "dziecko",
    "subcategory": "granica_lidera",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D07-S2",
    "profile": "LD",
    "day": 7,
    "slot": "poludnie",
    "category": "misje",
    "title": "Posłuchaj, zanim odpowiesz",
    "body": "Misja słuchania: gdy ktoś dziś mówi, policz w głowie do trzech, zanim odpowiesz. Może usłyszysz coś, czego nie usłyszysz, kiedy spieszysz się ze zdaniem.",
    "icon": "pulse",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "słuchanie",
      "asertywność"
    ],
    "audience": "dziecko",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D07-S3",
    "profile": "LD",
    "day": 7,
    "slot": "wieczor",
    "category": "medrzec",
    "title": "Tydzień pierwszy — co czuję?",
    "body": "Minął tydzień z Mędrczynią. Połóż dłoń na piersi. Powiedz na głos: „jestem Gwardzistą Odwagi”. Posłuchaj, jak to brzmi. Twoja tarcza już lśni jaśniej.",
    "icon": "artifact",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "refleksja",
      "tożsamość"
    ],
    "audience": "rodzic",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D08-S1",
    "profile": "LD",
    "day": 8,
    "slot": "poranek",
    "category": "medrzec",
    "title": "Mała odwaga to też odwaga",
    "body": "Posłuchaj, mały Gwardzisto: nie musisz dziś robić niczego wielkiego. Powiedzieć „dzień dobry” pani, której się boisz — to też odwaga. Liczy się każdy krok.",
    "icon": "medrzec",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "odwaga",
      "drobne_kroki"
    ],
    "audience": "dziecko",
    "subcategory": "odwaga_mala",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D08-S2",
    "profile": "LD",
    "day": 8,
    "slot": "poludnie",
    "category": "misje",
    "title": "Zadaj pytanie na lekcji",
    "body": "Misja głosu: dziś na lekcji zadaj jedno pytanie. Nawet jeśli wydaje ci się głupie. Pewnie inni też się zastanawiają. Twój głos pomaga im się odezwać.",
    "icon": "pulse",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "głos",
      "szkoła"
    ],
    "audience": "dziecko",
    "subcategory": "mowie_co_inni_boja",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D08-S3",
    "profile": "LD",
    "day": 8,
    "slot": "wieczor",
    "category": "medrzec",
    "title": "Czego się dziś bałam/bałem?",
    "body": "Wieczorem nazwij to po imieniu: czego się dziś bałam/bałem? Strach nazwany staje się mniejszy. Tarcza Słońca lubi prawdę.",
    "icon": "medrzec",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "refleksja",
      "strach"
    ],
    "audience": "rodzic",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D09-S1",
    "profile": "LD",
    "day": 9,
    "slot": "poranek",
    "category": "mentor",
    "title": "Rodzicu: pokaż swoją odwagę",
    "body": "Drogi Rodzicu, Lider uczy się odwagi od ciebie. Opowiedz dziecku przy śniadaniu: „dziś trochę się boję czegoś, ale i tak zrobię”. To więcej warte niż wykład.",
    "icon": "rodzic",
    "tone": "magic",
    "time": "2 min",
    "tags": [
      "rodzic",
      "modelowanie"
    ],
    "audience": "rodzic",
    "subcategory": "odwaga_mala",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D09-S2",
    "profile": "LD",
    "day": 9,
    "slot": "poludnie",
    "category": "misje",
    "title": "Wstaw się za kimś",
    "body": "Misja tarczy: jeśli dziś usłyszysz, że ktoś mówi o kimś brzydko, powiedz spokojnie: „nie mówmy tak”. Krótko, bez krzyku. Tarcza Słońca chroni innych.",
    "icon": "artifact",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "obrona",
      "grupa"
    ],
    "audience": "dziecko",
    "subcategory": "inicjatywa_grupa",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D09-S3",
    "profile": "LD",
    "day": 9,
    "slot": "wieczor",
    "category": "kraina",
    "title": "Niebo Marzeń otwiera bramę",
    "body": "Spójrz dziś w niebo, zanim zaśniesz. Niebo Marzeń to twoja kraina. Tam idą wszyscy Gwardziści, gdy potrzebują się naładować. Pomyśl o jednym marzeniu i zostaw je w chmurach.",
    "icon": "artifact",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "kraina",
      "marzenia"
    ],
    "audience": "dziecko",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D10-S1",
    "profile": "LD",
    "day": 10,
    "slot": "poranek",
    "category": "medrzec",
    "title": "Woda przed grzywą",
    "body": "Lwia grzywa nie świeci bez wody. Wypij dziś szklankę wody zaraz po wstaniu. Potem jeszcze kilka w ciągu dnia. Tarcza Słońca działa lepiej, gdy ciało ma wodę.",
    "icon": "pulse",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "woda",
      "ciało"
    ],
    "audience": "dziecko",
    "subcategory": "regeneracja_lidera",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D10-S2",
    "profile": "LD",
    "day": 10,
    "slot": "poludnie",
    "category": "misje",
    "title": "Przyznaj się do błędu",
    "body": "Najtrudniejsza misja Lidera: powiedz „pomyliłem się” albo „przepraszam”. Z głową w górze, nie w dół. Liderzy, którzy potrafią przeprosić, są najsilniejsi.",
    "icon": "pulse",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "przeprosiny",
      "odpowiedzialność"
    ],
    "audience": "dziecko",
    "subcategory": "przeprosiny",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D10-S3",
    "profile": "LD",
    "day": 10,
    "slot": "wieczor",
    "category": "medrzec",
    "title": "Komu dziś pomogłam/pomogłem?",
    "body": "Wieczorne pytanie: czyją tarczą byłam/byłem dziś? Choćby raz, choćby na chwilę. Lider liczy nie krzyki, ale tarcze. Zaśnij z tą myślą.",
    "icon": "medrzec",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "refleksja",
      "pomoc"
    ],
    "audience": "rodzic",
    "subcategory": "pomoc_proaktywna",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D11-S1",
    "profile": "LD",
    "day": 11,
    "slot": "poranek",
    "category": "medrzec",
    "title": "Twoja grzywa, twoje tempo",
    "body": "Mały Gwardzisto, nie musisz prowadzić od rana. Czasem grzywa potrzebuje rozkręcić się powoli. Zacznij dziś od jednej cichej rzeczy, którą lubisz.",
    "icon": "medrzec",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "tempo",
      "spokój"
    ],
    "audience": "dziecko",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D11-S2",
    "profile": "LD",
    "day": 11,
    "slot": "poludnie",
    "category": "misje",
    "title": "Powiedz, czego się boisz",
    "body": "Odważna misja: powiedz dziś komuś bliskiemu „boję się tego”. Bez wstydu. Lider, który mówi o strachu, daje innym pozwolenie, żeby też się przyznali.",
    "icon": "pulse",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "strach",
      "głos"
    ],
    "audience": "dziecko",
    "subcategory": "mowie_co_inni_boja",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D11-S3",
    "profile": "LD",
    "day": 11,
    "slot": "wieczor",
    "category": "mentor",
    "title": "Rodzicu: zapytaj o trudne",
    "body": "Drogi Rodzicu, dziś wieczorem zapytaj: „co dziś było najtrudniejsze?”. Nie „jak w szkole?”. Pytanie konkretne otwiera Lidera. Słuchaj bez rad — najpierw słuchaj.",
    "icon": "rodzic",
    "tone": "magic",
    "time": "2 min",
    "tags": [
      "rodzic",
      "rozmowa"
    ],
    "audience": "rodzic",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D12-S1",
    "profile": "LD",
    "day": 12,
    "slot": "poranek",
    "category": "kraina",
    "title": "Sztandar twojego dnia",
    "body": "Wyobraź sobie, że masz dziś sztandar. Co na nim piszesz na ten dzień? Jedno słowo. Może „spokój”. Może „pomoc”. Może „uśmiech”. Nieś go cały dzień.",
    "icon": "artifact",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "kraina",
      "intencja"
    ],
    "audience": "dziecko",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D12-S2",
    "profile": "LD",
    "day": 12,
    "slot": "poludnie",
    "category": "misje",
    "title": "Zaproś kogoś do drużyny",
    "body": "Misja drużyny: znajdź dziś osobę, która stoi z boku. Powiedz: „chodź z nami”. Bez wielkiego przedstawiania. Lider robi miejsce, a nie chowa miejsce.",
    "icon": "pulse",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "grupa",
      "włączanie"
    ],
    "audience": "dziecko",
    "subcategory": "inicjatywa_grupa",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D12-S3",
    "profile": "LD",
    "day": 12,
    "slot": "wieczor",
    "category": "medrzec",
    "title": "Czy zostawiłam/zostawiłem miejsce?",
    "body": "Wieczorne pytanie Mędrczyni: czy dziś zostawiłam/zostawiłem komuś miejsce, żeby też był ważny? Lider, który nie zostawia miejsca, staje się małym tyranem. A ty nie jesteś tyranem.",
    "icon": "medrzec",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "refleksja",
      "pokora"
    ],
    "audience": "rodzic",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D13-S1",
    "profile": "LD",
    "day": 13,
    "slot": "poranek",
    "category": "medrzec",
    "title": "Jeden krok, nie dziesięć",
    "body": "Gwardzisto, dziś poranny sekret: nie planuj dziesięciu rzeczy. Wybierz jeden krok odwagi. Tylko jeden. Mały. Konkretny. Zrobisz go i basta.",
    "icon": "pulse",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "fokus",
      "drobne_kroki"
    ],
    "audience": "dziecko",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D13-S2",
    "profile": "LD",
    "day": 13,
    "slot": "poludnie",
    "category": "misje",
    "title": "Zapytaj rodzica o jego dzień",
    "body": "Odwrócona misja: zapytaj dziś rodzica: „jak ci było dzisiaj?”. I posłuchaj. Lider też dba o dorosłych. To dziwnie piękne, gdy dziecko pyta jako pierwsze.",
    "icon": "rodzic",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "troska",
      "rodzina"
    ],
    "audience": "dziecko",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D13-S3",
    "profile": "LD",
    "day": 13,
    "slot": "wieczor",
    "category": "medrzec",
    "title": "Co dziś dał mi strach?",
    "body": "Mała mądrość: strach nie tylko boli. Czasem on nas pilnuje, czasem uczy. Wieczorem zapytaj: czego mnie dziś nauczył mój strach?",
    "icon": "medrzec",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "refleksja",
      "strach"
    ],
    "audience": "rodzic",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D14-S1",
    "profile": "LD",
    "day": 14,
    "slot": "poranek",
    "category": "mentor",
    "title": "Rodzicu: nie naprawiaj zbyt szybko",
    "body": "Drogi Rodzicu, gdy Lider opowiada o konflikcie, nie biegnij od razu z rozwiązaniem. Zapytaj: „jak chcesz to rozegrać?”. Dziecko-Lider potrzebuje próbować, nie być prowadzonym.",
    "icon": "rodzic",
    "tone": "magic",
    "time": "2 min",
    "tags": [
      "rodzic",
      "autonomia"
    ],
    "audience": "rodzic",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D14-S2",
    "profile": "LD",
    "day": 14,
    "slot": "poludnie",
    "category": "misje",
    "title": "Pomysł, którego się boisz",
    "body": "Misja głosu: powiedz dziś na głos pomysł, którego trochę się wstydzisz. Może wymyśliłaś/wymyśliłeś coś dziwnego. Powiedz go jednej osobie. Pomysły rosną, gdy się je wypuszcza.",
    "icon": "pulse",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "kreatywność",
      "głos"
    ],
    "audience": "dziecko",
    "subcategory": "mowie_co_inni_boja",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D14-S3",
    "profile": "LD",
    "day": 14,
    "slot": "wieczor",
    "category": "medrzec",
    "title": "Dwa tygodnie tarczy",
    "body": "Mały Gwardzisto, jesteś już dwa tygodnie ze mną. Twoja Tarcza Słońca jest cieplejsza. Połóż dłoń na piersi. Czujesz? To nie ja zrobiłam — to ty.",
    "icon": "artifact",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "refleksja",
      "tożsamość"
    ],
    "audience": "rodzic",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D15-S1",
    "profile": "LD",
    "day": 15,
    "slot": "poranek",
    "category": "medrzec",
    "title": "Półmetek — przystań na chwilę",
    "body": "Połowa drogi za nami, Gwardzisto. Dziś rano nie biegnij. Usiądź na łóżku przez minutę. Posłuchaj oddechu. Lider, który umie przystanąć, idzie dalej.",
    "icon": "medrzec",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "odpoczynek_lidera",
      "pauza"
    ],
    "audience": "dziecko",
    "subcategory": "granica_lidera",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D15-S2",
    "profile": "LD",
    "day": 15,
    "slot": "poludnie",
    "category": "misje",
    "title": "Dziś prowadzi ktoś inny",
    "body": "Misja pokory: niech dziś ktoś inny wybierze grę, film albo trasę spaceru. Powiedz: „wybierz ty”. I idź. Lider, który umie iść za kimś, jest bardzo silny.",
    "icon": "pulse",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "nie-prowadzenie",
      "pokora"
    ],
    "audience": "dziecko",
    "subcategory": "granica_lidera",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D15-S3",
    "profile": "LD",
    "day": 15,
    "slot": "wieczor",
    "category": "medrzec",
    "title": "Co mnie dziś zaskoczyło?",
    "body": "Wieczorem zapytaj siebie: co dziś było inne, niż się spodziewałam/spodziewałem? Tarcza Słońca lubi zaskoczenia — wtedy uczy się najwięcej.",
    "icon": "medrzec",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "refleksja",
      "ciekawość"
    ],
    "audience": "rodzic",
    "subcategory": "odwaga_mala",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D16-S1",
    "profile": "LD",
    "day": 16,
    "slot": "poranek",
    "category": "medrzec",
    "title": "Krok w stronę, której się boisz",
    "body": "Posłuchaj, mały Gwardzisto: tam, gdzie się trochę boisz, ale wiesz, że jest dobrze, tam idź. Nie tam, gdzie jest niebezpiecznie. Tam, gdzie jest TRUDNO, ale DOBRZE.",
    "icon": "medrzec",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "odwaga",
      "intuicja"
    ],
    "audience": "dziecko",
    "subcategory": "odwaga_mala",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D16-S2",
    "profile": "LD",
    "day": 16,
    "slot": "poludnie",
    "category": "misje",
    "title": "Komplement w twarz",
    "body": "Misja ciepła: powiedz dziś komuś prosto w twarz coś dobrego. Konkretnie. „Podoba mi się, jak rysujesz”. „Pomogłaś mi wczoraj, pamiętam”. Lider rozdaje światło.",
    "icon": "pulse",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "docenianie",
      "ciepło"
    ],
    "audience": "dziecko",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D16-S3",
    "profile": "LD",
    "day": 16,
    "slot": "wieczor",
    "category": "mentor",
    "title": "Rodzicu: zapytaj kogo dziś wspierał",
    "body": "Drogi Rodzicu, dziś wieczorem zamiast „jakie oceny”, zapytaj: „komu dziś pomogłaś, kogo wsparłeś?”. To pytanie kieruje Lidera na to, co naprawdę ważne.",
    "icon": "rodzic",
    "tone": "magic",
    "time": "2 min",
    "tags": [
      "rodzic",
      "rozmowa"
    ],
    "audience": "rodzic",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D17-S1",
    "profile": "LD",
    "day": 17,
    "slot": "poranek",
    "category": "kraina",
    "title": "Ognisko w piersi",
    "body": "W Niebie Marzeń pali się ognisko Gwardzistów. Wyobraź sobie, że masz takie małe ognisko w piersi. Grzeje cię i grzeje innych. Niesiesz je dziś przez dzień.",
    "icon": "artifact",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "kraina",
      "ciepło"
    ],
    "audience": "dziecko",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D17-S2",
    "profile": "LD",
    "day": 17,
    "slot": "poludnie",
    "category": "misje",
    "title": "Powiedz „nie wiem”",
    "body": "Trudna misja Lidera: kiedy ktoś dziś zapyta cię o coś, czego nie wiesz, powiedz: „nie wiem, ale sprawdzę”. Liderzy, którzy mówią „nie wiem”, są wiarygodni.",
    "icon": "pulse",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "pokora",
      "uczciwość"
    ],
    "audience": "dziecko",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D17-S3",
    "profile": "LD",
    "day": 17,
    "slot": "wieczor",
    "category": "medrzec",
    "title": "Mała pycha, ważna lekcja",
    "body": "Wieczorem nie udawaj. Czy dziś było coś, gdzie chciałam/chciałem być ważniejszy niż trzeba? To nie wstyd. To zaproszenie. Zauważam — i jutro inaczej.",
    "icon": "medrzec",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "refleksja",
      "pokora"
    ],
    "audience": "rodzic",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D18-S1",
    "profile": "LD",
    "day": 18,
    "slot": "poranek",
    "category": "medrzec",
    "title": "Dziś idę bez krzyku",
    "body": "Mały Gwardzisto, dziś spróbuj: cokolwiek się stanie, najpierw oddech, potem słowa. Lwia grzywa nie musi ryczeć. Może mówić cicho i też być słyszana.",
    "icon": "medrzec",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "spokój",
      "asertywność"
    ],
    "audience": "dziecko",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D18-S2",
    "profile": "LD",
    "day": 18,
    "slot": "poludnie",
    "category": "misje",
    "title": "Stań po swojej stronie",
    "body": "Misja granicy: jeśli ktoś dziś zacznie cię popychać do czegoś, co czujesz, że nie jest twoje — stań spokojnie i powiedz „nie, dziękuję”. To twoja tarcza.",
    "icon": "artifact",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "granice",
      "presja"
    ],
    "audience": "dziecko",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D18-S3",
    "profile": "LD",
    "day": 18,
    "slot": "wieczor",
    "category": "medrzec",
    "title": "Gdzie zabrakło mi odwagi?",
    "body": "Wieczorne uczciwe pytanie: czy było dziś coś, gdzie wolałam/wolałem milczeć, a w sercu wiedziałam/wiedziałem, że trzeba powiedzieć? Bez bicia się. Po prostu zobaczę.",
    "icon": "medrzec",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "refleksja",
      "odwaga"
    ],
    "audience": "rodzic",
    "subcategory": "odwaga_mala",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D19-S1",
    "profile": "LD",
    "day": 19,
    "slot": "poranek",
    "category": "mentor",
    "title": "Rodzicu: powierz mu odpowiedzialność",
    "body": "Drogi Rodzicu, daj dziś Liderowi jedno małe zadanie tylko dla niego: nakryć stół, przypilnować młodszego, kupić chleb. Odpowiedzialność rozwija tarczę bardziej niż słowa.",
    "icon": "rodzic",
    "tone": "magic",
    "time": "2 min",
    "tags": [
      "rodzic",
      "odpowiedzialność"
    ],
    "audience": "rodzic",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D19-S2",
    "profile": "LD",
    "day": 19,
    "slot": "poludnie",
    "category": "misje",
    "title": "Pierwsza pomoc na podwórku",
    "body": "Misja Gwardzisty: jeśli ktoś dziś się przewróci, zapłacze, zgubi coś — bądź pierwszą/pierwszym, która/który podejdzie. Nie z fanfarami. Po cichu. „Pomóc ci?”.",
    "icon": "pulse",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "pomoc",
      "troska"
    ],
    "audience": "dziecko",
    "subcategory": "pomoc_proaktywna",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D19-S3",
    "profile": "LD",
    "day": 19,
    "slot": "wieczor",
    "category": "kraina",
    "title": "Mędrczyni patrzy z chmury",
    "body": "Wyobraź sobie, że Mędrczyni patrzy z chmury i widzi twój dzień. Co mówi? Nie krzyczy. Mówi: „widzę cię, mała Gwardzistko, mały Gwardzisto. Jesteś w porządku”.",
    "icon": "medrzec",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "kraina",
      "akceptacja"
    ],
    "audience": "dziecko",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D20-S1",
    "profile": "LD",
    "day": 20,
    "slot": "poranek",
    "category": "medrzec",
    "title": "Zbroja z gliny, nie z żelaza",
    "body": "Posłuchaj, mały Gwardzisto: twoja zbroja jest z miękkiej gliny, nie z twardego żelaza. To dlatego, że odwaga ma być ciepła, a nie zimna. Można w niej oddychać.",
    "icon": "artifact",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "czułość",
      "tożsamość"
    ],
    "audience": "dziecko",
    "subcategory": "odwaga_mala",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D20-S2",
    "profile": "LD",
    "day": 20,
    "slot": "poludnie",
    "category": "misje",
    "title": "Spróbuj czegoś, w czym jesteś słaby",
    "body": "Misja odwagi: dziś zrób krótko coś, w czym ci nie wychodzi. Rysuj, śpiewaj, biegaj, licz. Krzywo? Świetnie. Lider, który próbuje, uczy innych próbować.",
    "icon": "pulse",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "próba",
      "porażka"
    ],
    "audience": "dziecko",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D20-S3",
    "profile": "LD",
    "day": 20,
    "slot": "wieczor",
    "category": "medrzec",
    "title": "Co wyszło krzywo, a było dobre?",
    "body": "Wieczorne pytanie: co dziś wyszło krzywo, a i tak było dobre, że spróbowałaś/spróbowałeś? Krzywe próby budują tarczę szybciej niż gładkie sukcesy.",
    "icon": "medrzec",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "refleksja",
      "porażka"
    ],
    "audience": "rodzic",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D21-S1",
    "profile": "LD",
    "day": 21,
    "slot": "poranek",
    "category": "medrzec",
    "title": "Trzy tygodnie — gdzie urosłaś/urosłeś?",
    "body": "Mały Gwardzisto, trzy tygodnie ze mną. Zatrzymaj się rano i zauważ jedną rzecz, w której jesteś inaczej niż na początku. Może bardziej spokojnie. Może bardziej śmiało.",
    "icon": "pulse",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "refleksja",
      "wzrost"
    ],
    "audience": "dziecko",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D21-S2",
    "profile": "LD",
    "day": 21,
    "slot": "poludnie",
    "category": "misje",
    "title": "Zaufaj komuś dziś",
    "body": "Misja zaufania: dziś poproś kogoś o pomoc w czymś, co normalnie chciałabyś/chciałbyś zrobić sama/sam. Lider, który prosi o pomoc, daje innym poczuć, że są ważni.",
    "icon": "pulse",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "zaufanie",
      "prośba"
    ],
    "audience": "dziecko",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D21-S3",
    "profile": "LD",
    "day": 21,
    "slot": "wieczor",
    "category": "mentor",
    "title": "Rodzicu: zauważ jego/jej spokój",
    "body": "Drogi Rodzicu, dzisiaj wieczorem powiedz konkretnie: „widzę, że ostatnio mniej krzyczysz”. Albo: „zauważyłam, że pomagasz młodszemu”. Konkret jak woda na korzeń.",
    "icon": "rodzic",
    "tone": "magic",
    "time": "2 min",
    "tags": [
      "rodzic",
      "docenianie"
    ],
    "audience": "rodzic",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D22-S1",
    "profile": "LD",
    "day": 22,
    "slot": "poranek",
    "category": "medrzec",
    "title": "Lider odpoczywa jak lew",
    "body": "Posłuchaj, Gwardzisto: lwy odpoczywają wiele godzin dziennie. Tylko wtedy są silne. Dziś zaplanuj jedną rzecz, którą zrobisz powoli, dla siebie. Bez celu. Po prostu.",
    "icon": "medrzec",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "odpoczynek_lidera",
      "spokój"
    ],
    "audience": "dziecko",
    "subcategory": "granica_lidera",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D22-S2",
    "profile": "LD",
    "day": 22,
    "slot": "poludnie",
    "category": "misje",
    "title": "Powiedz „ja tak czuję”",
    "body": "Misja głosu: zamiast „ty zawsze...” spróbuj dziś powiedzieć „ja tak czuję, gdy...”. To dwa zupełnie inne zdania. Pierwsze obwinia. Drugie otwiera. Tarcza Słońca lubi drugie.",
    "icon": "pulse",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "komunikacja",
      "asertywność"
    ],
    "audience": "dziecko",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D22-S3",
    "profile": "LD",
    "day": 22,
    "slot": "wieczor",
    "category": "medrzec",
    "title": "Jak inni odebrali mój głos?",
    "body": "Wieczorne pytanie: gdy dziś coś powiedziałam/powiedziałem stanowczo, jak inni zareagowali? Nie chodzi o to, czy się zgodzili. Chodzi o to, jak się czuli. Lider patrzy także na to.",
    "icon": "medrzec",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "refleksja",
      "empatia"
    ],
    "audience": "rodzic",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D23-S1",
    "profile": "LD",
    "day": 23,
    "slot": "poranek",
    "category": "mentor",
    "title": "Rodzicu: nie nazywaj jej/jego „dominującą””",
    "body": "Drogi Rodzicu, słowa lepią dzieci. Zamiast „uparta/uparty”, powiedz: „masz mocne zdanie”. Zamiast „dominująca”, powiedz: „wiesz, czego chcesz”. Lider słyszy etykiety długo.",
    "icon": "rodzic",
    "tone": "magic",
    "time": "2 min",
    "tags": [
      "rodzic",
      "język"
    ],
    "audience": "rodzic",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D23-S2",
    "profile": "LD",
    "day": 23,
    "slot": "poludnie",
    "category": "misje",
    "title": "Naprawić, nie pokonać",
    "body": "Misja sporu: jeśli pokłócisz się dziś z kimś, zamiast „wygrać”, spróbuj „naprawić”. Powiedz: „chcę, żebyśmy się dogadali”. Tarcza Słońca nie służy do bicia.",
    "icon": "artifact",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "konflikt",
      "pojednanie"
    ],
    "audience": "dziecko",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D23-S3",
    "profile": "LD",
    "day": 23,
    "slot": "wieczor",
    "category": "medrzec",
    "title": "Z kim mam coś do naprawienia?",
    "body": "Wieczorem pomyśl spokojnie: czy jest ktoś, z kim mam dziś coś do uładzenia? Może wystarczy jedno zdanie jutro. Mędrczyni przypomni.",
    "icon": "medrzec",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "refleksja",
      "pojednanie"
    ],
    "audience": "rodzic",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D24-S1",
    "profile": "LD",
    "day": 24,
    "slot": "poranek",
    "category": "kraina",
    "title": "Drużyna Gwardzistów wokół ciebie",
    "body": "W Niebie Marzeń jest cała drużyna takich jak ty. Wyobraź sobie, że stoją wokół ciebie dzisiaj rano. Nie jesteś sam, nie jesteś sama. Idziesz w drużynie, nawet kiedy idziesz sam.",
    "icon": "artifact",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "kraina",
      "wspólnota"
    ],
    "audience": "dziecko",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D24-S2",
    "profile": "LD",
    "day": 24,
    "slot": "poludnie",
    "category": "misje",
    "title": "Zacznij rozmowę z dorosłym",
    "body": "Odważna misja: zagadaj dziś dorosłego, którego trochę się boisz albo z którym mało rozmawiasz. Babcię, sąsiadkę, nauczyciela. Krótko. „Jak się ma pani dzisiaj?”.",
    "icon": "pulse",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "odwaga",
      "rozmowa"
    ],
    "audience": "dziecko",
    "subcategory": "odwaga_mala",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D24-S3",
    "profile": "LD",
    "day": 24,
    "slot": "wieczor",
    "category": "medrzec",
    "title": "Tarcza po dniu — schowaj ją",
    "body": "Wieczorem powiedz na głos: „dzisiaj odkładam tarczę”. Połóż dłoń na piersi, jakbyś ją zsuwała/zsuwał. Lider, który zasypia, nie musi pilnować świata. Świat poczeka do rana.",
    "icon": "artifact",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "sen",
      "odpoczynek_lidera"
    ],
    "audience": "rodzic",
    "subcategory": "regeneracja_lidera",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D25-S1",
    "profile": "LD",
    "day": 25,
    "slot": "poranek",
    "category": "medrzec",
    "title": "Twoje serce — twój kompas",
    "body": "Posłuchaj, Gwardzisto: gdy nie wiesz, jak postąpić, połóż dłoń na piersi i zapytaj: „co jest dobre?”. Nie „co jest łatwe”, nie „co inni powiedzą”. Co jest DOBRE. Tam jest twój kompas.",
    "icon": "medrzec",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "intuicja",
      "wartości"
    ],
    "audience": "dziecko",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D25-S2",
    "profile": "LD",
    "day": 25,
    "slot": "poludnie",
    "category": "misje",
    "title": "Bądź pierwszy w dobrej sprawie",
    "body": "Misja dnia: zrób dziś jedną rzecz pierwsza/pierwszy, której nikt nie zauważy. Pozbierać śmieć, wytrzeć stół, ustąpić miejsca. Cicho. Lider, którego nie widać, też świeci.",
    "icon": "pulse",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "inicjatywa",
      "ciche_dobro"
    ],
    "audience": "dziecko",
    "subcategory": "inicjatywa_grupa",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D25-S3",
    "profile": "LD",
    "day": 25,
    "slot": "wieczor",
    "category": "mentor",
    "title": "Rodzicu: nie obciążaj jej/go całą rodziną",
    "body": "Drogi Rodzicu, uwaga: Lider łatwo bierze na siebie problemy całej rodziny. Powiedz dziś: „to nie jest twoja sprawa, ja sobie z tym poradzę”. Dziecko-Lider potrzebuje to słyszeć.",
    "icon": "rodzic",
    "tone": "magic",
    "time": "2 min",
    "tags": [
      "rodzic",
      "parentyfikacja"
    ],
    "audience": "rodzic",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D26-S1",
    "profile": "LD",
    "day": 26,
    "slot": "poranek",
    "category": "medrzec",
    "title": "Nie musisz dziś wygrać",
    "body": "Mały Gwardzisto, dzisiejszy poranny szept Mędrczyni: nie musisz dziś niczego wygrać. Nie musisz być najlepsza/najlepszy. Wystarczy, że będziesz sobą. Tyle.",
    "icon": "medrzec",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "spokój",
      "wartość"
    ],
    "audience": "dziecko",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D26-S2",
    "profile": "LD",
    "day": 26,
    "slot": "poludnie",
    "category": "misje",
    "title": "Stań w drugim rzędzie",
    "body": "Misja pokory: dziś w grze, w klasie, w grupie — stań specjalnie w drugim rzędzie. Pozwól komuś być z przodu. Patrz, jak to jest. Co czujesz? Lider widzi też z tyłu.",
    "icon": "pulse",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "nie-prowadzenie",
      "perspektywa"
    ],
    "audience": "dziecko",
    "subcategory": "granica_lidera",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D26-S3",
    "profile": "LD",
    "day": 26,
    "slot": "wieczor",
    "category": "medrzec",
    "title": "Co widziałam/widziałem z drugiego rzędu?",
    "body": "Wieczorne pytanie: jak to było stać z tyłu? Co zobaczyłam/zobaczyłem nowego? Czasem z drugiego rzędu widać więcej niż z pierwszego.",
    "icon": "medrzec",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "refleksja",
      "perspektywa"
    ],
    "audience": "rodzic",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D27-S1",
    "profile": "LD",
    "day": 27,
    "slot": "poranek",
    "category": "kraina",
    "title": "Słońce nad Niebem Marzeń",
    "body": "Spójrz dziś rano w okno. Gdziekolwiek jesteś — słońce nad Niebem Marzeń wschodzi razem ze słońcem u ciebie. To samo słońce świeci wszystkim Gwardzistom na świecie.",
    "icon": "artifact",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "kraina",
      "wspólnota"
    ],
    "audience": "dziecko",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D27-S2",
    "profile": "LD",
    "day": 27,
    "slot": "poludnie",
    "category": "misje",
    "title": "Dziękuję komuś, kto pomaga niewidocznie",
    "body": "Misja wdzięczności: znajdź dziś kogoś, kto codziennie robi coś dla ciebie, a nigdy mu nie podziękowałaś/podziękowałeś. Pani sprzątająca, kierowca, babcia. Powiedz krótkie „dziękuję”.",
    "icon": "pulse",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "wdzięczność",
      "ciche_dobro"
    ],
    "audience": "dziecko",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D27-S3",
    "profile": "LD",
    "day": 27,
    "slot": "wieczor",
    "category": "mentor",
    "title": "Rodzicu: zapytaj o jej/jego marzenie",
    "body": "Drogi Rodzicu, dzisiaj wieczorem nie pytaj o lekcje. Zapytaj: „o czym ostatnio marzysz?”. Słuchaj bez prostowania, bez „bądź realistka”. Niebo Marzeń lubi takie pytania.",
    "icon": "rodzic",
    "tone": "magic",
    "time": "2 min",
    "tags": [
      "rodzic",
      "marzenia"
    ],
    "audience": "rodzic",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D28-S1",
    "profile": "LD",
    "day": 28,
    "slot": "poranek",
    "category": "medrzec",
    "title": "Twoja odwaga nie musi krzyczeć",
    "body": "Posłuchaj, mały Gwardzisto: czasem najodważniejszą rzeczą jest milczeć, gdy inni krzyczą. Czasem powiedzieć „nie wiem”. Czasem wrócić do kogoś i przeprosić. Odwaga ma wiele twarzy.",
    "icon": "medrzec",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "odwaga",
      "różne_oblicza"
    ],
    "audience": "dziecko",
    "subcategory": "odwaga_mala",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D28-S2",
    "profile": "LD",
    "day": 28,
    "slot": "poludnie",
    "category": "misje",
    "title": "Pomóż komuś bez pochwały",
    "body": "Sekretna misja: zrób dziś coś dobrego dla kogoś tak, żeby się nie dowiedział, że to ty. Zostaw, schowaj, naprawi, niech samo wyjdzie na dobre. To liderstwo bez aplauzu.",
    "icon": "pulse",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "ciche_dobro",
      "pomoc"
    ],
    "audience": "dziecko",
    "subcategory": "pomoc_proaktywna",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D28-S3",
    "profile": "LD",
    "day": 28,
    "slot": "wieczor",
    "category": "medrzec",
    "title": "Czy ktoś inny musi wiedzieć?",
    "body": "Wieczorne pytanie: czy potrafię zrobić dobro i nie powiedzieć o tym? Tarcza Słońca świeci najjaśniej, gdy nikt nie patrzy. Mędrczyni i tak widzi.",
    "icon": "medrzec",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "refleksja",
      "pokora"
    ],
    "audience": "rodzic",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D29-S1",
    "profile": "LD",
    "day": 29,
    "slot": "poranek",
    "category": "mentor",
    "title": "Rodzicu: planuj z nią/nim, nie za nią/nim",
    "body": "Drogi Rodzicu, jesteście prawie na końcu. Spróbuj dziś zaplanować dzień RAZEM z Liderem. „Co dziś robimy? Kiedy? Jak?”. Współplanowanie ćwiczy odpowiedzialność lepiej niż rozkazywanie.",
    "icon": "rodzic",
    "tone": "magic",
    "time": "2 min",
    "tags": [
      "rodzic",
      "współpraca"
    ],
    "audience": "rodzic",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D29-S2",
    "profile": "LD",
    "day": 29,
    "slot": "poludnie",
    "category": "misje",
    "title": "Spójrz na siebie z czułością",
    "body": "Niezwykła misja: stań dziś przed lustrem i powiedz do siebie: „dasz radę”. Bez śmiechu. Bez wstydu. Twój głos do siebie — tarcza najmocniejsza, jaka istnieje.",
    "icon": "artifact",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "samoakceptacja",
      "tarcza"
    ],
    "audience": "dziecko",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D29-S3",
    "profile": "LD",
    "day": 29,
    "slot": "wieczor",
    "category": "medrzec",
    "title": "Przedostatnia noc — co zostawiam?",
    "body": "Wieczorem pomyśl: z tych 29 dni, co chcę zatrzymać na dłużej? Jedno zachowanie, jedno zdanie, jedna myśl. Wybierz świadomie. To pójdzie z tobą dalej.",
    "icon": "medrzec",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "refleksja",
      "intencja"
    ],
    "audience": "rodzic",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D30-S1",
    "profile": "LD",
    "day": 30,
    "slot": "poranek",
    "category": "medrzec",
    "title": "Trzydziesty wschód słońca",
    "body": "Mały Gwardzisto, ostatni wschód razem. Połóż obie dłonie na piersi. Powiedz cicho: „mam tarczę, mam grzywę, mam serce”. Ja, Mędrczyni, kłaniam ci się.",
    "icon": "medrzec",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "zakończenie",
      "tożsamość"
    ],
    "audience": "dziecko",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D30-S2",
    "profile": "LD",
    "day": 30,
    "slot": "poludnie",
    "category": "misje",
    "title": "Powiedz komuś, kim się stałaś/stałeś",
    "body": "Ostatnia wielka misja: powiedz komu bliskiemu jedno zdanie: „uczę się być odważna/odważny z troski, nie z pychy”. Może nie zrozumieją od razu. Ale ty będziesz wiedzieć.",
    "icon": "pulse",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "tożsamość",
      "głos"
    ],
    "audience": "dziecko",
    "subcategory": "mowie_co_inni_boja",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "LD-D30-S3",
    "profile": "LD",
    "day": 30,
    "slot": "wieczor",
    "category": "kraina",
    "title": "Niebo Marzeń zawsze otwarte",
    "body": "Drogi/Droga Gwardzisto, kończymy 30 dni. Pamiętaj: Niebo Marzeń jest twoje na zawsze. Tarcza Słońca nigdy nie znika. Idziesz pierwsza, idziesz pierwszy — nie z pychy, z troski. Tak trzymaj.",
    "icon": "artifact",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "zakończenie",
      "kraina"
    ],
    "audience": "dziecko",
    "subcategory": "asertywnosc",
    "profileName": "Gwardzista Odwagi",
    "cecha": "Odwaga",
    "kraina": "Niebo Marzeń"
  },
  {
    "id": "MD-D01-S1",
    "profile": "MD",
    "day": 1,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "spokoj_dystans",
    "audience": "dziecko",
    "title": "Trzy spokojne oddechy na start",
    "body": "Zanim wstaniesz z łóżka, weź trzy spokojne oddechy. Pomyśl: „Jak ja się dzisiaj czuję?„. To Twój pierwszy most — do siebie.",
    "icon": "medrzec",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "oddech",
      "spokoj",
      "ja"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D01-S2",
    "profile": "MD",
    "day": 1,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "sluchanie_obu_stron",
    "audience": "dziecko",
    "title": "Misja: posłuchaj kogoś do końca",
    "body": "Wybierz jedną osobę i posłuchaj jej, nie przerywając, aż skończy mówić. Mędrczyni mówi: czasem ucho jest mostem.",
    "icon": "mentor",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "sluchanie",
      "most"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D01-S3",
    "profile": "MD",
    "day": 1,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "cudza_emocja_a_moja",
    "audience": "rodzic",
    "title": "Wieczorne pytanie: co tu jest moje?",
    "body": "Dzieci-mediatorzy chłoną emocje innych. Zapytajcie razem łagodnie: „Co dzisiaj było Twoje, a co cudze?„. To rozdziela, nie ocenia.",
    "icon": "rodzic",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "emocje",
      "granica"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D02-S1",
    "profile": "MD",
    "day": 2,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "perspektywa",
    "audience": "dziecko",
    "title": "Dwie strony tej samej rzeki",
    "body": "Pomyśl o czymś, co Cię dzisiaj czeka. Spójrz na to z jednej strony, potem z drugiej. Mędrczyni mówi: most widzi obie brzegi.",
    "icon": "medrzec",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "perspektywa",
      "most"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D02-S2",
    "profile": "MD",
    "day": 2,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "slowa_lacznikow",
    "audience": "dziecko",
    "title": "Misja: powiedz „rozumiem”",
    "body": "Gdy ktoś dziś coś Ci opowie, spróbuj zacząć odpowiedź od słowa „rozumiem”. To krótkie słowo jest jak mała kładka.",
    "icon": "mentor",
    "tone": "paper",
    "time": "1 min",
    "tags": [
      "slowa",
      "lacznik"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D02-S3",
    "profile": "MD",
    "day": 2,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "rytmy_dnia",
    "audience": "rodzic",
    "title": "Sen mediatora — 9 do 13 godzin",
    "body": "Dzieci, które dużo słuchają innych, potrzebują głębokiego odpoczynku. Spokojny wieczór, ciemny pokój, stała pora — to odbudowuje wstęgę.",
    "icon": "rodzic",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "sen",
      "rytm"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D03-S1",
    "profile": "MD",
    "day": 3,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "granica_mediatora",
    "audience": "dziecko",
    "title": "Nie musisz dziś łączyć wszystkich",
    "body": "Mędrczyni szepcze: „Jesteś mostem, ale most też odpoczywa”. Dziś możesz tylko być, nie godzić nikogo. To też jest mądrość.",
    "icon": "medrzec",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "granica",
      "odpoczynek"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D03-S2",
    "profile": "MD",
    "day": 3,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "konflikt_rowiesniczy",
    "audience": "dziecko",
    "title": "Misja: jedno pytanie zamiast oceny",
    "body": "Gdy kolega lub koleżanka się pokłócą, zamiast mówić „kto ma rację”, zapytaj: „Co się stało?”. Pytanie otwiera, ocena zamyka.",
    "icon": "mentor",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "konflikt",
      "pytanie"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D03-S3",
    "profile": "MD",
    "day": 3,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "granica_mediatora",
    "audience": "rodzic",
    "title": "Dziecko to nie mediator dorosłych",
    "body": "Jeśli w domu są spięcia między dorosłymi, powiedzcie wprost: „To praca dorosłych, nie Twoja”. Dziecko nie ma nieść tego ciężaru.",
    "icon": "rodzic",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "granica",
      "ochrona"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D04-S1",
    "profile": "MD",
    "day": 4,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "cudza_emocja_a_moja",
    "audience": "dziecko",
    "title": "Jaki dzisiaj jest mój kolor?",
    "body": "Zanim spotkasz innych ludzi, zauważ swój własny nastrój. Mędrczyni mówi: poznaj swój kolor, zanim zaczniesz mieszać go z innymi.",
    "icon": "medrzec",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "emocje",
      "ja"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D04-S2",
    "profile": "MD",
    "day": 4,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "kompromis_negocjacja",
    "audience": "dziecko",
    "title": "Misja: spotkanie w połowie drogi",
    "body": "Gdy nie zgadzacie się o coś małego (zabawkę, miejsce, kolejność), zaproponuj: „A może po połowie?”. Mały kompromis to początek dużego mostu.",
    "icon": "mentor",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "kompromis",
      "polowa"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D04-S3",
    "profile": "MD",
    "day": 4,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "slowa_lacznikow",
    "audience": "rodzic",
    "title": "Słowa, które budują kładkę",
    "body": "„Słyszę Cię”, „rozumiem, że to trudne”, „a co Ty czujesz?”. Te zdania nie kosztują nic, a dziecku-mediatorowi pokazują, jak się to robi.",
    "icon": "rodzic",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "slowa",
      "modelowanie"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D05-S1",
    "profile": "MD",
    "day": 5,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "sluchanie_obu_stron",
    "audience": "dziecko",
    "title": "Dwa uszy, jedne usta",
    "body": "Mędrczyni przypomina: masz dwa uszy i jedne usta nie przez przypadek. Dzisiaj posłuchaj dwa razy więcej, niż mówisz.",
    "icon": "medrzec",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "sluchanie",
      "uwaga"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D05-S2",
    "profile": "MD",
    "day": 5,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "perspektywa",
    "audience": "dziecko",
    "title": "Misja: opowiedz cudzą wersję",
    "body": "Po małej kłótni spróbuj powiedzieć: „Czyli Ty myślałeś, że…”. To pokazuje, że naprawdę słuchałeś. Często to wystarczy, by się rozluźnić.",
    "icon": "mentor",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "perspektywa",
      "powtorzenie"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D05-S3",
    "profile": "MD",
    "day": 5,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "rytmy_dnia",
    "audience": "rodzic",
    "title": "Woda, ruch, regularne posiłki",
    "body": "Mediator daje innym dużo uwagi. Pilnujcie podstaw: woda przy ręce, 60 minut ruchu, jedzenie o stałych porach. Ciało pierwsze, potem słuchanie.",
    "icon": "rodzic",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "cialo",
      "rytm",
      "woda"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D06-S1",
    "profile": "MD",
    "day": 6,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "spokoj_dystans",
    "audience": "dziecko",
    "title": "Klepsydra spokoju",
    "body": "Wyobraź sobie, że masz w sobie małą klepsydrę. Wdech — piasek leci w dół, wydech — w górę. Spokojnie, bez pośpiechu.",
    "icon": "medrzec",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "oddech",
      "klepsydra"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D06-S2",
    "profile": "MD",
    "day": 6,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "slowa_lacznikow",
    "audience": "dziecko",
    "title": "Misja: pytanie „a Ty co czujesz?”",
    "body": "Wybierz jedną osobę i zadaj jej dziś to pytanie. Nie poprawiaj jej odpowiedzi, po prostu posłuchaj. To otwiera serce po drugiej stronie.",
    "icon": "mentor",
    "tone": "paper",
    "time": "1 min",
    "tags": [
      "pytanie",
      "uczucia"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D06-S3",
    "profile": "MD",
    "day": 6,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "cudza_emocja_a_moja",
    "audience": "rodzic",
    "title": "Krótka rozmowa „oddawania” cudzych emocji",
    "body": "Wieczorem zapytajcie: „Czyje smutki nosiłeś dziś w plecaku?”. Nazwijcie je, „odłóżcie” symbolicznie na półkę. Dziecku robi się lżej.",
    "icon": "rodzic",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "emocje",
      "rytual"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D07-S1",
    "profile": "MD",
    "day": 7,
    "slot": "poranek",
    "category": "kraina",
    "subcategory": "spokoj_dystans",
    "audience": "dziecko",
    "title": "Zamek Czasu wita Cię cicho",
    "body": "W Zamku Czasu poranek jest najcichszą porą. Mędrczyni mówi: zanim świat zacznie hałasować, posłuchaj swojego serca. Co bije w nim dzisiaj?",
    "icon": "krysztal",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "zamek",
      "cisza"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D07-S2",
    "profile": "MD",
    "day": 7,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "nie-mediowanie",
    "audience": "dziecko",
    "title": "Misja: dziś nic nie godzę",
    "body": "Jeśli wokół Ciebie zaiskrzy, możesz odejść kawałek dalej. Mędrczyni mówi: czasem największą mądrością jest „dziś nie moja sprawa”.",
    "icon": "mentor",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "granica",
      "odpoczynek"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D07-S3",
    "profile": "MD",
    "day": 7,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "granica_mediatora",
    "audience": "rodzic",
    "title": "Pochwała za nie-mediowanie",
    "body": "Pochwalcie dziecko nie tylko za to, że kogoś pogodziło, ale też za to, że odpuściło. „Dobrze, że zadbałeś o siebie” — to ważna lekcja.",
    "icon": "rodzic",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "pochwala",
      "granica"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D08-S1",
    "profile": "MD",
    "day": 8,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "perspektywa",
    "audience": "dziecko",
    "title": "Klucz, który otwiera dwoje drzwi",
    "body": "Wyobraź sobie klucz z dwoma końcami. Twoja Wstęga Łączeń jest taka — potrafi otworzyć drzwi do Twojej myśli i do cudzej. Cudowne, prawda?",
    "icon": "artifact",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "klucz",
      "wstega"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D08-S2",
    "profile": "MD",
    "day": 8,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "konflikt_rowiesniczy",
    "audience": "dziecko",
    "title": "Misja: trzy kroki w sporze",
    "body": "Gdy dwoje się kłóci: 1) niech każdy powie jedno zdanie, 2) powtórz, co usłyszałeś, 3) zapytaj, co dalej. Krótko, spokojnie.",
    "icon": "mentor",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "konflikt",
      "kroki"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D08-S3",
    "profile": "MD",
    "day": 8,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "rytmy_dnia",
    "audience": "rodzic",
    "title": "Wieczór bez ekranu na 30 minut",
    "body": "Dziecko, które cały dzień słuchało innych, wieczorem potrzebuje ciszy. Książka, kąpiel, rozmowa — zamiast bodźców z ekranu. Mózg wycisza wstęgę.",
    "icon": "rodzic",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "ekran",
      "wieczor"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D09-S1",
    "profile": "MD",
    "day": 9,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "sluchanie_obu_stron",
    "audience": "dziecko",
    "title": "Każdy ma rację po swojemu",
    "body": "Mędrczyni mówi: dwie osoby mogą widzieć tę samą rzecz inaczej i obie mieć trochę racji. Dzisiaj pamiętaj o tym, gdy kogoś posłuchasz.",
    "icon": "medrzec",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "racja",
      "perspektywa"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D09-S2",
    "profile": "MD",
    "day": 9,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "kompromis_negocjacja",
    "audience": "dziecko",
    "title": "Misja: trzecia propozycja",
    "body": "Gdy Wy chcecie różnych rzeczy, wymyśl trzecią — taką, której nikt jeszcze nie powiedział. Często ta trzecia jest najlepsza.",
    "icon": "mentor",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "kompromis",
      "kreatywnosc"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D09-S3",
    "profile": "MD",
    "day": 9,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "slowa_lacznikow",
    "audience": "rodzic",
    "title": "Walidacja zamiast naprawiania",
    "body": "Gdy dziecko opowiada o kłótni, najpierw: „rozumiem, że było Ci ciężko”. Dopiero potem ewentualne pytania. Walidacja to najprostszy most.",
    "icon": "rodzic",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "walidacja",
      "sluchanie"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D10-S1",
    "profile": "MD",
    "day": 10,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "granica_mediatora",
    "audience": "dziecko",
    "title": "Twoje uczucia są na początku listy",
    "body": "Mędrczyni szepcze: zanim zatroszczysz się o cudze uczucia, sprawdź swoje. Pomyśl: „Ja dziś jestem…” i dokończ jednym słowem.",
    "icon": "medrzec",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "granica",
      "uczucia"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D10-S2",
    "profile": "MD",
    "day": 10,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "spokoj_dystans",
    "audience": "dziecko",
    "title": "Misja: trzy oddechy przed odpowiedzią",
    "body": "Gdy ktoś dziś powie coś, co Cię ruszy, najpierw weź trzy spokojne oddechy. Potem odpowiedz. Te trzy sekundy często zmieniają wszystko.",
    "icon": "mentor",
    "tone": "paper",
    "time": "1 min",
    "tags": [
      "oddech",
      "pauza"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D10-S3",
    "profile": "MD",
    "day": 10,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "cudza_emocja_a_moja",
    "audience": "rodzic",
    "title": "„Czyje to było uczucie?”",
    "body": "Jeśli dziecko wraca z dnia smutne bez powodu, zapytajcie łagodnie: „Czy to Twój smutek, czy może czyjś?”. Pomaga to rozpoznać własne granice.",
    "icon": "rodzic",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "emocje",
      "granica"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D11-S1",
    "profile": "MD",
    "day": 11,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "perspektywa",
    "audience": "dziecko",
    "title": "Spójrz na świat z dachu",
    "body": "Wyobraź sobie, że stoisz na dachu Zamku Czasu i widzisz cały dzień, który Cię czeka. Z góry wszystko wygląda spokojniej, prawda?",
    "icon": "krysztal",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "dystans",
      "perspektywa"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D11-S2",
    "profile": "MD",
    "day": 11,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "sluchanie_obu_stron",
    "audience": "dziecko",
    "title": "Misja: nie przerywaj przez minutę",
    "body": "Gdy ktoś będzie mówił, daj mu skończyć — nawet jeśli już wiesz, co chciał powiedzieć. Cisza to też jest część rozmowy.",
    "icon": "mentor",
    "tone": "paper",
    "time": "1 min",
    "tags": [
      "sluchanie",
      "cisza"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D11-S3",
    "profile": "MD",
    "day": 11,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "rytmy_dnia",
    "audience": "rodzic",
    "title": "Stała pora snu — most do jutra",
    "body": "Mediator potrzebuje przewidywalności wieczora. Ta sama godzina, te same czynności (mycie zębów, książka, światło). Rytuał uspokaja wrażliwy system nerwowy.",
    "icon": "rodzic",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "sen",
      "rytual"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D12-S1",
    "profile": "MD",
    "day": 12,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "slowa_lacznikow",
    "audience": "dziecko",
    "title": "Cztery słowa, które otwierają",
    "body": "„Rozumiem”, „opowiedz mi”, „co czujesz”, „dziękuję”. Wybierz dzisiaj jedno z nich i użyj go choć raz. Zobacz, co się stanie.",
    "icon": "medrzec",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "slowa",
      "otwarcie"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D12-S2",
    "profile": "MD",
    "day": 12,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "konflikt_rowiesniczy",
    "audience": "dziecko",
    "title": "Misja: pomóż, ale nie wybieraj stron",
    "body": "Gdy ktoś prosi: „powiedz, kto ma rację”, możesz odpowiedzieć: „Słyszę was oboje”. Mędrczyni mówi: most nie wybiera brzegu.",
    "icon": "mentor",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "konflikt",
      "neutralnosc"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D12-S3",
    "profile": "MD",
    "day": 12,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "granica_mediatora",
    "audience": "rodzic",
    "title": "Kiedy dziecko mówi „nie wiem, co robić”",
    "body": "Nie zawsze trzeba podpowiadać rozwiązanie. Czasem wystarczy: „Nie musisz tego rozwiązywać”. Dajecie wtedy zgodę na bycie dzieckiem, nie sędzią.",
    "icon": "rodzic",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "granica",
      "zgoda"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D13-S1",
    "profile": "MD",
    "day": 13,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "spokoj_dystans",
    "audience": "dziecko",
    "title": "Waga w sercu — co dziś przewazy?",
    "body": "Wyobraź sobie wagę w swoim sercu. Po jednej stronie spokój, po drugiej pośpiech. Co dzisiaj chcesz, żeby przewazyło?",
    "icon": "medrzec",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "waga",
      "spokoj"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D13-S2",
    "profile": "MD",
    "day": 13,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "kompromis_negocjacja",
    "audience": "dziecko",
    "title": "Misja: ustal małą zasadę",
    "body": "Z bratem, siostrą lub kolegą ustal jedną prostą zasadę na dzień (np. „kto pierwszy skończy, ten wybiera”). Wspólna zasada to wspólny most.",
    "icon": "mentor",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "zasada",
      "kompromis"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D13-S3",
    "profile": "MD",
    "day": 13,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "slowa_lacznikow",
    "audience": "rodzic",
    "title": "Pytanie, które kończy dobrze dzień",
    "body": "Zamiast „jak było w szkole?” spróbujcie: „Co dzisiaj było dla Ciebie najtrudniejsze, a co najjaśniejsze?”. Dwie strony tego samego mostu.",
    "icon": "rodzic",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "pytanie",
      "wieczor"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D14-S1",
    "profile": "MD",
    "day": 14,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "cudza_emocja_a_moja",
    "audience": "dziecko",
    "title": "Niewidzialny płaszcz",
    "body": "Wyobraź sobie, że wkładasz lekki, świetlisty płaszcz. Cudze nastroje mogą po nim spływać, jak deszcz po liściu. Ty zostajesz sobą.",
    "icon": "medrzec",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "ochrona",
      "wizualizacja"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D14-S2",
    "profile": "MD",
    "day": 14,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "perspektywa",
    "audience": "dziecko",
    "title": "Misja: zamień miejsce w wyobraźni",
    "body": "Gdy z kimś się nie zgadzasz, zamknij na chwilę oczy i wyobraź sobie, że stoisz po jego stronie. Co teraz widzisz inaczej?",
    "icon": "mentor",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "perspektywa",
      "wyobraznia"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D14-S3",
    "profile": "MD",
    "day": 14,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "cudza_emocja_a_moja",
    "audience": "rodzic",
    "title": "Nazwijcie emocje, które weszły dziś do domu",
    "body": "„Z czyich emocji dzisiaj coś przyniosłeś?”. Mediatorzy często nieświadomie przenoszą cudze uczucia. Nazwanie ich pomaga zostawić je za drzwiami.",
    "icon": "rodzic",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "emocje",
      "rytual"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D15-S1",
    "profile": "MD",
    "day": 15,
    "slot": "poranek",
    "category": "kraina",
    "subcategory": "perspektywa",
    "audience": "dziecko",
    "title": "Komnata Dwóch Luster",
    "body": "W Zamku Czasu jest komnata z dwoma lustrami naprzeciwko siebie. Mędrczyni mówi: tam każda historia ma dwa odbicia. Pamiętaj o tym dzisiaj.",
    "icon": "krysztal",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "zamek",
      "lustro"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D15-S2",
    "profile": "MD",
    "day": 15,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "slowa_lacznikow",
    "audience": "dziecko",
    "title": "Misja: powiedz „dziękuję, że mi powiedziałeś”",
    "body": "Gdy ktoś podzieli się dziś z Tobą czymś trudnym, podziękuj mu za zaufanie. To krótkie zdanie zostaje w sercu na długo.",
    "icon": "mentor",
    "tone": "paper",
    "time": "1 min",
    "tags": [
      "wdziecznosc",
      "zaufanie"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D15-S3",
    "profile": "MD",
    "day": 15,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "rytmy_dnia",
    "audience": "rodzic",
    "title": "Półmetek — krótka rozmowa o tym, co działa",
    "body": "Minęło 15 dni. Zapytajcie dziecko: „Co z naszych wieczornych rytuałów lubisz najbardziej?”. Wzmocnijcie to, co już buduje most.",
    "icon": "rodzic",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "polmetek",
      "rytm"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D16-S1",
    "profile": "MD",
    "day": 16,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "sluchanie_obu_stron",
    "audience": "dziecko",
    "title": "Echo, które zwraca słowa",
    "body": "Mędrczyni mówi: dobre słuchanie jest jak echo — odbija to, co słyszy. Dzisiaj spróbuj powtórzyć krótko to, co ktoś Ci powiedział.",
    "icon": "medrzec",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "echo",
      "sluchanie"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D16-S2",
    "profile": "MD",
    "day": 16,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "spokoj_dystans",
    "audience": "dziecko",
    "title": "Misja: krok w tył",
    "body": "Gdy zrobi się głośno albo nerwowo, zrób mały krok w tył — fizycznie albo w wyobraźni. Z dalszej odległości łatwiej zobaczyć most.",
    "icon": "mentor",
    "tone": "paper",
    "time": "1 min",
    "tags": [
      "dystans",
      "spokoj"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D16-S3",
    "profile": "MD",
    "day": 16,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "granica_mediatora",
    "audience": "rodzic",
    "title": "Zauważcie, kiedy dziecko bierze za dużo",
    "body": "Sygnały przeciążenia: zmęczenie, drażliwość, trudności ze snem, ból brzucha. Wtedy mniej zadań „naprawiania” innych, więcej zabawy dla siebie.",
    "icon": "rodzic",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "sygnaly",
      "przeciazenie"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D17-S1",
    "profile": "MD",
    "day": 17,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "nie-mediowanie",
    "audience": "dziecko",
    "title": "Dzień, w którym tylko jestem",
    "body": "Mędrczyni mówi: dzisiaj nie musisz nic naprawiać, łączyć ani tłumaczyć. Możesz po prostu być. To też jest mądrość mostu.",
    "icon": "medrzec",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "bycie",
      "odpoczynek"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D17-S2",
    "profile": "MD",
    "day": 17,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "kompromis_negocjacja",
    "audience": "dziecko",
    "title": "Misja: zamiana na dziś",
    "body": "Spróbuj zamienić się z kimś czymś małym — miejscem przy stole, kolejnością, kredką. Drobna zamiana uczy dużej sztuki kompromisu.",
    "icon": "mentor",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "zamiana",
      "kompromis"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D17-S3",
    "profile": "MD",
    "day": 17,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "slowa_lacznikow",
    "audience": "rodzic",
    "title": "Unikajmy „musisz” i „powinieneś”",
    "body": "Zamiast „musisz przeprosić” — „jak myślisz, co teraz pomogłoby?”. Mediator uczy się przez modelowanie, nie nakaz. Wasze słowa to jego słownik.",
    "icon": "rodzic",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "jezyk",
      "modelowanie"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D18-S1",
    "profile": "MD",
    "day": 18,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "perspektywa",
    "audience": "dziecko",
    "title": "Trzecia strona historii",
    "body": "Mędrczyni szepcze: w każdej kłótni są trzy strony — Twoja, jego i prawda gdzieś pośrodku. Dzisiaj pomyśl o tej trzeciej.",
    "icon": "medrzec",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "perspektywa",
      "prawda"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D18-S2",
    "profile": "MD",
    "day": 18,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "sluchanie_obu_stron",
    "audience": "dziecko",
    "title": "Misja: pozwól drugiej osobie zacząć",
    "body": "W rozmowie dzisiaj nie zaczynaj pierwsza/pierwszy. Daj komuś innemu otworzyć temat. Czasem dar mediatora to przestrzeń na cudze słowa.",
    "icon": "mentor",
    "tone": "paper",
    "time": "1 min",
    "tags": [
      "sluchanie",
      "przestrzen"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D18-S3",
    "profile": "MD",
    "day": 18,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "rytmy_dnia",
    "audience": "rodzic",
    "title": "Ruch w ciągu dnia — 60 minut",
    "body": "Dziecko-mediator dużo trzyma głową. Codzienny ruch (rower, plac zabaw, taniec) pomaga rozładować napięcie z całego ciała. Krótkie sesje też się liczą.",
    "icon": "rodzic",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "ruch",
      "cialo"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D19-S1",
    "profile": "MD",
    "day": 19,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "granica_mediatora",
    "audience": "dziecko",
    "title": "Mosty, których dziś nie zbudujesz",
    "body": "Mędrczyni mówi: są mosty, których nie potrafisz zbudować. To nie znaczy, że jesteś gorsza/gorszy. To znaczy, że dobrze znasz siebie.",
    "icon": "medrzec",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "granica",
      "samowiedza"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D19-S2",
    "profile": "MD",
    "day": 19,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "konflikt_rowiesniczy",
    "audience": "dziecko",
    "title": "Misja: zaproś dorosłego, gdy trzeba",
    "body": "Jeśli kłótnia robi się za duża (krzyki, popychanie), nie musisz tego dźwigać sama/sam. Zawołaj dorosłego. To też jest mądry ruch.",
    "icon": "mentor",
    "tone": "paper",
    "time": "1 min",
    "tags": [
      "pomoc",
      "doroslego"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D19-S3",
    "profile": "MD",
    "day": 19,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "cudza_emocja_a_moja",
    "audience": "rodzic",
    "title": "Rytuał „strząsania” dnia",
    "body": "Wieczorem stańcie razem, potrząśnijcie rękami i nogami przez 30 sekund. Symboliczne „strząsanie” cudzych emocji — proste, a działa na ciało.",
    "icon": "rodzic",
    "tone": "rose",
    "time": "1 min",
    "tags": [
      "rytual",
      "cialo"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D20-S1",
    "profile": "MD",
    "day": 20,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "slowa_lacznikow",
    "audience": "dziecko",
    "title": "„I” zamiast „ale”",
    "body": "Mędrczyni zdradza sekret: zamiast „masz rację, ale…” spróbuj „masz rację i…”. To małe słówko zmienia mur w kładkę.",
    "icon": "medrzec",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "slowa",
      "trik"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D20-S2",
    "profile": "MD",
    "day": 20,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "perspektywa",
    "audience": "dziecko",
    "title": "Misja: zapytaj „dlaczego tak myślisz?”",
    "body": "Gdy ktoś powie coś, z czym się nie zgadzasz, zamiast się kłócić — zapytaj „dlaczego tak myślisz?”. Mędrczyni mówi: pytanie jest mądrzejsze od sporu.",
    "icon": "mentor",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "pytanie",
      "ciekawosc"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D20-S3",
    "profile": "MD",
    "day": 20,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "granica_mediatora",
    "audience": "rodzic",
    "title": "Nie czyńcie dziecka swoim powiernikiem",
    "body": "Nawet jeśli dziecko świetnie słucha — Wasze trudne sprawy dorosłe omawiajcie z innym dorosłym. Mediator-dziecko musi mieć dzieciństwo.",
    "icon": "rodzic",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "ochrona",
      "granica"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D21-S1",
    "profile": "MD",
    "day": 21,
    "slot": "poranek",
    "category": "kraina",
    "subcategory": "spokoj_dystans",
    "audience": "dziecko",
    "title": "Wielka klepsydra Zamku Czasu",
    "body": "W środku Zamku stoi ogromna klepsydra. Mędrczyni mówi: gdy świat się spieszy, popatrz, jak piasek leci spokojnie. Dzisiaj Ty bądź jak ten piasek.",
    "icon": "krysztal",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "zamek",
      "klepsydra"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D21-S2",
    "profile": "MD",
    "day": 21,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "kompromis_negocjacja",
    "audience": "dziecko",
    "title": "Misja: lista „co dla nas obojga”",
    "body": "Gdy z kimś coś planujecie, wymyślcie razem listę 3 rzeczy, które obojgu się podobają. Łatwiej wybrać z tego, co łączy.",
    "icon": "mentor",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "lista",
      "kompromis"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D21-S3",
    "profile": "MD",
    "day": 21,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "slowa_lacznikow",
    "audience": "rodzic",
    "title": "Powtarzanie uczuć — proste narzędzie",
    "body": "Gdy dziecko mówi „byłam zła”, odpowiedzcie: „byłaś zła…” i poczekajcie. Powtórzenie nazywa, walidauje, otwiera dalsze opowiadanie. Bez doradzania.",
    "icon": "rodzic",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "walidacja",
      "powtorzenie"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D22-S1",
    "profile": "MD",
    "day": 22,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "cudza_emocja_a_moja",
    "audience": "dziecko",
    "title": "Sprawdź, czyje to jest",
    "body": "Jeśli rano czujesz coś dziwnego, zapytaj siebie: „Czy to moje, czy ktoś mi to dał?”. Mędrczyni mówi: nie wszystko, co czujesz, jest Twoje.",
    "icon": "medrzec",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "emocje",
      "granica"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D22-S2",
    "profile": "MD",
    "day": 22,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "sluchanie_obu_stron",
    "audience": "dziecko",
    "title": "Misja: dwa zdania od każdej strony",
    "body": "Jeśli ktoś chce, żebyś pomógł rozsądzić spór — poproś, by każda osoba powiedziała tylko dwa zdania. Krótko = uczciwiej dla obu.",
    "icon": "mentor",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "sluchanie",
      "zasada"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D22-S3",
    "profile": "MD",
    "day": 22,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "rytmy_dnia",
    "audience": "rodzic",
    "title": "Posiłki o stałych porach — kotwica dnia",
    "body": "Mediator dużo daje innym. Regularne pory jedzenia (śniadanie, drugie śniadanie, obiad, podwieczorek, kolacja) trzymają układ nerwowy w pionie.",
    "icon": "rodzic",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "jedzenie",
      "rytm"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D23-S1",
    "profile": "MD",
    "day": 23,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "spokoj_dystans",
    "audience": "dziecko",
    "title": "Wstąga, która tańczy spokojnie",
    "body": "Wyobraź sobie swoją Wstęgę Łączeń — lekka, świetlista, tańczy w powietrzu. Niech dzisiaj porusza się powoli, bez nerwów. Twoje tempo, Twój rytm.",
    "icon": "artifact",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "wstega",
      "spokoj"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D23-S2",
    "profile": "MD",
    "day": 23,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "konflikt_rowiesniczy",
    "audience": "dziecko",
    "title": "Misja: powiedz „chwila, oddychamy”",
    "body": "Gdy spór się rozgrzewa, zaproponuj krótką przerwę: „chwila, oddychamy”. Mędrczyni mówi: pauza to też część rozmowy.",
    "icon": "mentor",
    "tone": "paper",
    "time": "1 min",
    "tags": [
      "pauza",
      "konflikt"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D23-S3",
    "profile": "MD",
    "day": 23,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "perspektywa",
    "audience": "rodzic",
    "title": "Pytajcie o dwa punkty widzenia",
    "body": "Po opowieści dziecka o sporze zapytajcie: „A jak myślisz, co mogło czuć drugie dziecko?”. To nie usprawiedliwianie — to trening perspektywy.",
    "icon": "rodzic",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "perspektywa",
      "rozmowa"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D24-S1",
    "profile": "MD",
    "day": 24,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "nie-mediowanie",
    "audience": "dziecko",
    "title": "Dziś jestem tylko dzieckiem",
    "body": "Mędrczyni przypomina: dzisiaj możesz się bawić, śmiać, biegać, nic nie naprawiać. Bycie dzieckiem to też ważna część mostu.",
    "icon": "medrzec",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "zabawa",
      "dziecko"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D24-S2",
    "profile": "MD",
    "day": 24,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "slowa_lacznikow",
    "audience": "dziecko",
    "title": "Misja: jedno szczere „dziękuję”",
    "body": "Powiedz dzisiaj komuś szczere „dziękuję” — nie z grzeczności, ale z serca. Wybierz konkret: „dziękuję, że…”. Mosty wdzięczności są mocne.",
    "icon": "mentor",
    "tone": "paper",
    "time": "1 min",
    "tags": [
      "wdziecznosc",
      "konkret"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D24-S3",
    "profile": "MD",
    "day": 24,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "cudza_emocja_a_moja",
    "audience": "rodzic",
    "title": "Krótka „skala” wieczorna",
    "body": "Spytajcie: „Ile masz dziś w sobie swojego nastroju od 1 do 10, a ile czyjegoś?”. Zabawne, ale uczy rozróżniania źródła uczuć.",
    "icon": "rodzic",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "skala",
      "emocje"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D25-S1",
    "profile": "MD",
    "day": 25,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "perspektywa",
    "audience": "dziecko",
    "title": "Most ma dwie głowice",
    "body": "Każdy most opiera się na dwóch brzegach. Mędrczyni mówi: dzisiaj zauważ, że i Ty masz prawo być jednym z brzegów, nie tylko deską między nimi.",
    "icon": "medrzec",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "most",
      "granica"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D25-S2",
    "profile": "MD",
    "day": 25,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "kompromis_negocjacja",
    "audience": "dziecko",
    "title": "Misja: po trochu dla każdego",
    "body": "Gdy dzielisz coś z kimś (czas, miejsce, zabawkę), spróbuj: „trochę Ty, trochę ja”. Sprawiedliwie nie zawsze znaczy „po równo”, ale „uczciwie”.",
    "icon": "mentor",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "sprawiedliwosc",
      "podzial"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D25-S3",
    "profile": "MD",
    "day": 25,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "granica_mediatora",
    "audience": "rodzic",
    "title": "Konflikty dorosłych — wyraźnie poza dzieckiem",
    "body": "Jeśli zdarzy się spięcie między dorosłymi, powiedzcie dziecku: „To nasza sprawa, nie musisz się tym zajmować”. Ulga jest natychmiastowa.",
    "icon": "rodzic",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "granica",
      "doroslych"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D26-S1",
    "profile": "MD",
    "day": 26,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "sluchanie_obu_stron",
    "audience": "dziecko",
    "title": "Słuchaj nie tylko słów",
    "body": "Mędrczyni szepcze: gdy ktoś mówi, spójrz też na jego twarz i oczy. Czasem to, co najważniejsze, jest między słowami.",
    "icon": "medrzec",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "sluchanie",
      "twarz"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D26-S2",
    "profile": "MD",
    "day": 26,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "spokoj_dystans",
    "audience": "dziecko",
    "title": "Misja: trzymaj swój spokój",
    "body": "Gdy ktoś koło Ciebie się zdenerwuje, spróbuj zostać spokojna/spokojny. Twój spokój może być jak światło w pokoju — nie krzyczy, ale jest widać.",
    "icon": "mentor",
    "tone": "paper",
    "time": "1 min",
    "tags": [
      "spokoj",
      "swiatlo"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D26-S3",
    "profile": "MD",
    "day": 26,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "rytmy_dnia",
    "audience": "rodzic",
    "title": "Sygnały, że dziecko jest „pełne”",
    "body": "Mediator pełen cudzych spraw mówi mniej, gryzie paznokcie, ma mokre oczy bez powodu. To moment na ciszę, kąpiel, przytulenie — bez wypytywania.",
    "icon": "rodzic",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "sygnaly",
      "regulacja"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D27-S1",
    "profile": "MD",
    "day": 27,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "slowa_lacznikow",
    "audience": "dziecko",
    "title": "„Razem” — jedno z najmocniejszych słów",
    "body": "Mędrczyni mówi: gdy poczujesz, że ktoś jest sam ze swoim problemem, powiedz „pomyślmy razem”. Słowo „razem” jest jak Twoja Wstęga.",
    "icon": "medrzec",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "razem",
      "wstega"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D27-S2",
    "profile": "MD",
    "day": 27,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "konflikt_rowiesniczy",
    "audience": "dziecko",
    "title": "Misja: powtórz spokojnie, co usłyszysz",
    "body": "Jeśli ktoś przy Tobie krzyczy, powtórz to spokojnym głosem, ale bez przedrzeźniania: „mówisz, że to było niesprawiedliwe?”. Spokój wygładza fale.",
    "icon": "mentor",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "spokoj",
      "powtorzenie"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D27-S3",
    "profile": "MD",
    "day": 27,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "slowa_lacznikow",
    "audience": "rodzic",
    "title": "Mówcie głośno, gdy coś dobrze rozwiązaliście",
    "body": "Po małym sporze w domu powiedzcie wprost: „Cieszę się, że się dogadaliśmy”. Dziecko uczy się, że konflikt może mieć dobry koniec.",
    "icon": "rodzic",
    "tone": "amber",
    "time": "1 min",
    "tags": [
      "modelowanie",
      "konflikt"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D28-S1",
    "profile": "MD",
    "day": 28,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "granica_mediatora",
    "audience": "dziecko",
    "title": "Mam prawo powiedzieć „nie potrafię”",
    "body": "Mędrczyni mówi: nawet najmądrzejszy most czasem powie „nie udźwignę”. To nie porażka — to uczciwość wobec siebie. Bardzo Ci wolno.",
    "icon": "medrzec",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "nie",
      "granica"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D28-S2",
    "profile": "MD",
    "day": 28,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "perspektywa",
    "audience": "dziecko",
    "title": "Misja: zapytaj „jak Ty to widzisz?”",
    "body": "Wybierz jedną osobę i zapytaj ją o jej zdanie w czymś małym (np. co zjeść, gdzie usiąść). Pytanie o perspektywę buduje zaufanie.",
    "icon": "mentor",
    "tone": "paper",
    "time": "1 min",
    "tags": [
      "pytanie",
      "zaufanie"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D28-S3",
    "profile": "MD",
    "day": 28,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "cudza_emocja_a_moja",
    "audience": "rodzic",
    "title": "„Co dzisiaj było tylko Twoje?”",
    "body": "To pytanie zamyka dzień dla dziecka-mediatora najlepiej. Pomaga znaleźć w sobie własne ślady — radość, smutek, dumę — wśród cudzych emocji.",
    "icon": "rodzic",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "pytanie",
      "ja"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D29-S1",
    "profile": "MD",
    "day": 29,
    "slot": "poranek",
    "category": "medrzec",
    "subcategory": "spokoj_dystans",
    "audience": "dziecko",
    "title": "Wdech na cztery, wydech na sześć",
    "body": "Wdychaj powietrze licząc do czterech, wydychaj do sześciu. Powtórz trzy razy. Mędrczyni mówi: dłuższy wydech to klucz do spokojnego dnia.",
    "icon": "medrzec",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "oddech",
      "spokoj"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D29-S2",
    "profile": "MD",
    "day": 29,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "kompromis_negocjacja",
    "audience": "dziecko",
    "title": "Misja: ustąp w czymś małym",
    "body": "Dzisiaj świadomie ustąp komuś w jednej małej rzeczy. Nie dlatego, że musisz, ale dlatego, że możesz. To moc, nie słabość.",
    "icon": "mentor",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "ustepstwo",
      "moc"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D29-S3",
    "profile": "MD",
    "day": 29,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "rytmy_dnia",
    "audience": "rodzic",
    "title": "Choroba — zawsze pediatra, nie suplementy",
    "body": "Jeśli dziecko częściej choruje, ma długie bóle brzucha lub głowy — to sprawa dla pediatry. Żadne suplementy ani diety bez konsultacji. Najpierw lekarz.",
    "icon": "rodzic",
    "tone": "amber",
    "time": "2 min",
    "tags": [
      "zdrowie",
      "lekarz"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D30-S1",
    "profile": "MD",
    "day": 30,
    "slot": "poranek",
    "category": "kraina",
    "subcategory": "perspektywa",
    "audience": "dziecko",
    "title": "Brama Zamku Czasu otwiera się szeroko",
    "body": "Po 30 dniach Mędrczyni uśmiecha się do Ciebie. Twoja Wstęga Łączeń jest dłuższa i jaśniejsza. Most w Tobie wie więcej niż na początku.",
    "icon": "krysztal",
    "tone": "magic",
    "time": "1 min",
    "tags": [
      "zamek",
      "podsumowanie"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D30-S2",
    "profile": "MD",
    "day": 30,
    "slot": "poludnie",
    "category": "misje",
    "subcategory": "slowa_lacznikow",
    "audience": "dziecko",
    "title": "Misja: powiedz, co się nauczyłaś/nauczyłeś",
    "body": "Komuś bliskiemu opowiedz jedną rzecz, którą zrozumiałaś/zrozumiałeś przez te 30 dni o sobie albo o ludziach. Dzielenie się to też most.",
    "icon": "mentor",
    "tone": "paper",
    "time": "2 min",
    "tags": [
      "podsumowanie",
      "rozmowa"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  },
  {
    "id": "MD-D30-S3",
    "profile": "MD",
    "day": 30,
    "slot": "wieczor",
    "category": "mentor",
    "subcategory": "granica_mediatora",
    "audience": "rodzic",
    "title": "Dziecko, które łączy — chronicie cuda",
    "body": "Wasze dziecko nosi w sobie rzadki dar mostu. Wasza rola: chronić go, by nie został nadużyty. Dziecko ma być dzieckiem, nawet jeśli pięknie słucha.",
    "icon": "rodzic",
    "tone": "rose",
    "time": "2 min",
    "tags": [
      "ochrona",
      "podsumowanie"
    ],
    "profileName": "Strażnik Mostu",
    "cecha": "Skupienie",
    "kraina": "Zamek Czasu"
  }
];

// ─── Helpers do filtrowania ─────────────────────────────────────────

/** Wszystkie porady dla danego profilu. */
export function tipsForProfile(profile) {
  return DAILY_TIPS.filter(t => t.profile === profile);
}

/** 3 porady dnia (poranek/poludnie/wieczor) dla profilu i dnia 1-30. */
export function tipsForDay(profile, day) {
  return DAILY_TIPS.filter(t => t.profile === profile && t.day === day)
    .sort((a, b) => {
      const order = { poranek: 1, poludnie: 2, wieczor: 3 };
      return order[a.slot] - order[b.slot];
    });
}

/** Pojedyncza porada profilu/dnia/slotu. */
export function tip(profile, day, slot) {
  return DAILY_TIPS.find(t => t.profile === profile && t.day === day && t.slot === slot) || null;
}

/** Porada "dzisiejsza" wyliczona z daty — używaj jako fallback Porady Dnia. */
export function todaysTip(profile, date = new Date(), slot = null) {
  const day = ((date.getDate() - 1) % 30) + 1; // 1-30 cykl miesieczny
  if (slot) return tip(profile, day, slot);
  // domyslnie: wybierz wg pory dnia
  const h = date.getHours();
  const s = h < 12 ? 'poranek' : h < 18 ? 'poludnie' : 'wieczor';
  return tip(profile, day, s);
}

/** Filtr per kategoria (medrzec/misje/kraina/mentor) lub subcategory. */
export function tipsByCategory(profile, category) {
  return DAILY_TIPS.filter(t => t.profile === profile && t.category === category);
}
export function tipsBySubcategory(profile, subcategory) {
  return DAILY_TIPS.filter(t => t.profile === profile && t.subcategory === subcategory);
}

/** Filtr per audience (dziecko/rodzic). */
export function tipsForAudience(profile, audience) {
  return DAILY_TIPS.filter(t => t.profile === profile && t.audience === audience);
}

/** Lista subkategorii dostępnych dla profilu — do budowy chipów w UI. */
export function subcategoriesForProfile(profile) {
  const set = new Set();
  DAILY_TIPS.filter(t => t.profile === profile).forEach(t => set.add(t.subcategory));
  return Array.from(set).sort();
}

export default DAILY_TIPS;
